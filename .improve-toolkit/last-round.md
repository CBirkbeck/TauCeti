# Last round — r836 (2026-09-15T13:56Z)
...
**Open question to Chris (asked after r732, unanswered at r836):** `/cleanup` has not run in full on any staged
...
at 05:43:53Z. #6851 and #6854 stay out of the queue until the merge sweep re-enqueues them, and #6875 is 10/10 and waits for a human merge (r813). Main is `6f51dd839`.
...
1. **Queue:** `queuepos.py` each round; act only on `EJECTED`, and first read the removal reason (GraphQL
   ...
   merge-group simulation (cheap, read-only; r836: #6851, #6854 and #6875 clean against `6f51dd839`). Staged branches:
