...
|---|---|---|---|---|
| **#8525** | `8ae84de0c` | building (fix 2) | kind 2 (`KnotTheory/Grid/Grading/Parity.lean`: 47 `↦`, the roadmap sentence dropped; `Roadmap: CombinatorialHeegaardFloer`). Round 1 (07:17Z): `documentation` asked to move the dated provenance sentence to the PR body (done at `e5b5676d1`); round 2 (07:54Z): documentation ✅ but **`attribution` asked to keep the credit in the docstring**. **Fix 2 at `8ae84de0c`** (r1018): credit restored as a plain source citation ("The Lean proof is adapted from Tau Ceti split-branch commit `05c2722248`."), no dated wording; body explains | **CI**, then re-review |
...
**In progress (cap full): #8525 (fix 2 `8ae84de0c` building), #8530 (ready 07:52Z; 08:52Z) and #8531 (ready 08:09Z; 09:09Z). No drafts.** **#8526 merged 08:06:09Z** (r1019), the thirty-fourth.
**Staged (not opened; the cap is full):** `improve/hyperbolic-length-style` @ `f83e4696c` from `83e68369e` (`Conformal/Hyperbolic/Length.lean`: 73 `↦`, one calc chain split, the "Coordination with upstream Mathlib" section reduced to its lasting Mathlib-comparison fact and retitled; gate 12/0/0; astra cleared it; body `pr-body-hyplen.md`, `Roadmap: ConformalMapping` as #1832). **Open it as the next kind 2** when a slot frees. **Declined (r1019):** `Young/Tableau.lean` (a References bullet cites the Schur–Weyl roadmap as a source), `L2/Cesaro/ToCondExp.lean` and `ConditionallyIID/Map.lean` (the roadmap is cited as the name/provenance map to `cameronfreer/exchangeability`), and `ContinuousLog/Basic.lean` (a long "Roadmap role" section of open-frontier status; a docs rewrite, not a style pass). Remaining r976 hits: `ConditionallyIID/Construct.lean`, `ContCohomology/ShortExact.lean`, `Arrays/Dissociated.lean`, `DeFinetti/BlockFactorization.lean`; or re-score.
...
:03/:13/…/:53); it is session-only and expires 2026-09-30. **Handover:** `fork handover/improve-toolkit` tip is `978487c7c`
