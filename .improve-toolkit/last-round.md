# Last round — r756 (2026-09-14T22:47Z)
...
**Open question to Chris (asked after r732, unanswered at r756):** `/cleanup` has not run in full on any staged
...
|---|---|---|---|---|
| **#6851** | `fdeaff5cb7` | green | kind 1; **10/10 first board** (22:35:00Z), `ready-to-merge`, **QUEUED pos 32/32** | nobody — act only if `queuepos.py` says `EJECTED` |
| **#6854** | `f18fe6fedf` | **green** (`sandboxed-build` 21:45:37Z → 22:02:23Z) | kind 2; **ready 22:07:11Z**, no board at 22:46Z (39 min) | **pipeline** — drive only if no board by **23:07:11Z** |
| **#6855** | `352c92a114` | green | kind 3; **10/10 first board** (22:32:04Z), `ready-to-merge`, **QUEUED pos 30/32** | nobody — act only if `queuepos.py` says `EJECTED` |
...
sit about 30 deep in the queue. Main is `e2773303e`.
...
1. **Queue:** `queuepos.py` each round; act only on `EJECTED`. A `MERGING` PR's group build is the check-runs of
   ...
   merge-group simulation (cheap, read-only; r756: #6851 and #6855 clean against `e2773303e`). Staged branches:
