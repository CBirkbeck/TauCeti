# Last round — r786 (2026-09-15T03:47Z)
...
**Open question to Chris (asked after r732, unanswered at r786):** `/cleanup` has not run in full on any staged
...
|---|---|---|---|---|
| **#6851** | `fdeaff5cb7` | green | kind 1; **10/10 first board** (22:35:00Z), `ready-to-merge`, **QUEUED pos 10/41** | nobody — act only if `queuepos.py` says `EJECTED` |
| **#6854** | `217fecb812` | green | kind 2; **10/10** (r768 driven board, 00:49:59Z), `ready-to-merge`, **QUEUED pos 22/41** | nobody — act only if `queuepos.py` says `EJECTED` |
| **#6855** | `352c92a114` | green | kind 3; **10/10 first board** (22:32:04Z), `ready-to-merge`, **QUEUED pos 8/41** | nobody — act only if `queuepos.py` says `EJECTED` |
...
queued (#6855 8th, #6851 10th, #6854 22nd). Main is `8a6ed17ba`.
...
1. **Queue:** `queuepos.py` each round; act only on `EJECTED`. A `MERGING` PR's group build is the check-runs of
   ...
   merge-group simulation (cheap, read-only; r786: #6851, #6854 and #6855 clean against `8a6ed17ba`). Staged branches:
