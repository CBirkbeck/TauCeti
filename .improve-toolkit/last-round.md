# Last round — r769 (2026-09-15T00:58Z)
...
**Open question to Chris (asked after r732, unanswered at r769):** `/cleanup` has not run in full on any staged
...
|---|---|---|---|---|
| **#6851** | `fdeaff5cb7` | green | kind 1; **10/10 first board** (22:35:00Z), `ready-to-merge`, **QUEUED pos 21/31** | nobody — act only if `queuepos.py` says `EJECTED` |
| **#6854** | `217fecb81` | green | kind 2; **10/10 on the r768 driven board** (00:50:12Z, head `217fecb81`), after r759's `scope` fix; label still `awaiting-review` at 00:57Z, board ON-HEAD (00:49:59Z) | **pipeline** — its relabel waits in the Actions backlog (104 queued runs; `PR status labels` on `issue_comment` queued or pending since 00:52Z). Never label by hand; wait |
| **#6855** | `352c92a114` | green | kind 3; **10/10 first board** (22:32:04Z), `ready-to-merge`, **QUEUED pos 19/31** | nobody — act only if `queuepos.py` says `EJECTED` |
...
sit about 20 deep in the queue. Main is `ea41df940`.
...
1. **Queue:** `queuepos.py` each round; act only on `EJECTED`. A `MERGING` PR's group build is the check-runs of
   ...
   merge-group simulation (cheap, read-only; r769: #6851 and #6855 clean against `ea41df940`). Staged branches:
...
3. **#6854 (kind 2, quadratic separability) is 10/10** on the r768 driven board (00:50:12Z, head `217fecb81`, $0.91
   ...
   Its relabel waits in the Actions backlog: at 00:57Z the board was ON-HEAD but the label still read `awaiting-review`,
   and the `PR status labels` runs on `issue_comment` were queued or pending (104 queued runs repo-wide). The label is
   the pipeline's, so never add `ready-to-merge` by hand. Re-check each round.
