# Last round — r787 (2026-09-15T03:57Z)
...
**Open question to Chris (asked after r732, unanswered at r787):** `/cleanup` has not run in full on any staged
...
|---|---|---|---|---|
| **#6851** | `fdeaff5cb7` | green | kind 1; **10/10 first board** (22:35:00Z), `ready-to-merge`, **QUEUED pos 10/42** | nobody — act only if `queuepos.py` says `EJECTED` |
| **#6854** | `217fecb812` | green | kind 2; **10/10** (r768 driven board, 00:49:59Z), `ready-to-merge`, **QUEUED pos 22/42** | nobody — act only if `queuepos.py` says `EJECTED` |
| **#6855** | `352c92a114` | green | kind 3; **10/10 first board** (22:32:04Z), `ready-to-merge`, **QUEUED pos 8/42** | nobody — act only if `queuepos.py` says `EJECTED` |
...
queued (#6855 8th, #6851 10th, #6854 22nd). Main is `bd11a0a35`.
...
1. **Queue:** `queuepos.py` each round; act only on `EJECTED`. A `MERGING` PR's group build is the check-runs of
   ...
   merge-group simulation (cheap, read-only; r786 against `8a6ed17ba`, still valid at `bd11a0a35`, which changes no Lean file). Staged branches:
...
there). Diagnose with `du -sh /tmp/claude-1001/*/*`; at r773 one other session held 24G of the 25G, 27G at r775 and 32G at r783 (new Lean worktrees), still growing. Never delete
