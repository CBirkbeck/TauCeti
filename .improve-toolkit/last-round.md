# Last round — r790 (2026-09-15T04:27Z)
...
**Open question to Chris (asked after r732, unanswered at r790):** `/cleanup` has not run in full on any staged
...
|---|---|---|---|---|
| **#6851** | `fdeaff5cb7` | green | kind 1; **10/10 first board** (22:35:00Z), `ready-to-merge`, **QUEUED pos 8/43** | nobody — act only if `queuepos.py` says `EJECTED` |
| **#6854** | `217fecb812` | green | kind 2; **10/10** (r768 driven board, 00:49:59Z), `ready-to-merge`, **QUEUED pos 20/43** | nobody — act only if `queuepos.py` says `EJECTED` |
| **#6855** | `352c92a114` | green | kind 3; **10/10 first board** (22:32:04Z), `ready-to-merge`, **QUEUED pos 6/43** | nobody — act only if `queuepos.py` says `EJECTED` |
...
queued (#6855 6th, #6851 8th, #6854 20th). Main is `3f1ee32fe`.
...
1. **Queue:** `queuepos.py` each round; act only on `EJECTED`. A `MERGING` PR's group build is the check-runs of
   ...
   merge-group simulation (cheap, read-only; r790: #6851, #6854 and #6855 clean against `3f1ee32fe`). Staged branches:
