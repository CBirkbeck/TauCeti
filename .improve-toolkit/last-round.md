# Last round — r758 (2026-09-14T23:07Z)
...
**Open question to Chris (asked after r732, unanswered at r758):** `/cleanup` has not run in full on any staged
...
|---|---|---|---|---|
| **#6851** | `fdeaff5cb7` | green | kind 1; **10/10 first board** (22:35:00Z), `ready-to-merge`, **QUEUED pos 31/33** | nobody — act only if `queuepos.py` says `EJECTED` |
| **#6854** | `f18fe6fedf` | **green** (`sandboxed-build` 21:45:37Z → 22:02:23Z) | kind 2; **ready 22:07:11Z**, no board at 23:06Z (59 min) | **pipeline** — drive only if no board by **23:07:11Z** |
| **#6855** | `352c92a114` | green | kind 3; **10/10 first board** (22:32:04Z), `ready-to-merge`, **QUEUED pos 29/33** | nobody — act only if `queuepos.py` says `EJECTED` |
...
sit about 30 deep in the queue. Main is `babe56615`.
...
1. **Queue:** `queuepos.py` each round; act only on `EJECTED`. A `MERGING` PR's group build is the check-runs of
   ...
   merge-group simulation (cheap, read-only; r757: #6851 and #6855 clean against `babe56615`). Staged branches:
...
3. **#6854 (kind 2, quadratic separability) is ready and awaiting its first board.** CI went green at 22:02:23Z
   ...
   23:07:11Z.** **The 23:16 round drives if there is still no board** (none at 23:06Z, 59 min in; r758 held off one
   round inside the 32–67 min band).
