...
at 05:43:53Z. #6851 and #6854 stay out of the queue until the merge sweep re-enqueues them, and #6875 is 10/10 and waits for a human merge (r813). Main is `82f5e9ac2`.
...
1. **Queue:** `queuepos.py` each round; act only on `EJECTED`, and first read the removal reason (GraphQL
   ...
   merge-group simulation (cheap, read-only; r832: #6851, #6854 and #6875 clean against `82f5e9ac2`). Staged branches:
