...
|---|---|---|---|---|
| **#6093** | `18e85bea2` | green | 10/10, `ready-to-merge`, **QUEUED pos 6/39** | nobody — act only if `queuepos.py` says `EJECTED` |
| **#6796** | `06e8f7fdf` | green | 10/10, `ready-to-merge`, **QUEUED pos 7** | nobody |
| **#6800** | `716cf35ee` | green | **10/10** (driven, board 13:16:34Z, $0.98), `ready-to-merge`, **QUEUED pos 8** | nobody |
...
1. **Queue:** `queuepos.py` each round; act only on `EJECTED`. If main moves a lot, re-run r702's
   merge-group simulation (cheap, read-only; r733: all three clean against `a13882323`). Staged branches:
   `git merge-tree --write-tree --name-only origin/main <branch>` checks them without a checkout (r733: all three
   clean against `a13882323`, none of their files touched on main).
