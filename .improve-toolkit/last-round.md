# Last round — r986 (2026-09-24T03:09Z)
...
|---|---|---|---|---|
| **#8491** | `5c0611468` | green (02:02:17Z) | kind 2 (`DenseGraphLimits/Graphon/CutNormLimit.lean`: 54 `↦`, one calc-step chain split; gate 12/0/0; `Roadmap: DenseGraphLimits`). Astra cleared it; **marked ready 02:02:44Z** (r982); no board at 65 min, so **driven** at 03:08Z (r986) | **review drive** |
...
**In progress (cap full): #8491 (driven 03:08Z), #8492 (ready 02:13Z; step 4 no earlier than 03:13Z) and the draft #8497 (CondExp; astra ✓; build queued).** **#8487 merged 03:05:55Z** (r986) after its r986 drive approved it 10/10; twenty-four merged this session.
...
:03/:13/…/:53); it is session-only and expires 2026-09-30. **Handover:** `fork handover/improve-toolkit` tip is `c528f4c85`
...
0. **Now (r986):** #8487 merged (after a drive). In progress (cap full): #8491 (driven 03:08Z), #8492 (ready 02:13Z; 03:13Z) and the
   draft #8497 (CondExp; base `10ecfcfd4`; mark ready when green, after the guards). Drives cost ~$1 each, not ~$16, but the process then hangs ~15–20 min in its archive push to `TauCetiData` (403 retries) after
   ...
   [`CondExp.lean` #8497], [`CutNormLimit.lean` #8491], `Sobolev/W1p/DifferenceQuotient.lean`
