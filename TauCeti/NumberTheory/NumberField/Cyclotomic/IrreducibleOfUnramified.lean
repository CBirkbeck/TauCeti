...
theorem ramificationIdx_eq_totient (p k : ℕ) [Fact p.Prime] {F : Type*} [Field F]
  ...
  have : FiniteDimensional K F := finiteDimensional {p ^ (k + 1)} K F
  have : NumberField F := .of_module_finite K F
  exact le_antisymm (𝔔.ramificationIdx_le_finrank_of_numberField.trans (finrank_le_totient K F))
...
theorem inf_eq_bot_prime_pow_of_unramified {Ω : Type*} [Field Ω] [Algebra K Ω]
  ...
  have hurK : ∀ (𝔮 : Ideal (𝓞 K)) [𝔮.IsPrime] [𝔮.LiesOver (Ideal.span {(p : ℤ)})],
    ...
    TauCeti.RamificationInertia.isUnramifiedAt_of_isUnramifiedIn
      (fun P hP hL ↦ @hur P hP hL) 𝔮
  have htot : 𝔔.ramificationIdx (𝓞 K) = (p ^ (k + 1)).totient :=
    ramificationIdx_eq_totient (K := K) p k hurK 𝔔
  -- The degree is squeezed between `φ(p^(k+1))` on both sides.
  have hBK : Module.finrank K B = (p ^ (k + 1)).totient :=
    le_antisymm (finrank_le_totient K B) (totient_le_finrank_of_unramified p k B hurK)
  have : Algebra.IsUnramifiedAt ℤ (𝔔.under (𝓞 E)) :=
    TauCeti.RamificationInertia.isUnramifiedAt_of_isUnramifiedIn
      (fun P hP hL ↦ @hur P hP hL) _
  ...
  exact IntermediateField.finrank_eq_one_iff.mp
    (Ideal.finrank_eq_one_of_ramificationIdx_eq_finrank 𝔔 (he2.trans hBK.symm))

/-- **A subfield unramified at a prime `q` meets a `q`-th cyclotomic extension trivially.** The
case `k = 0` of `inf_eq_bot_prime_pow_of_unramified`, stated separately because `q` prime — rather
than a prime power — is the hypothesis a Frobenius argument has to hand, and because it takes
`q.Prime` as an argument rather than as an instance. -/
