...
* `IsPrimitiveRoot.isCyclotomicExtension_fixedField_of_inf_fixingSubgroup_eq_bot`
...
theorem IsPrimitiveRoot.isCyclotomicExtension_fixedField_of_inf_fixingSubgroup_eq_bot
  ...
  -- `K(ζ) = K(μ_m)`: both are the field generated by the roots of `Φ_m`
  have hadjζ : adjoin K {ζ} = Kμ := by
