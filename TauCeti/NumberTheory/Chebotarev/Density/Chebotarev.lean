...
theorem hasDirichletDensity_frobeniusPrimeSet (C : ConjClasses (L ≃ₐ[K] L)) :
  ...
  -- `Gal(L / L ^ ⟨σ⟩)` is cyclic, so it is abelian.
  exact AlgEquiv.card_algEquiv_fixedField_zpowers σ ▸ hasDirichletDensity_abelianFrobenius _ L
    (FixedPoints.isCyclic_algEquiv (Subgroup.zpowers σ) L).commGroup.mul_comm σ.toFixedFieldAlgEquiv
...
is `hasDirichletDensity_frobeniusPrimeSet` for the class of the identity. -/
