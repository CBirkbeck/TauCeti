...
the character is an arbitrary degree-one complex character of the Galois group. The cyclotomic
weight is the specialisation in which `L / K` is generated by roots of unity, so that the Galois
group is `(ZMod m)ˣ` and `χ` is a Dirichlet character. The declarations are named for the
generality they actually have.
