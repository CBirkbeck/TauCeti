...
the character is an arbitrary degree-one complex character of the Galois group. The Chebotarev
roadmap (Layer 4) calls this object the *cyclotomic character weight* because it applies it to
cyclotomic extensions, where the Galois group is `(ZMod m)ˣ` and `χ` is a Dirichlet character; the
declarations are named for the generality they actually have, and the roadmap's cyclotomic
specialisation is one instance of them.
...
`MultiplicativeIdealWeight` packaging and the `artinSymbol` totalization are the roadmap's. The
source likewise names the construction for a general Galois character.
-/
