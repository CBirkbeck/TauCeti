...
private theorem radicand_ne_zero_of_root_notMem (d : ℕ → K) (root : ℕ → L)
...
/-- Descent step of `squareClass_of_sq_mem` when the cross-term coefficient vanishes, so that
`y = b · rootₙ` with `b` in the `n`-th tower. Then `y · rootₙ` lies in the `n`-th tower and
squares to `r · dₙ`, so the level-`n` descent hypothesis produces a representation of `r` over
`Set.Iio (n + 1)` with `n` adjoined to the subset. -/
private theorem exists_sq_mul_prod_of_eq_mul_root (d : ℕ → K) (root : ℕ → L)
    (hroot : ∀ j, root j ^ 2 = algebraMap K L (d j)) {n : ℕ} {r : K} {y b : L}
    (hnext : root n ∉ sqrtTower (K := K) root n)
  ...
  · exact eq_sq_mul_prod_insert_of_mul_radicand hnT
      (radicand_ne_zero_of_root_notMem d root hroot hnext) hmul
...
theorem squareClass_of_sq_mem (d : ℕ → K) (root : ℕ → L)
  ...
  | succ n ih =>
      ...
      · have hx2 : root n ^ 2 ∈ sqrtTower (K := K) root n := by
        ...
        · exact exists_sq_mul_prod_of_eq_mul_root d root hroot hnext hb
