...
import TauCeti.NumberTheory.NumberField.RamifiedPrimes
...
* `TauCeti.Multiquadratic.mem_ramifiedPrimes_adjoin_iff_dvd_fundamentalDiscriminant`: the
  ramified primes of the field generated by a square root, including the square radicand `1`.
...
`ℚ(x) = ℚ` is unramified. -/
theorem mem_ramifiedPrimes_adjoin_iff_dvd_fundamentalDiscriminant {M : Type*} [Field M]
    [NumberField M] {a : ℤ} (hsf : Squarefree a) {x : M}
    (hx : x ^ 2 = algebraMap ℤ M a) {p : ℕ} (hp : p.Prime) :
    p ∈ ramifiedPrimes (IntermediateField.adjoin ℚ {x} : IntermediateField ℚ M) ↔
  ...
  · let _ : NumberField (IntermediateField.adjoin ℚ {x} : IntermediateField ℚ M) :=
      NumberField.of_intermediateField _
    have hd : NumberField.discr (IntermediateField.adjoin ℚ {x} : IntermediateField ℚ M) =
        fundamentalDiscriminant a :=
      discr_adjoin_singleton_eq_fundamentalDiscriminant hsf hx hsq
