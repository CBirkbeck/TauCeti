...
The same rewriting turns the two shapes of the quadratic norm form into the single statement
`A² - D * B² = 4 * N(z)` with `D = fundamentalDiscriminant d`, which is the form genus theory
consumes.
...
`NumberField.exists_sq_sub_mul_sq_eq_norm_of_mod_four_ne_one` by `2` otherwise. -/
