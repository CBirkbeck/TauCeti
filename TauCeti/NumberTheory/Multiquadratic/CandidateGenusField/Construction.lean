...
construction: that compositum contains an element squaring to `d`. The canonical `GenusField`
module identifies this compositum with the imaginary quadratic genus field; here we establish the
square-root input used by that characterization.
