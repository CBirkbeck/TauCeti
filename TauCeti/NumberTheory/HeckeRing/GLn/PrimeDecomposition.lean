...
in the splitting theorem itself — `natDiagGL` takes its junk value on a tuple with a zero
entry — while the entrywise helpers below need only that the entries are nonzero, which is all
`Nat.factorization` and `ordCompl` ask for.
...
theorem diagElem_eq_primePowDiag_mul_diagOrdCompl (a : Fin n → ℕ) (ha_pos : ∀ i, 0 < a i) (p : ℕ)
  ...
  exact (diagElem_mul_of_coprime n _ _
    (coprime_prod_primePowDiag_diagOrdCompl n p hp a fun i ↦ (ha_pos i).ne')).symm
