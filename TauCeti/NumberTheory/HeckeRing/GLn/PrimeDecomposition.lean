...
subring `R_p` (Shimura's `R_p`). For `0 < p`, monotonicity is what makes the exponent vector a
divisibility chain, hence a canonical diagonal. `p` itself is left unrestricted, so the
definition carries no hypothesis; at `p = 0` a positive exponent gives `0 ^ e i = 0` and the
generator falls to `natDiagGL`'s junk value, so that reading is the `0 < p` one — in practice
`p` prime.
...
is part of the generating set, not an afterthought: for `0 < p` it is exactly the condition
making the entries a divisibility chain, so each generator names a canonical double coset.

`p` is unrestricted, so no hypothesis is needed to form the subring. The canonical-double-coset
reading is the `0 < p` one: at `p = 0` any positive exponent gives a zero entry, and that
generator is `natDiagGL`'s junk value rather than a diagonal coset. -/
