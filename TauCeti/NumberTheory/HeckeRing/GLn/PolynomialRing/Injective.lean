...
/-- Every element in the image of `evalHom` belongs to `pLocalSubring`. -/
lemma evalHom_mem_pLocalSubring (n : ℕ) [NeZero n] (p : ℕ) (P : MvPolynomial (Fin n) ℤ) :
    evalHom n p P ∈ pLocalSubring n p := by
  apply MvPolynomial.induction_on P
  · intro a
    show evalHom n p (MvPolynomial.C a) ∈ pLocalSubring n p
    rw [show evalHom n p (MvPolynomial.C a) = (a : IntegralHeckeRing n) from
      MvPolynomial.eval₂Hom_C _ _ _, show (a : IntegralHeckeRing n) =
        a • (1 : IntegralHeckeRing n) from (zsmul_one a).symm]
    exact (pLocalSubring n p).zsmul_mem (pLocalSubring n p).one_mem a
  · intro f g hf hg; rw [map_add]; exact (pLocalSubring n p).add_mem hf hg
  · intro f i hf
    rw [map_mul]
    refine (pLocalSubring n p).mul_mem hf ?_
    show evalHom n p (MvPolynomial.X i) ∈ pLocalSubring n p
    rw [show evalHom n p (MvPolynomial.X i) = heckeGen n p i from
      MvPolynomial.eval₂Hom_X' _ _ _]
    exact heckeGen_mem_pLocalSubring n p i

/-- The restricted evaluation homomorphism into `pLocalSubring`. -/
noncomputable def evalHomR (n : ℕ) [NeZero n] (p : ℕ) :
    MvPolynomial (Fin n) ℤ →+* pLocalSubring n p where
  toFun P := ⟨evalHom n p P, evalHom_mem_pLocalSubring n p P⟩
  map_zero' := Subtype.ext (map_zero _)
  map_one' := Subtype.ext (map_one _)
  map_add' x y := Subtype.ext (map_add _ x y)
  map_mul' x y := Subtype.ext (map_mul _ x y)

/-- A product of two basis elements is the structure-constant family of their
representatives. This is the `b₁ = b₂ = 1` case of `HeckeCosetModule.single_mul`; it belongs
beside that lemma in `HeckeRing/Associativity.lean` and should move there when this file is
prepared as a PR (kept local for now so as not to disturb #2202 mid-review). -/
  ...
  have hf_det : ∀ D'', (heckeGen 2 q.1 1 ^ (e 1)) D'' ≠ 0 →
    ...
    have h_eq : diagCoset (fun _ : Fin 2 ↦ q.1 ^ (e 1)) = D'' := by
      ...
      exact hD'' (by rw [show (diagElem (fun _ : Fin 2 ↦ q.1 ^ (e 1))) D'' =
        (Finsupp.single (diagCoset (fun _ : Fin 2 ↦ q.1 ^ (e 1))) (1 : ℤ)) D'' from rfl,
...
lemma support_mul_exists (f g : IntegralHeckeRing 2) (D : HeckeCoset (posDetInt 2) (SLnZ 2) (SLnZ 2))
  ...
  have h : (Finsupp.sum f (fun D₁ b₁ ↦ Finsupp.sum g (fun D₂ b₂ ↦
      b₁ • b₂ • HeckeCosetModule.structureConstants ℤ (SLnZ 2) (SLnZ 2) (SLnZ 2) (HeckeCoset.rep D₁)
...
lemma T_single_diag_mul_T_scalar (c : ℕ) (hc : 0 < c)
  ...
  have h_single : HeckeCosetModule.single ℤ (diagCoset a) α =
      α • diagElem a := by
    show HeckeCosetModule.single ℤ (diagCoset a) α =
         α • HeckeCosetModule.single ℤ (diagCoset a) 1
    rw [HeckeCosetModule.single_smul]; congr 1; ring
  rw [h_single, smul_mul_assoc, diagElem_mul_const a ha_pos ha_div c hc]
  show α • HeckeCosetModule.single ℤ (diagCoset (a * fun _ ↦ c)) 1 = _
  rw [HeckeCosetModule.single_smul]; congr 1; ring
...
lemma T_elem_ppow_factor (p : ℕ) (hp : p.Prime) (i j : ℕ) (hij : i ≤ j) :
  ...
  have h_ji_pos : ∀ idx : Fin 2, 0 < (![1, p^(j-i)] : Fin 2 → ℕ) idx := by
    ...
    · simp
    · simp [pow_pos hp.pos]
  have h_ji_div : IsDvdChain (![1, p^(j-i)] : Fin 2 → ℕ) := by
    intro k hk
    have hk0 : k = 0 := by omega
    subst hk0; simp
  rw [diagElem_mul_const (![1, p^(j-i)] : Fin 2 → ℕ) h_ji_pos h_ji_div (p^i) (pow_pos hp.pos _)]
...
private lemma T_elem_p_ppow_eval_at_one_ppow_succ_zero (p : ℕ) (hp : p.Prime) {n : ℕ}
  ...
  show (Finsupp.single (diagCoset (![p, p ^ n] : Fin 2 → ℕ)) (1 : ℤ)) _ = 0
...
lemma evalHomR_injective (n : ℕ) [NeZero n] (p : ℕ) (_hp : p.Prime)
    ...
    Function.Injective (evalHomR n p) := by
  intro P Q hPQ
