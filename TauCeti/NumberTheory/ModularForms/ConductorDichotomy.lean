...
theorem exists_unitsMap_eq_and_apply_ne (hd : d ∣ N) {χ : (ZMod N)ˣ →* ℂˣ} (hχ : ¬ ∀ u : (ZMod N)ˣ,
    ZMod.unitsMap hd u = 1 → χ u = 1) (u : (ZMod N)ˣ) : ∃ u' : (ZMod N)ˣ, ZMod.unitsMap hd u' =
    ZMod.unitsMap hd u ∧ χ u' ≠ χ u := by
...
private lemma N_dvd_unit_val_mul_val_sub_one (N : ℕ) [NeZero N] (u : (ZMod N)ˣ) : (N : ℤ) ∣ ((((u⁻¹
    : (ZMod N)ˣ) : ZMod N).val : ℤ) * (((u : (ZMod N)ˣ) : ZMod N).val : ℤ) - 1) := by
...
@[simp]
lemma gamma0LiftOfUnit_apply_one_zero (N : ℕ) [NeZero N] (u : (ZMod N)ˣ) : (gamma0LiftOfUnit N u :
    SL(2, ℤ)) 1 0 = (N : ℤ) := by
  rw [gamma0LiftOfUnit]
  simp
...
@[simp]
lemma gamma0LiftOfUnit_apply_one_one (N : ℕ) [NeZero N] (u : (ZMod N)ˣ) : (gamma0LiftOfUnit N u :
    SL(2, ℤ)) 1 1 = (((u : (ZMod N)ˣ) : ZMod N).val : ℤ) := by
  rw [gamma0LiftOfUnit]
  simp

/-- The upper-left entry of the controlled lift is the value of `u⁻¹`. -/
@[simp]
lemma gamma0LiftOfUnit_apply_zero_zero (N : ℕ) [NeZero N] (u : (ZMod N)ˣ) : (gamma0LiftOfUnit N u :
    SL(2, ℤ)) 0 0 = (((u⁻¹ : (ZMod N)ˣ) : ZMod N).val : ℤ) := by
  rw [gamma0LiftOfUnit]
  simp

/-- The upper-right entry of the controlled lift is the Bézout cofactor. -/
@[simp]
lemma gamma0LiftOfUnit_apply_zero_one (N : ℕ) [NeZero N] (u : (ZMod N)ˣ) : (gamma0LiftOfUnit N u :
    SL(2, ℤ)) 0 1 = ((((u⁻¹ : (ZMod N)ˣ) : ZMod N).val : ℤ) * (((u : (ZMod N)ˣ) : ZMod N).val : ℤ) -
    1) / (N : ℤ) := by
  rw [gamma0LiftOfUnit]
  simp
...
@[simp]
lemma Gamma0Map_toHomUnits_gamma0LiftOfUnit (N : ℕ) [NeZero N] (u : (ZMod N)ˣ) : (Gamma0Map
    N).toHomUnits (gamma0LiftOfUnit N u) = u := by
  refine Units.ext ?_
...
lemma natCast_dvd_val_sub_val_of_unitsMap_eq (hd : d ∣ N) {u u' : (ZMod N)ˣ} (h : ZMod.unitsMap hd u
    = ZMod.unitsMap hd u') : (d : ℤ) ∣ ((((u : (ZMod N)ˣ) : ZMod N).val : ℤ) - (((u' : (ZMod N)ˣ) :
    ZMod N).val : ℤ)) := by
...
private lemma eq_T_mul_mul_T_of_sub_eq {l Nl i j a a' e e' b b' : ℤ} (hNl : Nl ≠ 0) (hi : i * Nl = a
    - a') (hj : j * Nl = e - e') (hdet : a * e - b * (l * Nl) = 1) (hdet' : a' * e' - b' * (l * Nl)
    = 1) : (!![a, l * b; Nl, e] : Matrix (Fin 2) (Fin 2) ℤ) = !![(1 : ℤ), i; 0, 1] * !![a', l * b';
    Nl, e'] * !![(1 : ℤ), j; 0, 1] := by
  ext p q
...
theorem exists_eq_T_zpow_mul_conjScale_mul_T_zpow_of_apply_ne {l : ℕ} [NeZero l] (hlN : l ∣ N) {χ :
    (ZMod N)ˣ →* ℂˣ} (hχ : ¬ ∀ u : (ZMod N)ˣ, ZMod.unitsMap (Nat.div_dvd_of_dvd hlN) u = 1 → χ u =
    1) (u : (ZMod N)ˣ) : ∃ (i j : ℤ) (u' : (ZMod N)ˣ), χ u' ≠ χ u ∧ conjScale l (gamma0LiftOfUnit N
    u) ((N / l : ℕ) : ℤ) (gamma0LiftOfUnit_apply_one_zero_eq_mul hlN u) = ModularGroup.T ^ i *
    conjScale l (gamma0LiftOfUnit N u') ((N / l : ℕ) : ℤ) (gamma0LiftOfUnit_apply_one_zero_eq_mul
    hlN u') * ModularGroup.T ^ j := by
...
theorem eq_zero_of_not_forall_unitsMap_eq_one {l : ℕ} [NeZero l] (hlN : l ∣ N) (k : ℤ) {χ : (ZMod
    N)ˣ →* ℂˣ} (hχ : ¬ ∀ u : (ZMod N)ˣ, ZMod.unitsMap (Nat.div_dvd_of_dvd hlN) u = 1 → χ u = 1) (f :
    ℍ → ℂ) (hnb : ∀ (γ : SL(2, ℤ)) (hγ : γ ∈ Gamma0 N), (f ∣[k] scaleGL l) ∣[k] mapGL ℝ γ = (χ
    ((Gamma0Map N).toHomUnits ⟨γ, hγ⟩) : ℂ) • (f ∣[k] scaleGL l)) (hT : f ∣[k] (mapGL ℝ
    ModularGroup.T : GL (Fin 2) ℝ) = f) : f = 0 := by
...
lemma toUnitHom_eq_one_of_unitsMap_eq_one {χ : DirichletCharacter ℂ N} {d : ℕ} (hd : d ∣ N) (h :
    χ.FactorsThrough d) (u : (ZMod N)ˣ) (hu : ZMod.unitsMap hd u = 1) : χ.toUnitHom u = 1 :=
...
private lemma unitsMap_Gamma0Map_toHomUnits_eq_of_diag {l : ℕ} (hlN : l ∣ N) [NeZero (N / l)] {γ :
    SL(2, ℤ)} (hγ : γ ∈ Gamma0 N) (γ' : ↥(Gamma0 (N / l))) {j : ℤ} (hdiag : γ 1 1 = (γ' : SL(2, ℤ))
    1 1 - (γ' : SL(2, ℤ)) 1 0 * j) : ZMod.unitsMap (Nat.div_dvd_of_dvd hlN) ((Gamma0Map
    N).toHomUnits ⟨γ, hγ⟩) = (Gamma0Map (N / l)).toHomUnits γ' := by
...
statement. -/
theorem cuspFormOfSmulSlashScaleGL_mem_cuspFormCharSpace {l : ℕ} [NeZero l] [NeZero (N / l)] (hlN :
    l ∣ N) (k : ℤ) {χ : DirichletCharacter ℂ N} (hfac : χ.FactorsThrough (N / l)) (f : ℍ → ℂ) (g :
    CuspForm ((Gamma1 N).map (mapGL ℝ)) k) (hgχ : g ∈ cuspFormCharSpace k χ.toUnitHom) (hg : ⇑g = (l
    : ℂ) ^ (1 - k) • (f ∣[k] scaleGL l)) (hT : f ∣[k] (mapGL ℝ ModularGroup.T : GL (Fin 2) ℝ) = f) :
    cuspFormOfSmulSlashScaleGL l N hlN k χ.toUnitHom (toUnitHom_eq_one_of_unitsMap_eq_one
    (Nat.div_dvd_of_dvd hlN) hfac) f g hgχ hg hT ∈ cuspFormCharSpace k hfac.χ₀.toUnitHom := by
...
theorem exists_cuspForm_mem_cuspFormCharSpace_or_eq_zero {l : ℕ} [NeZero l] [NeZero (N / l)] (hlN :
    l ∣ N) (k : ℤ) (χ : DirichletCharacter ℂ N) (f : ℍ → ℂ) (g : CuspForm ((Gamma1 N).map (mapGL ℝ))
    k) (hgχ : g ∈ cuspFormCharSpace k χ.toUnitHom) (hg : ⇑g = (l : ℂ) ^ (1 - k) • (f ∣[k] scaleGL
    l)) (hT : f ∣[k] (mapGL ℝ ModularGroup.T : GL (Fin 2) ℝ) = f) : (∃ hfac : χ.FactorsThrough (N /
    l), ∃ F : CuspForm ((Gamma1 (N / l)).map (mapGL ℝ)) k, F ∈ cuspFormCharSpace k hfac.χ₀.toUnitHom
    ∧ ⇑F = f) ∨ f = 0 := by
  classical
