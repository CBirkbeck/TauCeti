...
level. For a squarefree `L` all of whose primes already divide `N`, there is a cusp form `g` of
level `L * N`, again with nebentypus `χ` read at the raised level, whose `q`-expansion keeps
exactly the coefficients at indices coprime to `L`:

`aₙ(g) = aₙ(f)` when `(n, L) = 1`, and `aₙ(g) = 0` otherwise.

## Main results

* `TauCeti.exists_mem_cuspFormCharSpace_qExpansion_coeff_eq_ite_coprime`: the filter itself,
  Miyake's Lemma 4.6.5.
...
prime, only that its prime factors already divide the level, so the single-prime step is stated
...
/-- **The coprime-index filter at a single prime power dividing the level.** From
`f ∈ S_k(Γ₁(N), χ)` and a `p` whose prime factors divide `N`, the form `f - V_p (T_p f)` lies in
`S_k(Γ₁(p * N), χ)` and has the coefficients of `f` at the indices prime to `p`. -/
...
private theorem exists_mem_cuspFormCharSpace_qExpansion_coeff_eq_ite_coprime_prod (m : ℕ) :
  ...
  | succ m ih =>
    ...
    · rw [show χ.comp (ZMod.unitsMap (hMqM.trans hM'dvd)) =
          (χ.comp (ZMod.unitsMap hMqM)).comp (ZMod.unitsMap hM'dvd) by
        rw [MonoidHom.comp_assoc, ZMod.unitsMap_comp]]
      exact hg'
    · rw [hg'q n, hg₁q n, hprod]
      exact ite_coprime_ite_dvd_eq_ite_coprime_mul hq n _ _

/-- **Miyake's Lemma 4.6.5, the coprime-index filter.** For `f ∈ S_k(Γ₁(N), χ)` and a squarefree
`L` whose primes all divide `N`, there is a cusp form `g` of level `L * N`, with the nebentypus
`χ` read at that level, whose `q`-expansion is that of `f` restricted to the indices coprime
to `L`:
...
theorem exists_mem_cuspFormCharSpace_qExpansion_coeff_eq_ite_coprime (χ : (ZMod N)ˣ →* ℂˣ)
    ...
    (hL : Squarefree L) (hLN : L.primeFactors ⊆ N.primeFactors) :
  ...
  rw [hprod] at hM'eq
  subst hM'eq
  exact ⟨g, hg, fun n ↦ by rw [hgq n, hprod]⟩
