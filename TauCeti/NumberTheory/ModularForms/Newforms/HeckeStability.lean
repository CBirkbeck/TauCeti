...
for a general element of the ring; the generators at good primes are what the eigenform arguments
...
* `TauCeti.coe_heckeRingHomCuspCharSpace_heckeTGeneratorGamma0_mem_cuspFormsNew`: the
  corresponding newspace stability on `S_k(N, χ)`, and
  `TauCeti.cuspFormsNew_comap_map_heckeRingHomCuspCharSpace_heckeTGeneratorGamma0_le` in
  `Submodule.map` form.
...
theorem heckeTCuspNat_mem_cuspFormsNew [NeZero N] (hp : p.Prime)
  ...
  exact CuspForm.map_mem_peterssonOrthogonal
    (HeckeRing.GL2.peterssonInnerCosets_heckeTCuspNat_left k hpN)
    (fun g hg ↦ diamondOpCusp_mem_cuspFormsOld _
      (heckeTCuspNat_mem_cuspFormsOld hp hpN hg)) hf
...
theorem coe_heckeRingHomCuspCharSpace_heckeTGeneratorGamma0_mem_cuspFormsNew [NeZero N]
    {χ : (ZMod N)ˣ →* ℂˣ} (hp : p.Prime) (hpN : Nat.Coprime p N)
    {F : cuspFormCharSpace k χ}
...
theorem cuspFormsNew_comap_map_heckeRingHomCuspCharSpace_heckeTGeneratorGamma0_le [NeZero N]
    (χ : (ZMod N)ˣ →* ℂˣ) (hp : p.Prime) (hpN : Nat.Coprime p N) (k : ℤ) :
