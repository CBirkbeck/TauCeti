...
* `TauCeti.cuspFormsOldMultiples`: the part of the old subspace generated by the *new* forms at
  proper divisor levels that are multiples of a given `m`. At `m` the conductor of a nebentypus
  character this is Miyake's `S_k^♭(N, χ)`.
...
Taking `m` to be the conductor of a nebentypus character gives Miyake's `S_k^♭(N, χ)` (p. 162).
The character plays no part in the *definition* — only in the results that consume it — so it is
