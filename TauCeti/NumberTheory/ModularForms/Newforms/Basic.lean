...
the character transition along each `M ∣ N`. Recovering `S_k^♭(N, χ)` needs those; this space is
the uncharactered approximation from below, and it is what the inclusion into `cuspFormsOld`
below is stated for. -/
