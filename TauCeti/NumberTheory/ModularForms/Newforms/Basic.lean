...
* `TauCeti.cuspFormsOldProjection`, `TauCeti.cuspFormsNewProjection`: the two `ℂ`-linear
  projections of `S_k(Γ₁(N))` along the old/new decomposition, as `Submodule.projection`, with
  their defining equations, ranges, kernels and idempotence.
...
* `TauCeti.isCompl_cuspFormsOld_cuspFormsNew`: the two subspaces are complements, so every cusp
  form splits uniquely; `TauCeti.cuspFormsOldProjection_add_cuspFormsNewProjection_eq_self` is
  that splitting, and the `eq_self_iff` / `apply_eq_zero_iff` pairs characterise each
  projection's fixed points and kernel.
...
* AINTLIB's [`LeanModularForms/HeckeRIngs/GL2/Newforms/Basic.lean`](https://github.com/CBirkbeck/AINTLIB/blob/dev/leanmodularforms/projects/LeanModularForms/LeanModularForms/HeckeRIngs/GL2/Newforms/Basic.lean),
  ...
  for the old/new decomposition of lines 293-400: the `IsCompl` statement, the two projections
  and their part lemmas. The `IsCompl` proof is not adapted from there — that source derives it
  through a real bilinear form, whereas here it is assembled from
  `CuspForm.isCompl_peterssonOrthogonal`.
-/
...
/-- **The oldform projection**, the `ℂ`-linear endomorphism of `S_k(Γ₁(N))` projecting onto the
old subspace along the new one. -/
noncomputable def cuspFormsOldProjection (N : ℕ) [NeZero N] (k : ℤ) :
  ...
  (cuspFormsOld N k).projection (cuspFormsNew N k) (isCompl_cuspFormsOld_cuspFormsNew N k)

/-- **The newform projection**, projecting onto the new subspace along the old one. -/
noncomputable def cuspFormsNewProjection (N : ℕ) [NeZero N] (k : ℤ) :
    CuspForm ((Gamma1 N).map (mapGL ℝ)) k →ₗ[ℂ] CuspForm ((Gamma1 N).map (mapGL ℝ)) k :=
  (cuspFormsNew N k).projection (cuspFormsOld N k) (isCompl_cuspFormsOld_cuspFormsNew N k).symm

/-- Defining equation for the sealed `cuspFormsOldProjection`. It is `@[simp]` so that the
projection normalises to `Submodule.projection`, at which point Mathlib's own simp set for
`range`, `ker`, idempotence and the characteristic biconditionals applies directly. -/
@[simp]
lemma cuspFormsOldProjection_def (N : ℕ) [NeZero N] (k : ℤ) :
    cuspFormsOldProjection N k =
      (cuspFormsOld N k).projection (cuspFormsNew N k) (isCompl_cuspFormsOld_cuspFormsNew N k) :=
  (rfl)

/-- Defining equation for the sealed `cuspFormsNewProjection`. `@[simp]` for the same reason as
its old-subspace counterpart. -/
@[simp]
lemma cuspFormsNewProjection_def (N : ℕ) [NeZero N] (k : ℤ) :
    cuspFormsNewProjection N k =
      (cuspFormsNew N k).projection (cuspFormsOld N k)
        (isCompl_cuspFormsOld_cuspFormsNew N k).symm :=
  (rfl)

/-- The image of the oldform projection is old. -/
theorem cuspFormsOldProjection_apply_mem [NeZero N]
    (f : CuspForm ((Gamma1 N).map (mapGL ℝ)) k) :
    cuspFormsOldProjection N k f ∈ cuspFormsOld N k :=
  Submodule.projection_apply_mem _ f

/-- The image of the newform projection is new. -/
theorem cuspFormsNewProjection_apply_mem [NeZero N]
    (f : CuspForm ((Gamma1 N).map (mapGL ℝ)) k) :
    cuspFormsNewProjection N k f ∈ cuspFormsNew N k :=
  Submodule.projection_apply_mem _ f

/-- **Reconstruction:** a cusp form is the sum of its old and new components. -/
theorem cuspFormsOldProjection_add_cuspFormsNewProjection_eq_self [NeZero N]
    (f : CuspForm ((Gamma1 N).map (mapGL ℝ)) k) :
    cuspFormsOldProjection N k f + cuspFormsNewProjection N k f = f :=
  Submodule.projection_add_projection_eq_self (isCompl_cuspFormsOld_cuspFormsNew N k) f

/-- The oldform projection fixes exactly the old subspace. -/
theorem cuspFormsOldProjection_eq_self_iff [NeZero N]
    {f : CuspForm ((Gamma1 N).map (mapGL ℝ)) k} :
    cuspFormsOldProjection N k f = f ↔ f ∈ cuspFormsOld N k :=
  Submodule.projection_eq_self_iff _ f

/-- The newform projection fixes exactly the new subspace. -/
theorem cuspFormsNewProjection_eq_self_iff [NeZero N]
    {f : CuspForm ((Gamma1 N).map (mapGL ℝ)) k} :
    cuspFormsNewProjection N k f = f ↔ f ∈ cuspFormsNew N k :=
  Submodule.projection_eq_self_iff _ f

/-- The oldform projection vanishes exactly on the new subspace. -/
theorem cuspFormsOldProjection_apply_eq_zero_iff [NeZero N]
    {f : CuspForm ((Gamma1 N).map (mapGL ℝ)) k} :
    cuspFormsOldProjection N k f = 0 ↔ f ∈ cuspFormsNew N k :=
  Submodule.projection_apply_eq_zero_iff _

/-- The newform projection vanishes exactly on the old subspace. -/
theorem cuspFormsNewProjection_apply_eq_zero_iff [NeZero N]
    {f : CuspForm ((Gamma1 N).map (mapGL ℝ)) k} :
    cuspFormsNewProjection N k f = 0 ↔ f ∈ cuspFormsOld N k :=
  Submodule.projection_apply_eq_zero_iff _
