...
`𝕋 → Module.End` — is not proved here. Its two counting ingredients are now available in
`HeckeSlash/Composition.lean`: `card_pairs_mem_rightCoset_eq_multiplicity` is the left/right
handedness reconciliation that `DoubleCoset.multiplicity` requires, and
`card_pairs_mem_rightCoset_congr` the count that each right coset of a fixed `D` is hit by the
same number of pairs. What is missing is the assembly, together with the twisted weights it has
to carry.
