...
@[simp]
private theorem τfun_apply (h : ℝ) (q : ℂ) :
...
`Im τ → ∞`. -/
private theorem tendsto_τfun_atImInfty {h : ℝ} (hh : 0 < h) :
...
private theorem cuspFunction_eq_eval_τfun_of_ne_zero {Γ' : Subgroup (GL (Fin 2) ℝ)} {k' : ℤ} {h : ℝ}
...
private theorem cuspFunction_norm_eq_mul_restProd_of_ne_zero (f : ModularForm (G Γ) k) {h : ℝ}
    {q : ℂ} (hq : q ≠ 0) :
...
private theorem isBigO_one_restProd_τfun (f : ModularForm (G Γ) k) {h : ℝ} (hh : 0 < h) :
...
`𝓝[≠] 0`. -/
private theorem cuspFunction_norm_eventuallyEq_mul (f : ModularForm (G Γ) k) {h : ℝ} :
...
private theorem isBigO_pow_cuspFunction_norm_punctured (f : ModularForm (G Γ) k)
...
private theorem cuspFunction_norm_apply_zero_eq_zero (f : ModularForm (G Γ) k)
    (hh : 0 < (G Γ).strictWidthInfty)
...
theorem qExpansion_coeff_eq_zero_norm_of_qExpansion_coeff_eq_zero (f : ModularForm (G Γ) k)
  ...
  _root_.TauCeti.ModularFormClass.qExpansion_coeff_eq_zero_of_cuspFunction_isBigO_pow
    ...
    (TauCeti.isBigO_nhds_of_isBigO_punctured
      (isBigO_pow_cuspFunction_norm_punctured f hh hper N hcoeff)
      (fun _ ↦ by rw [cuspFunction_norm_apply_zero_eq_zero f hh hper hperSL
        (Nat.zero_lt_of_lt hn) hcoeff, norm_zero]))
