...
import TauCeti.NumberTheory.ModularForms.LevelOne.FundamentalDomainBoundary.Winding.RhoOneTelescope
...
* `TauCeti.ModularForm.hasCauchyPVAt_fdBoundary_rho_one` (the principal value `-πi/3`).
* `TauCeti.ModularForm.windingNumber_fdBoundary_rho_one` (the winding number `-1/6`).
...
chord. -/
private lemma lt_norm_fdBoundary_sub_rho_one_arc_of_far (harc : t ∈ Icc (1 : ℝ) 3)
    (hd : 0 < δ) (hd1 : δ < 1) (hfar : δ < t - 1) :
    2 * Real.sin (δ * (Real.pi / 12)) < ‖fdBoundary H t - ((UpperHalfPlane.ρ : ℂ) + 1)‖ := by
  rw [norm_fdBoundary_sub_rho_one_arc H harc,
...
private lemma norm_fdBoundary_sub_rho_one_arc_le_of_near (harc : t ∈ Icc (1 : ℝ) 3)
    (hd1 : δ < 1) (hnear : t - 1 ≤ δ) :
...
private lemma lt_norm_of_far_left_rho_one (hH : Real.sqrt 3 / 2 < H) (hd : 0 < δ)
    (hlin : δ * (H - Real.sqrt 3 / 2) = ε) (ht : t ∈ Ico (0 : ℝ) (1 - δ)) :
    ε < ‖fdBoundary H t - ((UpperHalfPlane.ρ : ℂ) + 1)‖ := by
  rw [fdBoundary_sub_rho_one_of_mem_Icc_zero_one H ⟨ht.1, by linarith [ht.2]⟩, ← hlin,
...
`ρ + 1`. -/
private lemma lt_norm_of_far_right_rho_one (hH : Real.sqrt 3 / 2 < H) (hε₁ : ε < 1)
    (hεH : ε < H - Real.sqrt 3 / 2) (hd : 0 < δ) (hd1 : δ < 1)
    (h2sin : 2 * Real.sin (δ * (Real.pi / 12)) = ε) (ht : t ∈ Ioc (1 + δ : ℝ) 5) :
    ε < ‖fdBoundary H t - ((UpperHalfPlane.ρ : ℂ) + 1)‖ := by
  rcases le_or_gt t 3 with ht3 | ht3
  · rw [← h2sin]
    refine lt_norm_fdBoundary_sub_rho_one_arc_of_far ⟨by linarith [ht.1], ht3⟩ hd hd1 ?_
    linarith [ht.1]
  · rcases le_or_gt t 4 with ht4 | ht4
    · calc ε < 1 := hε₁
        _ ≤ ‖fdBoundary H t - ((UpperHalfPlane.ρ : ℂ) + 1)‖ :=
          norm_fdBoundary_sub_rho_one_segment4 H ⟨ht3.le, ht4⟩
    · calc ε < H - Real.sqrt 3 / 2 := hεH
        _ ≤ ‖fdBoundary H t - ((UpperHalfPlane.ρ : ℂ) + 1)‖ :=
          norm_fdBoundary_sub_rho_one_segment5 hH ⟨ht4.le, ht.2⟩

/-- Over the excised corner, the contour stays within distance `ε` of `ρ + 1`. -/
private lemma norm_le_of_near_rho_one {δL δR : ℝ} (hH : Real.sqrt 3 / 2 < H)
  ...
  · rw [fdBoundary_sub_rho_one_of_mem_Icc_zero_one H ⟨by linarith [ht.1], h1⟩,
      norm_mul, Complex.norm_real, Complex.norm_I, mul_one, Real.norm_eq_abs,
      abs_of_nonneg (mul_nonneg (by linarith) (by linarith)), ← hlin]
    exact mul_le_mul_of_nonneg_right (by linarith [ht.1]) (by linarith)
  · rw [← h2sin]
    refine norm_fdBoundary_sub_rho_one_arc_le_of_near
...
private lemma delta_right_spec_rho_one (hε : 0 < ε)
...
private lemma truncated_integral_spec_rho_one (hH : Real.sqrt 3 / 2 < H) (hε : 0 < ε)
  ...
  obtain ⟨hδR_pos, hδR_lt, h2sin⟩ := delta_right_spec_rho_one hε hε₃
  ...
  obtain ⟨hi_left, hi_right, hval⟩ :=
    ftc_logDeriv_telescope_rho_one H hH hδL_pos hδL_le hδR_pos hδR_lt
  have hconv : ∀ s : ℝ,
  ...
  have hae_left : ∀ᵐ s ∂volume, s ∈ uIoc (0 : ℝ) (1 - δL) →
    ...
    rw [if_pos (lt_norm_of_far_left_rho_one hH hδL_pos hlin hsIco), hconv s]
  have hae_right : ∀ᵐ s ∂volume, s ∈ uIoc (1 + δR : ℝ) 5 →
    ...
    rw [uIoc_of_le (by linarith)] at hmem
    rw [if_pos (lt_norm_of_far_right_rho_one hH hε₁ hεH hδR_pos hδR_lt h2sin hmem), hconv s]
  have hmid : EqOn (fun s ↦ if ε < ‖fdBoundary H s - ((UpperHalfPlane.ρ : ℂ) + 1)‖
    ...
    exact if_neg (not_lt.mpr (norm_le_of_near_rho_one hH hδL_le hlin hδR_lt h2sin hs))
  ...
  rw [← intervalIntegral.integral_add_adjacent_intervals (hi02.trans himid) hi25,
    ...
    hval, log_fdBoundary_one_sub_sub_rho_one hH hδL_pos hδL_le,
    log_fdBoundary_one_add_sub_rho_one H hδR_pos hδR_lt, hlin, h2sin, hδ12]
  push_cast
  ring
...
angle defect. -/
theorem hasCauchyPVAt_fdBoundary_rho_one (hH : Real.sqrt 3 / 2 < H) :
  ...
  have hspec : ∀ ε ∈ Ioo (0 : ℝ)
    ...
    fun ε hε ↦ truncated_integral_spec_rho_one hH hε.1
...
theorem windingNumber_fdBoundary_rho_one (hH : Real.sqrt 3 / 2 < H) :
    Contour.windingNumber (fdBoundary H) 0 5 ((UpperHalfPlane.ρ : ℂ) + 1) =
      -(1 / 6 : ℂ) := by
  rw [Contour.windingNumber_eq_of_hasCauchyPVAt (hasCauchyPVAt_fdBoundary_rho_one hH)]
