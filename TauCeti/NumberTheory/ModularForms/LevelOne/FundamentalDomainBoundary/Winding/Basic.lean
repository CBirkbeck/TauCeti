...
* `TauCeti.ModularForm.fdBoundaryArcExcisionHalfWidth`: the parameter half-width whose
  ...
  bound by `TauCeti.ModularForm.fdBoundaryArcExcisionHalfWidth_pos_lt_one_and_two_mul_sin_eq`.
...
`fdBoundaryArcExcisionHalfWidth_pos_lt_one_and_two_mul_sin_eq` nonetheless assumes the narrower
...
lemma fdBoundaryArcExcisionHalfWidth_pos_lt_one_and_two_mul_sin_eq {ε : ℝ} (hε : 0 < ε)
