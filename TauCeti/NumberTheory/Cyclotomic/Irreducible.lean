...
irreducible over `K`. This is the Layer 7.2 output that Layer 9 consumes: `Gal(K(ζ_q)/K)` cyclic
of order `q - 1`. The subtraction is truncated `ℕ` subtraction, which agrees with the totient
because `q` is prime.

`hirr` is an explicit hypothesis rather than derived, so a caller cannot reach the order `q - 1`
without supplying the irreducibility that justifies it. -/
