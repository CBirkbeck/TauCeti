...
`(F ≃ₐ[K] F) ≃* (ZMod n)ˣ` — and hence for cyclicity — use `autEquivPow` directly; for the degree
rather than the automorphism count use `IsCyclotomicExtension.finrank`. Keeping `hirr` as a
hypothesis rather than deriving it stops a caller from silently assuming the order. -/
...
irreducible over `K`. This is `card_aut_eq_totient` with `φ q = q - 1`; the subtraction is
truncated `ℕ` subtraction, which agrees with the totient because `q` is prime. -/
theorem card_aut_eq_sub_one (q : ℕ) (F : Type*) [Field F] [Algebra K F] (hq : q.Prime)
    [IsCyclotomicExtension {q} K F] (hirr : Irreducible (cyclotomic q K)) :
