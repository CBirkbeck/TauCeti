...
* `TauCeti.summable_absNorm_rpow_of_one_lt`: over the height-one primes of `𝓞 K`, the series
  `∑ N(𝔭) ^ (-s)` converges for every `1 < s`.
* `TauCeti.summable_absNorm_rpow_subtype_of_one_lt`: the same over an arbitrary set of height-one
  primes.  This is the family `NumberField.Set.primeIdealZetaSum` sums, so it is the form its
  consumers need.
* `NumberField.Set.primeIdealZetaSum_mono_set_of_one_lt`: for `1 < s`, the prime ideal zeta sum
  is monotone under inclusion of sets of primes.
...
/-- **The prime-indexed Dirichlet series converges for `s > 1`.** The prime-indexed analogue of
`TauCeti.summable_absNorm_rpow_ideal_iff`. -/
theorem summable_absNorm_rpow_of_one_lt {s : ℝ} (hs : 1 < s) :
  ...
  -- `Summable.comp_injective` delivers the composite; `congr` matches it to the goal pointwise,
  -- the two sides differing only by the projection out of the `nonZeroDivisors` subtype.  The
  -- `congr` is load-bearing: without it unification diverges rather than merely running long.
...
theorem summable_absNorm_rpow_subtype_of_one_lt (S : Set (HeightOneSpectrum (𝓞 K))) {s : ℝ}
  ...
  (summable_absNorm_rpow_of_one_lt hs).subtype S
...
/-- **The prime ideal zeta sum is monotone under inclusion of sets of primes**, for `1 < s`.

The hypothesis `1 < s` carries the convergence of the larger sum and is not decoration: `tsum`
returns `0` on a family that is not summable, so an inequality between two such sums can fail
with a positive left-hand side and a vanishing right-hand side. -/
theorem primeIdealZetaSum_mono_set_of_one_lt {S T : Set (HeightOneSpectrum (𝓞 K))} (hST : S ⊆ T)
    {s : ℝ} (hs : 1 < s) : S.primeIdealZetaSum s ≤ T.primeIdealZetaSum s := by
  ...
  -- is injective, and matches the terms of the two sums exactly.
  exact (summable_absNorm_rpow_subtype_of_one_lt S hs).tsum_le_tsum_of_inj (Set.inclusion hST)
    (Set.inclusion_injective hST) (fun _ _ ↦ by positivity) (fun _ ↦ le_rfl)
