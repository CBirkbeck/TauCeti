...
* `TauCeti.F4ShortRoot.frobenius`: the `p ^ k`-power Frobenius on the carrier's points.
* `TauCeti.F4ShortRoot.coe_frobenius` and `coe_frobenius_apply`: its matrix and entrywise actions.
* `TauCeti.F4ShortRoot.frobenius_rootSubgroupPoints`: its action on every numbered simple-root
  subgroup.
* `TauCeti.F4ShortRoot.frobenius_weightTorusPoints`: its action on the split weight torus.
* `TauCeti.F4ShortRoot.frobenius_zero`, `frobenius_add` and `frobenius_pow`: its iteration laws.
* `TauCeti.F4ShortRoot.frobenius_eq_self_iff` and
  `TauCeti.F4ShortRoot.map_subtype_fixedSubgroup_frobenius_eq`: which points it fixes, and the
