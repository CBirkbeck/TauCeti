...
[Reviews and tests of this file](https://cbirkbeck.github.io/tauceti-reviewed-by-test/#m=TauCeti.Algebra.Lie.HighestWeight.KostantMultiplicity)
...
* `TauCeti.formalCharacter_coeff_eq_kostantMultiplicity`: the coefficients of the formal character
...
theorem formalCharacter_coeff_eq_kostantMultiplicity (mu : Dual K H) :
    (formalCharacter K H M).coeff mu
      = (IsKilling.rootSystem H).kostantMultiplicity b lam mu :=
  RootPairing.coeff_eq_kostantMultiplicity_of_mul_weylDenominator_eq_weylNumerator _ _
...
theorem finrank_weightSpace_eq_kostantMultiplicity (mu : Dual K H) :
    (finrank K (weightSpace M (mu : H → K)) : ℤ)
      = (IsKilling.rootSystem H).kostantMultiplicity b lam mu := by
  rw [← formalCharacter_coeff_eq_finrank_weightSpace,
    formalCharacter_coeff_eq_kostantMultiplicity hv hgen]
