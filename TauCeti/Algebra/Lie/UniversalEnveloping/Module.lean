...
* `TauCeti.UniversalEnvelopingAlgebra.map_representation`: a homomorphism of Lie modules
  intertwines the two enveloping-algebra actions; the previous item at the canonical structures.
...
`TauCeti.UniversalEnvelopingAlgebra.map_smul_of_map_ι_smul` at the canonical module structures. -/
@[simp]
theorem map_representation (f : M →ₗ⁅R,L⁆ N) (u : U) (m : M) :
    f (representation R L M u m) = representation R L N u (f m) := by
  rw [← asModule_smul_def R L M u m, ← asModule_smul_def R L N u (f m)]
  exact map_smul_of_map_ι_smul (f : M →ₗ[R] N)
    (fun x m => by rw [asModule_ι_smul R L M, asModule_ι_smul R L N]; exact f.map_lie x m) u m
