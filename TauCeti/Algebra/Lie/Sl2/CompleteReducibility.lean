...
/-- **An `L`-equivariant projection of `M` onto a nonzero Lie submodule.** This is step one applied
inside `M →ₗ[K] M`: the endomorphisms vanishing on `N` sit properly inside those acting on `N` by a
scalar, because a linear projection onto `N` lies in the larger one and not the smaller, so step one
supplies an invariant `φ` outside it. Invariance is exactly equivariance, and `φ` acts on `N` by a
nonzero scalar, so rescaling makes it a projection. -/
...
omit [CharZero K] [IsAlgClosed K] [LieAlgebra K L] [LieModule K L M] in
/-- **The kernel of an equivariant projection onto `N` is a complement to `N`.** Equivariance is
what makes the kernel a Lie submodule rather than merely a submodule; the projection identities then
give disjointness and codisjointness directly. -/
private theorem exists_isCompl_of_equivariant_projection {N : LieSubmodule K L M} {ψ : M →ₗ[K] M}
