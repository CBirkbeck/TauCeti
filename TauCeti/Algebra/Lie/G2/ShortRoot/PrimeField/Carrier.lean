...
* `TauCeti.G2ShortRoot.PrimeField.carrierAlgebra` and
  `TauCeti.G2ShortRoot.PrimeField.carrierGenericMatrix`: the carrier's coordinate Hopf algebra and
  its universal matrix-valued point.
...
noncomputable def groupScheme : Grp (Over (Spec (CommRingCat.of (ZMod 3)))) :=
...
theorem groupScheme_eq_generatedGroupScheme :
    groupScheme = TauCeti.GeneralLinear.generatedGroupScheme 7 generator := by
  rw [groupScheme_def, TauCeti.GeneralLinear.generatedGroupScheme_def, definingIdeal_def]
...
noncomputable def carrierι : groupScheme ⟶ TauCeti.GeneralLinear.groupScheme (ZMod 3) 7 :=
  eqToHom groupScheme_eq_generatedGroupScheme ≫
...
theorem carrierι_def :
    carrierι =
      eqToHom groupScheme_eq_generatedGroupScheme ≫
        TauCeti.GeneralLinear.generatedGroupSchemeι 7 generator := by
  rw [carrierι]
...
noncomputable def rootSubgroup (k : Fin 2 ⊕ Fin 2) :
  ...
  eqToHom (AdditiveGroup.groupScheme_def (ZMod 3)) ≫
    TauCeti.GeneralLinear.generatorToGeneratedGroupScheme 7 generator (.inl k) ≫
      eqToHom groupScheme_eq_generatedGroupScheme.symm
...
theorem rootSubgroup_def (k : Fin 2 ⊕ Fin 2) :
    rootSubgroup k =
      eqToHom (AdditiveGroup.groupScheme_def (ZMod 3)) ≫
        TauCeti.GeneralLinear.generatorToGeneratedGroupScheme 7 generator (.inl k) ≫
          eqToHom groupScheme_eq_generatedGroupScheme.symm := by
  rw [rootSubgroup]
...
theorem rootSubgroup_comp_carrierι (k : Fin 2 ⊕ Fin 2) :
  ...
  rw [rootSubgroup_def, carrierι_def]
  simp only [Category.assoc, eqToHom_trans_assoc, eqToHom_refl, Category.id_comp]
  rw [TauCeti.GeneralLinear.generatorToGeneratedGroupScheme_comp_ι]
    ...
    weightTorus ≫ carrierι =
      eqToHom
        ...
        (hopfSpec (CommRingCat.of (ZMod 3))).map (generator (.inr ())).op ≫
          eqToHom (TauCeti.GeneralLinear.groupScheme_def (ZMod 3) 7).symm := by
  rw [weightTorus_def, carrierι_def]
  simp only [Category.assoc, eqToHom_trans_assoc, eqToHom_refl, Category.id_comp]
  rw [TauCeti.GeneralLinear.generatorToGeneratedGroupScheme_comp_ι]
