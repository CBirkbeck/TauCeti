...
* `TauCeti.Submodule.IsLattice.finrank_eq_finrank`: a full lattice and its ambient space have the
  same finrank.
* `LinearEquiv.extendOfIsLattice`: extension of an integral linear equivalence between
...
ambient spaces. -/
noncomputable def _root_.LinearEquiv.extendOfIsLattice {S : Submodule ℤ V} {T : Submodule ℤ W}
...
@[simp]
theorem _root_.LinearEquiv.extendOfIsLattice_apply {S : Submodule ℤ V} {T : Submodule ℤ W}
    [S.IsLattice ℚ] [T.IsLattice ℚ] (e : S ≃ₗ[ℤ] T) (x : S) :
    LinearEquiv.extendOfIsLattice e (x : V) = (e x : W) := by
  let b := Module.Free.chooseBasis ℤ S
  have h : (LinearEquiv.extendOfIsLattice e).toLinearMap.restrictScalars ℤ ∘ₗ S.subtype =
...
canonical extension. -/
theorem _root_.LinearEquiv.eq_extendOfIsLattice {S : Submodule ℤ V} {T : Submodule ℤ W}
  ...
  exact (h (Module.Free.chooseBasis ℤ S i)).trans
    (LinearEquiv.extendOfIsLattice_apply e (Module.Free.chooseBasis ℤ S i)).symm

/-- The rational extension maps the source full submodule onto the target full submodule. -/
theorem _root_.LinearEquiv.extendOfIsLattice_map {S : Submodule ℤ V} {T : Submodule ℤ W}
    [S.IsLattice ℚ] [T.IsLattice ℚ] (e : S ≃ₗ[ℤ] T) :
    S.map ((LinearEquiv.extendOfIsLattice e).restrictScalars ℤ).toLinearMap = T := by
  apply le_antisymm
  · rintro _ ⟨x, hx, rfl⟩
