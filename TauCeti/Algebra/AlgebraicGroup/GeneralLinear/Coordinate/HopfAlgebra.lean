...
/-- The image of the coordinate ring map is generated by the images of the variables. -/
private theorem coordinateRingMap_mem_adjoin_range_X (p : MatrixMonoid.CoordinateRing R n) :
    coordinateRingMap R n p ∈
      Algebra.adjoin R
        (Set.range fun ij : Fin n × Fin n => coordinateRingMap R n (MvPolynomial.X ij)) := by
  -- the map is the evaluation sending each variable to its image, so `p` is that evaluation at `p`
  have heval :
    ...
    intro ij
    simp
  -- rewrite only the left-hand side: `← heval` on the whole goal would also fire inside the
  -- generating family, nesting one evaluation inside another
  have hp : coordinateRingMap R n p =
      MvPolynomial.aeval (fun ij : Fin n × Fin n =>
        coordinateRingMap R n (MvPolynomial.X ij)) p := by rw [heval]
  rw [hp, Algebra.adjoin_range_eq_range_aeval]
  exact ⟨p, rfl⟩

/-- The antipode of the image of the coordinate ring map is generated by the antipodes of the
images of the variables. -/
private theorem antipode_coordinateRingMap_mem_adjoin_range_antipode_X
    (p : MatrixMonoid.CoordinateRing R n) :
    antipode R n (coordinateRingMap R n p) ∈
      Algebra.adjoin R
        (Set.range fun ij : Fin n × Fin n =>
          antipode R n (coordinateRingMap R n (MvPolynomial.X ij))) := by
  rw [antipode_coordinateRingMap, Algebra.adjoin_range_eq_range_aeval]
  exact ⟨p, by simp⟩
...
private theorem adjoin_X_union_antipode_X :
  ...
  have hpoly : ∀ p : MatrixMonoid.CoordinateRing R n, coordinateRingMap R n p ∈ B := fun p =>
    Algebra.adjoin_mono Set.subset_union_left (coordinateRingMap_mem_adjoin_range_X R n p)
  have hantipodePoly : ∀ p : MatrixMonoid.CoordinateRing R n,
    ...
    Algebra.adjoin_mono Set.subset_union_right
      (antipode_coordinateRingMap_mem_adjoin_range_antipode_X R n p)
