...
The generators are the powers `det ^ m`, each of which is congruent to `1` modulo that ideal
(`TauCeti.SpecialLinear.groupLike_zpow_sub_one_mem_span`); linearity carries the statement to the
whole group algebra. -/
...
An element of the kernel Hopf ideal has vanishing counit, so its image is exactly the difference
that `determinantCoordinateMap_sub_algebraMap_counit_mem_span` places in that ideal. -/
