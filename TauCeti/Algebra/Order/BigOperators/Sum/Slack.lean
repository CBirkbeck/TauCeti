...
Pure arithmetic in an ordered ring — neither linearity nor strict monotonicity is used. -/
theorem sum_sub_le_sum_of_forall_sub_le {ι R : Type*} [Ring R] [PartialOrder R] [IsOrderedRing R]
    {s : Finset ι} {d a : ι → R} {η η' : R} (ha : ∀ i ∈ s, d i - η' ≤ a i)
    (hη : (s.card : R) * η' ≤ η) :
    (∑ i ∈ s, d i) - η ≤ ∑ i ∈ s, a i := by
  have hlb := Finset.sum_le_sum ha
  rw [Finset.sum_sub_distrib, Finset.sum_const, nsmul_eq_mul] at hlb
