...
`Δ < Γ'` gives a member of `Γ'` outside `Δ`, and a convex subgroup is closed under inversion, so
replacing that member by its inverse if necessary produces one above `1`.
...
theorem exists_one_lt_of_lt [MulLeftStrictMono Γ] {Γ' Δ : ConvexSubgroup Γ} (hlt : Δ < Γ') :
    ∃ z, z ∈ Γ' ∧ z ∉ Δ ∧ 1 < z := by
