...
* `TauCeti.ConvexSubgroup.mem_closure_of_one_mem_of_mul_mem_of_one_le` : For a set of elements
  `≥ 1` containing `1` and closed under multiplication, membership in the convex closure is
  boundedness by a single member.
...
/-- **The convex closure of a multiplicatively closed set of elements `≥ 1` is the union of the
"balls" its members bound.** The point is that *one* member already suffices: closure under
multiplication makes the set of elements bounded by a single `g ∈ S` a subgroup, and the
hypothesis `1 ≤ g` makes it convex.

Consumers meet this whenever a convex subgroup is generated by the values a multiplicative
map attains — for instance the characteristic subgroup `cΓ_v` of a valuation, generated by the
attained values `≥ 1`. -/
theorem mem_closure_of_one_mem_of_mul_mem_of_one_le {S : Set Γ} (h1 : (1 : Γ) ∈ S)
    (hmul : ∀ x ∈ S, ∀ y ∈ S, x * y ∈ S) (hge : ∀ x ∈ S, 1 ≤ x) {x : Γ} :
    x ∈ closure S ↔ ∃ g ∈ S, g⁻¹ ≤ x ∧ x ≤ g := by
  refine ⟨fun hx ↦ ?_, fun ⟨g, hg, hgx, hxg⟩ ↦ ?_⟩
  · -- the right-hand side already describes a convex subgroup containing `S`
    let B : ConvexSubgroup Γ :=
      { toSubgroup :=
          { carrier := {y | ∃ g ∈ S, g⁻¹ ≤ y ∧ y ≤ g}
            one_mem' := ⟨1, h1, by simp, le_rfl⟩
...
nontriviality is required — at `γ = 1` both sides are `⊥`. Membership in
`closure {γ}` is boundedness of `|x|ₘ` by *some* power of `|γ|ₘ`, and the powers of `|γ ^ n|ₘ`
are cofinal among the powers of `|γ|ₘ`, so the two families cut out the same subgroup.
