...
* `Subgroup.exists_one_lt_of_lt` and `Subgroup.exists_lt_one_of_lt` : a strictly smaller subgroup
  is separated from the larger one by an element above, respectively below, `1`.
...
That the witness exceeds `1` is the point: a monotone map out of `Γ` gives only `≤`, and a witness
of this shape is what turns such a bound into the strict inequality a cofinality argument needs.
Convexity is not involved, so this is stated for subgroups. -/
theorem _root_.Subgroup.exists_one_lt_of_lt [MulLeftStrictMono Γ] {Γ' Δ : Subgroup Γ}
    (hlt : Δ < Γ') : ∃ z, z ∈ Γ' ∧ z ∉ Δ ∧ 1 < z := by
...
theorem _root_.Subgroup.exists_lt_one_of_lt [MulLeftStrictMono Γ] {Γ' Δ : Subgroup Γ}
    (hlt : Δ < Γ') : ∃ z, z ∈ Γ' ∧ z ∉ Δ ∧ z < 1 := by
