...
/-- Membership in the archimedean ball, in terms of archimedean classes. This names the step
between the convex subgroup and the `Subgroup` it wraps, so proofs below cross that boundary by
rewriting rather than by type ascription. -/
private theorem mem_archimedeanBall {y x : Γ} :
    x ∈ archimedeanBall y ↔ MulArchimedeanClass.mk y ≤ MulArchimedeanClass.mk x :=
...
theorem mem_closure_singleton {y x : Γ} :
  ...
  · intro hx
    have hb := mem_closure.mp hx (archimedeanBall y) fun z hz ↦ by
      ...
      subst hz
      exact mem_archimedeanBall.mpr le_rfl
    exact MulArchimedeanClass.mk_le_mk.mp (mem_archimedeanBall.mp hb)
  · rintro ⟨n, hn⟩
