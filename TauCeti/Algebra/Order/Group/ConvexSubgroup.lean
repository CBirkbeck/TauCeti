...
* `TauCeti.ConvexSubgroup.lt_closure_singleton` : An element outside a convex subgroup and
  below `1` generates a strictly larger one.
...
/-- An element outside a convex subgroup and below `1` generates a strictly larger convex
subgroup. Wedhorn needs this in the proof of Lemma 7.2 to know `cΓ_v ⊊ H` before Lemma 7.1
may be applied to the subgroup `H` generated by the largest value of a generating set. -/
theorem lt_closure_singleton {C : ConvexSubgroup Γ} {h : Γ} (hC : h ∉ C) (h1 : h < 1) :
    C < closure {h} := by
  have hbelow : ∀ γ ∈ C, h < γ := fun _ hγ ↦ C.lt_of_not_mem_of_lt_one hC h1 hγ
  refine lt_of_le_of_ne (fun x hx ↦ ?_) fun hEq ↦ ?_
  · rw [mem_closure_singleton, mabs_eq_inv_self.mpr h1.le]
    refine ⟨1, ?_⟩
    rw [pow_one]
    refine mabs_le.mpr ⟨?_, ?_⟩
    · rw [inv_inv]
      exact (hbelow x hx).le
    · have hx' := inv_lt_inv' (hbelow x⁻¹ (inv_mem hx))
      rw [inv_inv] at hx'
      exact hx'.le
  · exact absurd (hEq ▸ subset_closure ({h} : Set Γ) rfl) hC
