...
* `TauCeti.ConvexSubgroup.mem_closure_of_mulClosed` : For a multiplicatively closed set of
  elements `≥ 1`, membership in the convex closure is boundedness by a single member.
...
attained values `≥ 1`. -/
theorem mem_closure_of_mulClosed {S : Set Γ} (h1 : (1 : Γ) ∈ S)
...
/-- **A nontrivial element and its powers generate the same convex subgroup.** Membership in
