...
/-- An element at most `1` and outside `H` has class strictly below `1` in the quotient: the
class is at most `1` by monotonicity, and it cannot be `1`, which would put `a` in `H`. -/
