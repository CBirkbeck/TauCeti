...
/-- **Wedhorn Remark 1.19, at the subgroup the element generates.** An element below `1` is
cofinal for the convex subgroup it generates: this is `isCofinalElement_iff_subset_closure` at
`H = closure {γ}`, where the containment it asks for is the identity. It is the shape a coarsening
argument wants — one chooses `H` to be generated by the value one intends to make cofinal, and
cofinality is then automatic.

See [T. Wedhorn, *Adic Spaces*][wedhorn_adic] (arXiv:1910.05934v1), Remark 1.19. -/
theorem isCofinalElement_closure_singleton {γ : Γ} (hγ1 : γ < 1) :
    IsCofinalElement (closure {γ}).toSubgroup γ :=
  (isCofinalElement_iff_subset_closure hγ1).mpr (by simp)
