...
Definition 1.16, which takes `H` to be any subgroup and quantifies over arbitrary
`γ ∈ Γ` — membership in `H` is not required). Distinct from Mathlib's `IsCofinal`,
...
theorem isCofinalElement_iff_subset_closure {H : ConvexSubgroup Γ} {γ : Γ} (hγ1 : γ < 1) :
    IsCofinalElement H.toSubgroup γ ↔ (H : Set Γ) ⊆ closure {γ} := by
