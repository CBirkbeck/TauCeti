...
`isCofinalElement_iff_subset_closure` at `H = closure {γ}`, where the containment it asks for is
the identity. It is the shape a coarsening argument wants: one chooses `H` to be generated by the
value one intends to make cofinal, and cofinality is then automatic. -/
