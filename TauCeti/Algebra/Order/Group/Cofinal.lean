...
* `TauCeti.IsCofinalElement.mul_of_lt_of_mem` : Cofinal elements may be
  ...
  Proposition 1.20).
* `TauCeti.lt_closure_singleton` : An element strictly below every member of a convex
  subgroup generates a strictly larger one (the `cΓ_v ⊊ H` step of Wedhorn Lemma 7.2).
...
/-- An element strictly below every member of a convex subgroup generates a strictly larger
convex subgroup. Wedhorn needs this in the proof of Lemma 7.2 to know `cΓ_v ⊊ H` before
Lemma 7.1 may be applied to the subgroup `H` generated by the largest value of a generating
set. No hypothesis `h < 1` is required: it follows by taking `γ = 1`. -/
theorem lt_closure_singleton {C : ConvexSubgroup Γ} {h : Γ}
    (hbelow : ∀ γ ∈ C, h < γ) : C < closure {h} := by
  have h1 : h < 1 := hbelow 1 (one_mem C)
  ...
  · rw [mem_closure_singleton, mabs_eq_inv_self.mpr h1.le]
    ...
    · have hx' := inv_lt_inv' (hbelow x⁻¹ (inv_mem hx))
      rw [inv_inv] at hx'
      exact hx'.le
  · refine lt_irrefl h (hbelow h ?_)
    rw [hEq]
    exact subset_closure ({h} : Set Γ) rfl
