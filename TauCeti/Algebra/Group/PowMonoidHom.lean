...
theorem mk_eq_one_iff_exists_pow {M : Type*} [CommMonoid M] (n : ℕ) (u : Mˣ) :
  ...
  refine ⟨fun ⟨v, hv⟩ ↦ ⟨v, by simpa using congrArg Units.val hv⟩, fun ⟨z, hz⟩ ↦ ?_⟩
  cases n with
  | zero => exact ⟨1, Units.ext (by simpa using hz)⟩
  | succ _ => exact ⟨(isUnit_pow_succ_iff.mp (hz ▸ u.isUnit)).unit, Units.ext hz⟩
