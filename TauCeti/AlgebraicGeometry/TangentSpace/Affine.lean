...
private theorem _root_.AlgHom.surjective : Function.Surjective (f : H →+* k) := fun r ↦
  ⟨algebraMap k H r, by simp⟩

/-- The kernel of an augmentation to a field is canonically maximal. -/
instance _root_.AlgHom.kernelIsMaximal : (RingHom.ker (f : H →+* k)).IsMaximal :=
  RingHom.ker_isMaximal_of_surjective (f : H →+* k) (AlgHom.surjective f)

/-- The stalk at an augmentation point is an `H`-algebra through the germ map. -/
noncomputable instance _root_.AlgHom.kernelStalkAlgebra :
    Algebra H ((Spec (CommRingCat.of H)).presheaf.stalk (AlgHom.kernelPoint f)) :=
  StructureSheaf.stalkAlgebra H (AlgHom.kernelPoint f)

/-- The stalk at an augmentation point is a `k`-algebra through `k → H`. -/
noncomputable instance _root_.AlgHom.kernelStalkBaseAlgebra :
    Algebra k ((Spec (CommRingCat.of H)).presheaf.stalk (AlgHom.kernelPoint f)) :=
  Algebra.restrictScalars k H _

/-- Scalar extension from `k` through `H` to the stalk at an augmentation point is compatible. -/
instance _root_.AlgHom.kernelStalkIsScalarTower :
    IsScalarTower k H ((Spec (CommRingCat.of H)).presheaf.stalk (AlgHom.kernelPoint f)) :=
  IsScalarTower.of_algebraMap_eq' rfl

/-- The stalk at an augmentation point is the localization of `H` at the augmentation kernel. -/
instance _root_.AlgHom.kernelStalkIsLocalization :
    IsLocalization.AtPrime
      ((Spec (CommRingCat.of H)).presheaf.stalk (AlgHom.kernelPoint f))
      (RingHom.ker (f : H →+* k)) := by
  unfold AlgHom.kernelStalkAlgebra
  ...
  change @IsLocalization H _ (RingHom.ker (f : H →+* k)).primeCompl
    ((Spec.structureSheaf H).presheaf.stalk (AlgHom.kernelPoint f)) _
      (StructureSheaf.stalkAlgebra H (AlgHom.kernelPoint f))
  simpa only [AlgHom.kernelPoint_asIdeal] using
    (StructureSheaf.IsLocalization.to_stalk H (AlgHom.kernelPoint f))

/-- The ground field is canonically the residue field at an augmentation point. -/
noncomputable def _root_.AlgHom.kernelResidueFieldRingEquiv :
    k ≃+* IsLocalRing.ResidueField
      ((Spec (CommRingCat.of H)).presheaf.stalk (AlgHom.kernelPoint f)) :=
  (RingHom.quotientKerEquivOfSurjective (AlgHom.surjective f)).symm.trans
...
@[simp]
theorem _root_.AlgHom.kernelResidueFieldRingEquiv_apply (r : k) :
    AlgHom.kernelResidueFieldRingEquiv f r =
      algebraMap k
        (IsLocalRing.ResidueField
          ((Spec (CommRingCat.of H)).presheaf.stalk (AlgHom.kernelPoint f))) r := by
  ...
  change (IsLocalization.AtPrime.equivQuotMaximalIdeal (RingHom.ker (f : H →+* k))
      ((Spec (CommRingCat.of H)).presheaf.stalk (AlgHom.kernelPoint f)))
        ((RingHom.quotientKerEquivOfSurjective (AlgHom.surjective f)).symm r) = _
  have h : (RingHom.quotientKerEquivOfSurjective (AlgHom.surjective f)).symm r =
      Ideal.Quotient.mk (RingHom.ker (f : H →+* k)) (algebraMap k H r) := by
    simpa using RingHom.quotientKerEquivOfSurjective_symm_apply
      (f := (f : H →+* k)) (AlgHom.surjective f) (algebraMap k H r)
  rw [h,
...
noncomputable def _root_.AlgHom.kernelCotangentLinearEquivZariski :
    (RingHom.ker (f : H →+* k)).Cotangent ≃ₗ[k]
      TauCeti.AlgebraicGeometry.ZariskiCotangentSpace
        (Spec (CommRingCat.of H)) (AlgHom.kernelPoint f) :=
  (Ideal.cotangentLocalizationEquiv
    (Rₚ := (Spec (CommRingCat.of H)).presheaf.stalk (AlgHom.kernelPoint f))
...
@[simp]
theorem _root_.AlgHom.kernelCotangentLinearEquivZariski_toCotangent (a : RingHom.ker (f : H →+* k))
    :
    AlgHom.kernelCotangentLinearEquivZariski f
        ((RingHom.ker (f : H →+* k)).toCotangent a) =
      (maximalIdeal
          ((Spec (CommRingCat.of H)).presheaf.stalk (AlgHom.kernelPoint f))).toCotangent
        ⟨algebraMap H
            ((Spec (CommRingCat.of H)).presheaf.stalk (AlgHom.kernelPoint f)) a,
          by
            rw [← Ideal.mem_under,
...
@[simp]
theorem _root_.AlgHom.kernelCotangentLinearEquivZariski_smul (r : k)
    (x : (RingHom.ker (f : H →+* k)).Cotangent) :
    r • AlgHom.kernelCotangentLinearEquivZariski f x =
      AlgHom.kernelResidueFieldRingEquiv f r • AlgHom.kernelCotangentLinearEquivZariski f x := by
  rw [← (AlgHom.kernelCotangentLinearEquivZariski f).map_smul r x]
  -- Expose the restricted-scalar wrapper before applying the localization equivalence's
  -- native residue-field scalar formula.
  unfold AlgHom.kernelCotangentLinearEquivZariski
  ...
  rw [← IsScalarTower.algebraMap_smul H r x,
    ...
    IsLocalization.AtPrime.equivQuotMaximalIdeal_apply_mk,
    AlgHom.kernelResidueFieldRingEquiv_apply]
  rw [← Ideal.Quotient.algebraMap_eq]
  exact (IsScalarTower.algebraMap_smul
    (IsLocalRing.ResidueField
      ((Spec (CommRingCat.of H)).presheaf.stalk (AlgHom.kernelPoint f)))
    (algebraMap H ((Spec (CommRingCat.of H)).presheaf.stalk (AlgHom.kernelPoint f))
      (algebraMap k H r))
    (AlgHom.kernelCotangentLinearEquivZariski f x)).symm
...
theorem _root_.AlgHom.finrank_kernelCotangent_eq_finrank_zariskiCotangentSpace :
    Module.finrank k (RingHom.ker (f : H →+* k)).Cotangent =
      Module.finrank k
        (TauCeti.AlgebraicGeometry.ZariskiCotangentSpace
          (Spec (CommRingCat.of H)) (AlgHom.kernelPoint f)) :=
  (AlgHom.kernelCotangentLinearEquivZariski f).finrank_eq
