...
* `TauCeti.ValuationSpectrum.spaRationalOpen` : the rational subset of a presentation, as an open.
...
def spaRationalOpen {P : PairOfDefinition A} (Aplus : Subring A) (p : P.Presentation) :
    Opens ↥(spa Aplus) :=
...
structure RationalIndex {P : PairOfDefinition A} (Aplus : Subring A)
  ...
  le_open : spaRationalOpen Aplus pres ≤ V
...
noncomputable def structurePresheaf (P : PairOfDefinition A) (Aplus : Subring A) :
  ...
  map_id V := by
    ...
    -- `rationalIndexRestrict _ ⋙ rationalIndexDiagram _ _` to be seen as
    -- `rationalIndexDiagram _ _`, and the sealed `Presentation.hom` blocks that at the
