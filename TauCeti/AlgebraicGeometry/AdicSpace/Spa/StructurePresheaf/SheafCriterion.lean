...
`PairOfDefinition.IsSheafyPair` is stated in Mathlib's shape: the structure presheaf is a sheaf
...
* `TauCeti.Huber.PairOfDefinition.isSheafyPair_iff_isSheafFor_rationalCover` : the
  reconciliation.
...
This is what makes the Mathlib-shaped definition usable: `PairOfDefinition.IsSheafyPair`
quantifies over all opens and all covers, while Wedhorn checks only rational ones. -/
theorem _root_.TauCeti.Huber.PairOfDefinition.isSheafyPair_iff_isSheafFor_rationalCover
  ...
  (P.isSheafyPair_iff Aplus).trans (isSheaf_iff_isSheafFor_rationalCover Aplus _)
