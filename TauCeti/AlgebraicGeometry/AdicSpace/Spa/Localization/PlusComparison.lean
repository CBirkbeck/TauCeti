...
The argument runs on the adic spectrum. A point `w` of `Spa (A⟨T'/s'⟩, A_U'⁺)` pulls back along
`σ` to a point of `Spa (A⟨T/s⟩, A_U⁺)`: the denominator `s` becomes a unit in `A⟨T'/s'⟩`, each
fraction `t/s` has `w`-value at most `1`, and `A_U⁺` is the closure of the image of the integral
closure of `A⁺[T/s]`, a subring generated by the image of `A⁺` together with those fractions.
Wedhorn's Proposition 7.52(1), which recovers `A_U'⁺` as the sub-unit locus of its own adic
spectrum, then turns that statement about points into the membership statement for `σ`.
...
-- Generalises `TauCeti.Huber.extendToLocalization_le_one_of_mem_integralClosure_adjoin_plus` from
-- the canonical extension to an arbitrary factorisation `φ = ψ ∘ algebraMap`; reusing it needs the
-- missing bridge `w.valuation.comap ψ = (w.valuation.comap φ).extendToLocalization …`.
...
private theorem vle_one_of_mem_integralClosure_adjoin_plus (Aplus : Subring A) (T : Finset A)
  ...
  have key (y : S) : y ∈ (w.valuation.comap ψ).integer ↔ w.toValuativeRel.vle (ψ y) 1 := by
    rw [Valuation.mem_integer_iff, Valuation.comap_apply, ← map_one w.valuation, valuation_le_iff]
  ...
  have hle : (Algebra.adjoin Aplus (Set.range fun t : T ↦ (divBy (t : A) s : S))).toSubring ≤
      (w.valuation.comap ψ).integer := by
    rw [Algebra.adjoin_eq_ring_closure, Subring.closure_le]
    rintro y (⟨⟨a, ha⟩, rfl⟩ | ⟨⟨t, ht⟩, rfl⟩)
    · exact (key (algebraMap A S a)).mpr (hψ a ▸ hA a ha)
    · exact (key _).mpr ((hdiv t).symm ▸ hT t ht)
  exact (key x).mp ((Valuation.integer.integers (w.valuation.comap ψ)).mem_of_integral
...
theorem spaCompletedLocalizationHomeomorph_spaComapOfRationalSubsetSubset (P : PairOfDefinition A)
    ...
    ∀ w : spa (completedPlusSubring P Aplus T' s' S' hden'),
      spaCompletedLocalizationHomeomorph P Aplus hP T s S hden
          (spaComapOfRationalSubsetSubset P Aplus hIplus hAplus T s S hden T' s' S' hden' hsub w) =
  ...
  simp only [spaCompletedLocalizationHomeomorph_apply, spaLocToRationalSubset_val,
    spaComapLoc_val, spaComapOfRationalSubsetSubset, spaComap_val]
