...
/-- **The non-analytic locus is preserved by `Spa(φ)`** — Wedhorn Lemma 7.46(1), first assertion.
If `v` has open support then so does its pullback, because the support of a pullback is the
`φ`-preimage of the support (`Spv.suppFun_comap`) and `φ` is continuous.

No hypothesis on `φ` beyond continuity is needed; the second assertion of 7.46(1), that the
*analytic* locus is preserved, is the one that needs `φ` adic. -/
theorem not_isAnalyticPoint_comap {φ : A →+* B} (hφ : Continuous φ) {v : Spv B}
  ...
  have hsupp : (comap φ v).supp = Ideal.comap φ v.supp := by
    have := suppFun_comap φ v
    simpa [PrimeSpectrum.ext_iff, suppFun_asIdeal] using congrArg PrimeSpectrum.asIdeal this
  rw [hsupp]
...
/-- **The analytic locus is preserved by `Spa(φ)` for adic `φ`** — Wedhorn Lemma 7.46(1), second
assertion.

`main` has no predicate for "`φ` is adic", so the condition is spelled out: the extended ideal of
definition of `B` is generated by the image of that of `A`. That is exactly what adicness supplies,
and it is all the proof uses.

Given an element of `B`'s extended ideal of definition outside `supp v`, some generator `φ a` must
also lie outside — otherwise the whole span, and with it that element, would sit inside the ideal
`supp v`. Then `a` witnesses analyticity of the pullback. -/
theorem isAnalyticPoint_comap_of_extendedIdealOfDefinition_le_span (P : PairOfDefinition A)
    (Q : PairOfDefinition B) {φ : A →+* B}
    (hadic : Q.extendedIdealOfDefinition ≤ Ideal.span (φ '' P.extendedIdealOfDefinition))
    {v : Spv B} (hv : IsAnalyticPoint v) : IsAnalyticPoint (comap φ v) := by
  obtain ⟨b, hbQ, hb⟩ :=
    (isAnalyticPoint_iff_exists_mem_extendedIdealOfDefinition_notMem_supp Q v).mp hv
  rw [isAnalyticPoint_iff_exists_mem_extendedIdealOfDefinition_notMem_supp P]
  by_contra hcon
  push Not at hcon
  refine hb (Ideal.span_le.mpr ?_ (hadic hbQ))
  rintro _ ⟨a, ha, rfl⟩
  simpa using hcon a ha
