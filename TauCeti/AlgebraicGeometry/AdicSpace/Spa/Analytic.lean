...
**Wedhorn, *Adic Spaces* (arXiv:1910.05934v1), Definition 7.39, Remark 7.40(2), (3),
Lemma 7.46(1), and Proposition 7.49.**
...
* `TauCeti.ValuationSpectrum.not_isAnalyticPoint_comap` : **Wedhorn Lemma 7.46(1)**, first
  assertion, that `Spa(φ)` maps the non-analytic locus into the non-analytic locus.
* `TauCeti.ValuationSpectrum.isAnalyticPoint_comap_of_isOpen_map` : **Wedhorn Lemma 7.46(1)**,
  second assertion, that `Spa(φ)` maps the analytic locus into the analytic locus once the image
  of an ideal of definition of `A` generates an open ideal of `B`.
* `TauCeti.ValuationSpectrum.comap_preimage_setOf_isAnalyticPoint` : the two assertions of
  Lemma 7.46(1) together, as an equality of loci.
...
* T. Wedhorn, *Adic Spaces*, arXiv:1910.05934v1, Definition 7.39, Remark 7.40(2), (3),
  Lemma 7.46(1), and Proposition 7.49.
-/
...
/-- **Wedhorn Lemma 7.46(1), first assertion.** A non-analytic point of `Spv B` pulls back along
a continuous `φ : A →+* B` to a non-analytic point of `Spv A`: `Spa(φ)` maps the non-analytic
locus into the non-analytic locus. Continuity of `φ` is the only hypothesis.

The locus form of this and of the second assertion is `comap_preimage_setOf_isAnalyticPoint`. -/
theorem not_isAnalyticPoint_comap {φ : A →+* B} (hφ : Continuous φ) {v : Spv B}
    (hv : ¬ IsAnalyticPoint v) : ¬ IsAnalyticPoint (comap φ v) := by
  rw [isAnalyticPoint_def, not_not] at hv ⊢
  rw [supp_comap]
  exact hv.preimage hφ
...
/-- **Wedhorn Lemma 7.46(1), second assertion.** An analytic point of `Spv B` pulls back along
`φ : A →+* B` to an analytic point of `Spv A`, as soon as the ideal of `B` generated by the image
of an extended ideal of definition of `A` is open: `Spa(φ)` maps the analytic locus into the
analytic locus. Openness is what Wedhorn's hypothesis that `φ` is adic supplies, for every choice
of `P`.

The hypothesis is deliberately openness rather than a containment
`Q.extendedIdealOfDefinition ≤ Ideal.map φ P.extendedIdealOfDefinition` against a pair of
definition `Q` of `B`. A topological ring has many pairs of definition — `PairOfDefinition` is
data, admitting any finitely generated ideal that induces the topology — and their ideals of
definition are comparable only up to powers, so that containment is strictly stronger than
adicness: over `ℤ_p` it already fails for `φ = id`, with `P` at `(p²)` and `Q` at `(p)`. -/
theorem isAnalyticPoint_comap_of_isOpen_map (P : PairOfDefinition A) {φ : A →+* B}
    (hopen : IsOpen (Ideal.map φ P.extendedIdealOfDefinition : Set B))
    {v : Spv B} (hv : IsAnalyticPoint v) : IsAnalyticPoint (comap φ v) := by
  rw [isAnalyticPoint_iff_exists_mem_extendedIdealOfDefinition_notMem_supp P]
  by_contra hcon
  push Not at hcon
  exact hv (Ideal.isOpen_of_isOpen_subideal
    (Ideal.map_le_iff_le_comap.mpr fun a ha ↦ by simpa using hcon a ha) hopen)

/-- **Wedhorn Lemma 7.46(1)** as a statement about loci: under the hypotheses of its two
assertions a point of `Spv B` is analytic exactly when its pullback along `φ` is, so `Spa(φ)` maps
the analytic locus into the analytic locus and the non-analytic locus into the non-analytic
locus. -/
theorem comap_preimage_setOf_isAnalyticPoint (P : PairOfDefinition A) {φ : A →+* B}
    (hφ : Continuous φ) (hopen : IsOpen (Ideal.map φ P.extendedIdealOfDefinition : Set B)) :
    comap φ ⁻¹' {v : Spv A | IsAnalyticPoint v} = {v : Spv B | IsAnalyticPoint v} :=
  Set.Subset.antisymm (fun _ ↦ not_imp_not.mp (not_isAnalyticPoint_comap hφ))
    fun _ ↦ isAnalyticPoint_comap_of_isOpen_map P hopen
