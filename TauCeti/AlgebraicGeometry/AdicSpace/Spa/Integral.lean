...
That criterion returns integrality of `f` over `A⁺`, and a ring of integral elements is
integrally closed in `A` by definition, so `f ∈ A⁺` follows. Nothing beyond a Huber ring and a
ring of integral elements enters: the criterion asks neither for a domain nor for a chosen ring
of definition.
...
theorem mem_of_forall_vle_one {Aplus : Subring A} (hplus : IsRingOfIntegralElements Aplus)
    {f : A} (hf : ∀ v ∈ spa Aplus, v.toValuativeRel.vle f 1) : f ∈ Aplus :=
  Subring.isIntegrallyClosedIn_iff.mp hplus.isIntegrallyClosedIn
    (isIntegral_of_forall_continuous_valuation_le_one hplus.isOpen fun v hcont hv ↦
...
The inclusion `⊆` is the defining condition of `spa` and needs no hypothesis at all; the content
is `mem_of_forall_vle_one`, which supplies `⊇`. -/
theorem coe_eq_setOf_forall_vle_one {Aplus : Subring A} (hplus : IsRingOfIntegralElements Aplus) :
    (Aplus : Set A) = {a | ∀ v ∈ spa Aplus, v.toValuativeRel.vle a 1} :=
  Set.ext fun _ ↦
    ⟨fun ha v hv ↦ ((mem_spa_iff Aplus v).mp hv).2 _ ha, mem_of_forall_vle_one hplus⟩
