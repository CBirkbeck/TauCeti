...
* The instance `T0Space (Spv A)` : The spectral topology of `Spv A` is T0.
-/
...
lemma compactSpace_patchTopology : @CompactSpace (Spv A) (patchTopology A) :=
  have hemb : @Topology.IsClosedEmbedding (Spv A) (A × A → Bool) (patchTopology A) _ toPatch :=
    @Topology.IsClosedEmbedding.mk (Spv A) (A × A → Bool) (patchTopology A) _ toPatch
      (@Topology.IsEmbedding.mk (Spv A) (A × A → Bool) (patchTopology A) _ toPatch
        (@Topology.IsInducing.mk (Spv A) (A × A → Bool) (patchTopology A) _ toPatch rfl)
        toPatch_injective)
      isClosed_range_toPatch
...
/-- The spectral topology of `Spv A` is generated by the basic opens (the defining equation
of the instance). -/
