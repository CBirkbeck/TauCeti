...
It also settles quasi-compactness. Patch-clopen means patch-closed in a compact space, and
compactness survives the passage to the coarser spectral topology, so every patch-clopen set —
in particular every `Spv(A)(T/s)` — is quasi-compact. That is the half of Wedhorn's Theorem 4.9
and Lemma 7.5(1) which says the generating family consists of quasi-compact opens, as opposed to
merely generating the topology.
...
* `TauCeti.ValuationSpectrum.isCompact_of_isClopen_patchTopology` : A patch-clopen set is
...
topology is the *smaller*, so this is `patchTopology A ≤ inferInstance`: the spectral topology
is generated by the basic opens, and each of those is patch-open. -/
...
/-- **Patch-clopen implies quasi-compact.** A set clopen for the patch topology is closed in a
compact space, hence patch-compact; and compactness passes to the coarser spectral topology
along the identity, which `patchTopology_le` makes continuous.

This is what upgrades "generates the topology" to Wedhorn's actual phrasing of Lemma 7.5(1) and
Theorem 4.9, that the rational subsets are a basis of **quasi-compact** opens. -/
lemma isCompact_of_isClopen_patchTopology {U : Set (Spv A)}
    (hU : @IsClopen (Spv A) (patchTopology A) U) : IsCompact U := by
  have hc : @IsCompact (Spv A) (patchTopology A) U :=
    @IsClosed.isCompact (Spv A) (patchTopology A) U compactSpace_patchTopology
      (@IsClopen.isClosed (Spv A) (patchTopology A) U hU)
...
lemma isCompact_basicOpen (f s : A) : IsCompact (basicOpen f s) :=
  isCompact_of_isClopen_patchTopology (isClopen_patchTopology_basicOpen f s)
...
lemma isCompact_basicOpenFinset (T : Finset A) (s : A) : IsCompact (basicOpenFinset T s) :=
  isCompact_of_isClopen_patchTopology (isClopen_patchTopology_basicOpenFinset T s)
