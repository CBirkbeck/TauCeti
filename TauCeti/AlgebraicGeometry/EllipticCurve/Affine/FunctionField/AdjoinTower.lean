...
This is `IntermediateField.extendRight` at `IntermediateField.adjoin K {g}` — the two are the
same term, since `extendRight F M` is `F.map (Algebra.algHom K L M)` and `Algebra.algHom` is an
alias of `IsScalarTower.toAlgHom`. It is spelled with `map` rather than through that abbreviation
because every lemma the tower below runs on — `relfinrank_map_map`, `IntermediateField.adjoin_map`,
`map_map`, `map_mono`, `mem_map` — is stated for `map`, so the abbreviation would have to be
unfolded again at each step, and because `Mathlib.FieldTheory.IntermediateField.ExtendRight` is
not otherwise in this file's import closure. -/
noncomputable def ratFuncAdjoinRange (g : RatFunc K) : IntermediateField K W.FunctionField :=
  (IntermediateField.adjoin K {g}).map (IsScalarTower.toAlgHom K (RatFunc K) W.FunctionField)

/-- The defining equation of `ratFuncAdjoinRange`, in the form used by `IntermediateField.map`
lemmas. -/
