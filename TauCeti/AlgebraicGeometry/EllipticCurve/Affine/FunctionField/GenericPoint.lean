...
* `WeierstrassCurve.Affine.algebraMap_XClass` and `WeierstrassCurve.Affine.algebraMap_YClass`: the
  classes of `X - x` and `Y - y` are `genericX - x` and `genericY - y`.
...
theorem algebraMap_YClass (y : F) :
  ...
  rw [CoordinateRing.YClass, map_sub, map_sub, ← genericY_def, algebraMap_mk_C_C]
