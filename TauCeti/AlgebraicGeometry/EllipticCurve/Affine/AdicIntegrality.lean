...
private lemma valued_rhs_eq {x : F} (hA1 : 1 < Valued.v x) :
  ...
  have h1 : Valued.v (W.a₂ * x ^ 2) ≤ A ^ 2 := by
    rw [map_mul, map_pow]
    exact (mul_le_mul' (valued_a₂_le_one hW) le_rfl).trans_eq (one_mul _)
  have h2 : Valued.v (W.a₄ * x) ≤ A ^ 2 := by
    rw [map_mul]
    exact ((mul_le_mul' (valued_a₄_le_one hW) le_rfl).trans_eq (one_mul _)).trans
      (le_self_pow hA1.le (by lia))
  have h3 : Valued.v W.a₆ ≤ A ^ 2 :=
    (valued_a₆_le_one hW).trans (hA1.le.trans (le_self_pow hA1.le (by lia)))
...
private lemma valued_lhs_eq {x y : F} (hAB : Valued.v x < Valued.v y)
  ...
  have h2 : Valued.v (W.a₁ * x * y) < B ^ 2 := by
    ...
    calc Valued.v W.a₁ * Valued.v x * B ≤ 1 * Valued.v x * B :=
        mul_le_mul' (mul_le_mul' (valued_a₁_le_one hW) le_rfl) le_rfl
      _ = Valued.v x * B := by rw [one_mul]
      _ < B * B := mul_lt_mul_of_pos_right hAB (zero_lt_one.trans hB1)
      _ = B ^ 2 := (sq B).symm
  have h3 : Valued.v (W.a₃ * y) < B ^ 2 := by
    rw [map_mul]
    calc Valued.v W.a₃ * B ≤ 1 * B := mul_le_mul' (valued_a₃_le_one hW) le_rfl
...
private lemma valued_lhs_le {x y : F} {C : ℤᵐ⁰} (hxC : Valued.v x ≤ C)
  ...
  · rw [map_mul, map_mul]
    calc Valued.v W.a₁ * Valued.v x * Valued.v y ≤ 1 * C * C :=
        mul_le_mul' (mul_le_mul' (valued_a₁_le_one hW) hxC) hyC
      _ = C ^ 2 := by rw [one_mul, sq]
  · rw [map_mul]
    calc Valued.v W.a₃ * Valued.v y ≤ 1 * C := mul_le_mul' (valued_a₃_le_one hW) hyC
      _ = C := one_mul C
...
/-- **An affine point that is not a double pole is integral.**
