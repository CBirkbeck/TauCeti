...
/-!
# The coordinate-ring map: surjectivity, and the classes cut out by a point
...
ideal `XYIdeal` cut out by a point. This file fills the two gaps between those: `map` is also
surjective when `f` is, and `map` commutes with all three of those constructions.
...
* `WeierstrassCurve.Affine.CoordinateRing.map_XYIdeal`: `map W f` carries the ideal at a point to
  the ideal at the image point, `XYIdeal W x y ↦ XYIdeal (W.map f) (f x) (y.map f)`.
...
`AdjoinRoot.mapRingEquiv` is the other route to the same object. `map_XYIdeal` is then the
coordinate-ring shadow of "an isomorphism carries the maximal ideal at a point to the maximal ideal
at the image point".
...
separability, `MapsInfinity`, duals, and induced point maps": `XYIdeal` is the ideal at a point, so
`map_XYIdeal` is that compatibility for the induced point map.
...
/-- **`CoordinateRing.map f` carries the ideal at a point to the ideal at the image point**:
`XYIdeal W x y` pushes forward to `XYIdeal (W.map f) (f x) (y.map f)`. -/
