...
* `TauCeti.WeierstrassCurve.Affine.CoordinateRing.evalAlgHom`: evaluation of the coordinate ring
...
* `TauCeti.WeierstrassCurve.Affine.CoordinateRing.algHom_ext`: two algebra homomorphisms out of
  the coordinate ring that agree on the two coordinate functions are equal.
* `TauCeti.WeierstrassCurve.Affine.CoordinateRing.algHom_injective`: over a field, an algebra
...
theorem algHom_ext {f g : W.CoordinateRing →ₐ[R] A}
    ...
    (hY : f (_root_.WeierstrassCurve.Affine.CoordinateRing.mk W Y) =
      g (_root_.WeierstrassCurve.Affine.CoordinateRing.mk W Y)) : f = g := by
...
private noncomputable def evalRingHom
  ...
  (AdjoinRoot.evalEval h).comp
    (_root_.WeierstrassCurve.Affine.CoordinateRing.map W (algebraMap R A))

private theorem evalRingHom_mk
    (h : Polynomial.evalEval x y (W.map (algebraMap R A)).polynomial = 0) (p : R[X][Y]) :
    evalRingHom h (_root_.WeierstrassCurve.Affine.CoordinateRing.mk W p) =
      (p.map (mapRingHom (algebraMap R A))).evalEval x y := by
  rw [evalRingHom, RingHom.comp_apply,
    _root_.WeierstrassCurve.Affine.CoordinateRing.map_mk, AdjoinRoot.evalEval_mk]
...
noncomputable def evalAlgHom (h : (W.map (algebraMap R A)).toAffine.Equation x y) :
  ...
  commutes' c := by
    ...
    have hmk : _root_.WeierstrassCurve.Affine.CoordinateRing.mk W (CC c) =
...
theorem algHom_injective (f : W.CoordinateRing →ₐ[F] A)
    (hx : Transcendental F (f (_root_.WeierstrassCurve.Affine.CoordinateRing.mk W (C X)))) :
    Function.Injective f := by
  apply (injective_iff_map_eq_zero f).2
  intro z hz
  by_contra hz₀
  obtain ⟨p, q, hpq⟩ := _root_.WeierstrassCurve.Affine.CoordinateRing.exists_smul_basis_eq z
  set N : F[X] := Algebra.norm F[X] z with hN_def
  have hN₀ : N ≠ 0 :=
    (Algebra.norm_ne_zero_iff_of_basis
      (_root_.WeierstrassCurve.Affine.CoordinateRing.basis W)).2 hz₀
  have hnorm : algebraMap F[X] W.CoordinateRing N =
      z * _root_.WeierstrassCurve.Affine.CoordinateRing.mk W
        (C p + C q * (-(Y : F[X][Y]) - C (C W.a₁ * X + C W.a₃))) := by
    rw [hN_def, ← hpq]
    simpa [_root_.WeierstrassCurve.Affine.CoordinateRing.smul] using
      _root_.WeierstrassCurve.Affine.CoordinateRing.coe_norm_smul_basis (W' := W) p q
  refine hx ⟨N, hN₀, ?_⟩
