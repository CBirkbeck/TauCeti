...
and nothing more. Of the *target* algebra, only `algHom_ext` drops commutativity: it says `R[W]`
is generated by its two coordinate functions, which constrains the source alone. The evaluation
results stay over a commutative target — they evaluate a bivariate polynomial in it, and those
naming the base-changed curve `W⁄A` need it to be a Weierstrass curve over `A` as well.
...
variable {A : Type*} [Semiring A] [Algebra R A] {W : _root_.WeierstrassCurve.Affine R}
