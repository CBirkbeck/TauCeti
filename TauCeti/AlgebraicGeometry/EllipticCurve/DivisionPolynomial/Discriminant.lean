...
The headline is an **unconditional** divisibility transfer between Mathlib's division polynomials:
for any commutative ring, any `x`, and any `d`,
...
proof is an explicit Bézout certificate in the `b`-invariants. No point of a curve, and no
hypothesis of any kind, appears in that statement.
...
* `TauCeti.WeierstrassCurve.dvd_four_mul_Δ_of_dvd_Ψ₂Sq_of_dvd_four_mul_Ψ₃`: unconditionally, a
  common divisor of `Ψ₂Sq(x)` and `4·Ψ₃(x)` divides `4Δ`.
...
This is the whole arithmetic content of the file, and it is unconditional: it holds for every
commutative ring, every `x`, and every `d`, and no point of a curve occurs in it. `4Δ` lies in the
ideal generated by `Ψ₂Sq(x)` and `4·Ψ₃(x)` at any `x`, exhibited by an explicit Bézout
