...
import TauCeti.FieldTheory.KummerCharacter
...
`TauCetiRoadmap/EllipticCurves/README.md`, **Layer 2**, the Weil pairing
`e_N : E[N] × E[N] → μ_N`, pinned in `Suggested.lean` as `weilPairing`. In Silverman's
construction (AEC III.8.1), for `T ∈ E[N]` one takes `g_T` with `g_T^N = [N]^* f_T`, where
`div f_T = N (T) - N (O)`, and sets `e_N(S, T) = τ_S g_T / g_T`. That is this character, at
`φ = [N]`, `n = N` and `g = g_T`, evaluated at `S`; its multiplicativity in `S` is the
bilinearity of the pairing in its first variable.
...
noncomputable def kummerCharacter (g : W₁.FunctionFieldˣ)
  ...
  TauCeti.kummerCharacter ((translationHom W₁).comp (AddMonoidHom.toMultiplicative φ.ker.subtype))
    n g fun S ↦ by simpa using mem_ker_iff.mp (Multiplicative.toAdd S).2 _ hg
...
theorem algebraMap_kummerCharacter {g : W₁.FunctionFieldˣ}
  ...
  rw [kummerCharacter, TauCeti.algebraMap_kummerCharacter]
  simp
...
theorem kummerCharacter_eq_one_iff {g : W₁.FunctionFieldˣ}
  ...
  simp only [MonoidHom.coe_comp, Function.comp_apply, AddMonoidHom.coe_toMultiplicative,
    translationHom_apply, toAdd_ofAdd, AddSubgroup.coe_subtype]
...
theorem kummerCharacter_eq_of_principal_eq {g g' : W₁.FunctionFieldˣ}
  ...
  refine MonoidHom.ext fun S ↦ Subtype.ext <| Units.ext <|
    (algebraMap F W₁.FunctionField).injective ?_
  rw [algebraMap_kummerCharacter, algebraMap_kummerCharacter, hc, map_mul, AlgEquiv.commutes,
    mul_div_mul_left _ _ (by simp)]
