...
public import TauCeti.AlgebraicGeometry.EllipticCurve.Isogeny.MulByInt
...
generic point built in `Isogeny/MulByInt.lean`.
...
image is read off its degree — `Affine.infinityPlace.algebraMap_eq_sq` says it is the square of
Mathlib's `RatFunc.inftyValuation`, which on a polynomial is `exp (natDegree)`. So the two
...
* `TauCeti.Isogeny.infinityPlace_algebraMap_polynomial`: `v_∞` of a polynomial's image is
  `exp (2 * natDegree)`.
...
/-- **The valuation at infinity of a polynomial in `x`** is `exp` of twice its degree: the
extension `F(W)/F(x)` is quadratic, so `v_∞` is the square of Mathlib's `inftyValuation`, which
on a polynomial is `exp (natDegree)`. This is `infinityPlace.X` for a general polynomial. -/
theorem infinityPlace_algebraMap_polynomial {p : F[X]} (hp : p ≠ 0) :
    W.infinityPlace (algebraMap F[X] W.FunctionField p) =
      WithZero.exp (2 * (p.natDegree : ℤ)) := by
  classical
  -- The norm of a scalar is its `finrank`-th power, and `F(W)/F(x)` is quadratic; then Mathlib
  -- values a nonzero polynomial at its degree. Routed through the coordinate ring, which is the
  -- tower that is registered — `F[X] → RatFunc F → F(W)` is not.
  ...
  rw [Affine.infinityPlace_apply, IsScalarTower.algebraMap_apply F[X] (RatFunc F) W.FunctionField,
    Algebra.norm_algebraMap, Affine.finrank_functionField, map_pow,
...
theorem infinityPlace_phiFunctionField (n : ℤ) :
  ...
  rw [phiFunctionField_eq_algebraMap, infinityPlace_algebraMap_polynomial W (W.Φ_ne_zero n),
    W.natDegree_Φ n]
  push_cast
  ring_nf
...
theorem infinityPlace_psiFunctionField_sq {n : ℤ} (hnF : (n : F) ≠ 0) :
  ...
  rw [psiFunctionField_sq,
    show Affine.CoordinateRing.mk W (C (W.ΨSq n)) =
      algebraMap F[X] W.CoordinateRing (W.ΨSq n) from rfl,
    ← IsScalarTower.algebraMap_apply F[X] W.CoordinateRing W.FunctionField,
    infinityPlace_algebraMap_polynomial W (W.ΨSq_ne_zero hnF), W.natDegree_ΨSq hnF]
