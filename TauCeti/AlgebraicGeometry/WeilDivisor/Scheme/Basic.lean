...
`WeilDivisor`; no scheme-specific copies of those declarations are introduced. The
codimension-one points themselves are supplied by
`TauCeti.AlgebraicGeometry.CodimensionOnePoint`.
