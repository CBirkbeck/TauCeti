...
theorem monodromyFunctor_map_app {p q : CoveringSpace X} (f : p ⟶ q) (x : X) :
    ((monodromyFunctor X).map f).app (FundamentalGroupoid.mk x) =
      eqToHom (Functor.congr_obj (monodromyFunctor_obj p) (FundamentalGroupoid.mk x)) ≫
        ↾(ContinuousMap.fiberMap f.hom.left.hom (proj_hom_comp_hom_left_hom f) x) ≫
...
instance monodromyFunctor_faithful (X : TopCat.{u}) : (monodromyFunctor X).Faithful where
  map_injective {p q} f g h := by
    ...
    calc
      f.hom.left e =
          (ContinuousMap.fiberMap f.hom.left.hom (proj_hom_comp_hom_left_hom f) (p.proj e) e' :
            (q : TopCat)) :=
        (ContinuousMap.fiberMap_apply_coe f.hom.left.hom (proj_hom_comp_hom_left_hom f)
          (p.proj e) e').symm
      _ = ContinuousMap.fiberMap g.hom.left.hom (proj_hom_comp_hom_left_hom g) (p.proj e) e' := by
        exact Subtype.ext_iff.mp happ
      _ = g.hom.left e :=
        ContinuousMap.fiberMap_apply_coe g.hom.left.hom (proj_hom_comp_hom_left_hom g)
          (p.proj e) e'
