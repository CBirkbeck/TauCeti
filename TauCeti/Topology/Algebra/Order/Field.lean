...
bounded error tends to `1`. The additive error washes out under division because the
denominator blows up; no property of the denominator beyond divergence is used. -/
theorem tendsto_ratio_one_of_div_atTop_pm_bounded {𝕜 : Type*} {α : Type*} [Field 𝕜]
    [LinearOrder 𝕜] [IsStrictOrderedRing 𝕜] [TopologicalSpace 𝕜] [OrderTopology 𝕜]
    {l : Filter α} {g f : α → 𝕜} (hg : Tendsto g l atTop)
    (h_le : ∃ C : 𝕜, ∀ᶠ s in l, f s ≤ g s + C)
    (h_lower : ∃ C : 𝕜, ∀ᶠ s in l, g s - C ≤ f s) :
  ...
  refine (add_zero (1 : 𝕜) ▸ h0.const_add 1).congr' ?_
