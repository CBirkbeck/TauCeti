...
The hexacode is Hermitian self-dual but not Euclidean self-orthogonal. The two root choices
`ω` and `ω²` give distinct codes meeting exactly in the constant words; the coordinate
ordering `(1,2,4,6,3,5)` carries the conjugate code back to the original code.

The matrix calculations are stated over a field of characteristic two wherever possible.
The named specialization `galoisFieldCode` uses Mathlib's `GaloisField 2 2` and the chosen
...
theorem galoisFieldCode_def : galoisFieldCode = code omega := (rfl)

/-- Encoding repeats the message in the first three coordinates and appends the checks
`a₀ + ωa₁ + ωa₂`, `ωa₀ + a₁ + ωa₂`, and `ωa₀ + ωa₁ + a₂`. -/
...
theorem vecMul_generatorMatrix_injective (ω : F) :
  ...
  have hG : generatorMatrix ω = (fromCols 1 A).submatrix id finSumFinEquiv.symm := by
    ext i j
    -- `finSumFinEquiv.symm` reduces on these numerals, selecting the two blocks of `fromCols`.
...
/-- The Euclidean Gram matrix is nonzero: the hexacode is not self-orthogonal for the
...
theorem generatorMatrix_mul_transpose_ne_zero :
  ...
  have h01 : (generatorMatrix ω * (generatorMatrix ω)ᵀ) 0 1 = ω ^ 2 := by
    ...
    grind
  intro h
  rw [h, Matrix.zero_apply] at h01
  have hω0 : ω = 0 := (pow_eq_zero_iff (by decide)).mp h01.symm
  simp [hω0] at hω

/-- The Frobenius-conjugate generator checks precisely the hexacode. -/
theorem checkedBy_map_frobenius_generatorMatrix :
  ...
  have hle : code ω ≤ ((generatorMatrix ω).map (frobenius F 2)).checkedBy := by
    ...
    simpa only [Matrix.transpose_mul, Matrix.transpose_transpose, Matrix.transpose_zero] using
      congrArg Matrix.transpose (generatorMatrix_mul_map_frobenius_transpose_eq_zero hω)
...
theorem mem_code_sq_iff [CharP F 2] (ω : F) (x : Fin 6 → F) :
  ...
  simp only [mem_code, ← frobenius_def, ← map_mul, ← map_add,
    (frobenius F 2).injective.eq_iff]
...
@[simp]
theorem mem_galoisFieldCode (x : Fin 6 → GaloisField 2 2) :
    x ∈ galoisFieldCode ↔ x 3 = x 0 + omega * x 1 + omega * x 2 ∧
      x 4 = omega * x 0 + x 1 + omega * x 2 ∧
      x 5 = omega * x 0 + omega * x 1 + x 2 := by
  rw [galoisFieldCode_def, mem_code]

/-- The named hexacode has dimension three over its alphabet. -/
@[simp]
theorem finrank_galoisFieldCode : Module.finrank (GaloisField 2 2) galoisFieldCode = 3 := by
  rw [galoisFieldCode_def, finrank_code]
...
theorem isPermutationEquivalent_code_omega_sq :
    IsPermutationEquivalent (code (omega ^ 2)) galoisFieldCode := by
  rw [galoisFieldCode_def]
