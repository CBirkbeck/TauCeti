...
/-- The defining equation of `sqrtTwoCone₁`, which the definition itself does not expose. -/
@[simp]
theorem sqrtTwoCone₁_def :
    sqrtTwoCone₁ = PointedCone.hull ℝ {(1, 0, 0), (0, -1, 0)} := (rfl)
...
@[simp]
theorem sqrtTwoCone₂_def :
    sqrtTwoCone₂ = PointedCone.hull ℝ {(1, 0, 1), (0, -1, -Real.sqrt 2)} := (rfl)
