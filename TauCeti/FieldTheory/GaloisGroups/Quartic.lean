...
* `TauCeti.hasGaloisLabel_four_one_iff_splits_resolvent`: the row for `4T2` with the resolvent
  cubic asked to split completely, and
  `TauCeti.hasGaloisLabel_four_zero_or_two_iff_existsUnique_isRoot_resolvent`: the row for
  `{4T1, 4T3}` with the resolvent cubic asked to have exactly one root. Both drop the
  discriminant condition, which the resolvent condition already implies.
...
theorem HasGaloisLabel.eq_of_four {j k : TransitiveGroupIndex 4} (hj : HasGaloisLabel f j)
  ...
  hj.eq_of (fun h h' => h.eq_of_four h') hk
...
theorem existsUnique_hasGaloisLabel_four (hsep : f.Separable) (hirr : Irreducible f)
  ...
  existsUnique_hasGaloisLabel hsep hirr hdeg (fun G _ => exists_transitiveGroupLabel_four G)
    fun h h' => h.eq_of_four h'
...
theorem hasGaloisLabel_four_four_iff :
  ...
  have hresdeg := natDegree_quarticD4Spec_specialize f
  have hres0 : quarticD4Spec.specialize F f ≠ 0 := fun h0 => by simp [h0] at hresdeg
  rw [Polynomial.irreducible_iff_roots_eq_zero_of_degree_le_three
      (p := quarticD4Spec.specialize F f) (by omega) (by omega),
    Multiset.eq_zero_iff_forall_notMem]
  simp only [Polynomial.mem_roots hres0, ← not_exists]
...
theorem hasGaloisLabel_four_one_iff_splits_resolvent :
  ...
  · intro h
    ...
    refine ⟨hirr, hdeg, ?_⟩
    apply (splits_iff_isSquare_discr_of_natDegree_eq_three_of_isRoot
      (natDegree_quarticD4Spec_specialize f) hchar ha).2
    rwa [← discr_quarticD4Spec_specialize hf hdeg]
  · rintro ⟨hirr, hdeg, hsplit⟩
    have hresdeg := natDegree_quarticD4Spec_specialize f
    obtain ⟨a, ha⟩ := hsplit.exists_eval_eq_zero
      (degree_ne_of_natDegree_ne (by rw [hresdeg]; norm_num))
    have hressq :=
      (splits_iff_isSquare_discr_of_natDegree_eq_three_of_isRoot hresdeg hchar ha).1 hsplit
...
theorem hasGaloisLabel_four_zero_or_two_iff_existsUnique_isRoot_resolvent :
  ...
  · intro h
    ...
    by_contra hxa
    have hresdeg := natDegree_quarticD4Spec_specialize f
    have hsplit := Splits.of_natDegree_eq_three_of_isRoot_of_isRoot_of_ne hresdeg ha hx hxa
    apply hsq
    rw [discr_quarticD4Spec_specialize hf hdeg]
    exact (splits_iff_isSquare_discr_of_natDegree_eq_three_of_isRoot
      hresdeg hchar ha).1 hsplit
  · rintro ⟨hirr, hdeg, a, ha, huniq⟩
    ...
    have hsq : ¬ IsSquare f.discr := by
      ...
      have hressq : IsSquare (quarticD4Spec.specialize F f).discr := by
        rwa [← discr_quarticD4Spec_specialize hf hdeg]
      have hsplit :=
        (splits_iff_isSquare_discr_of_natDegree_eq_three_of_isRoot hresdeg hchar ha).2 hressq
      obtain ⟨x, hx, hxa⟩ := hsplit.exists_isRoot_ne hressep (by omega) a
      exact hxa (huniq x hx)
