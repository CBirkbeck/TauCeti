...
/-!
# The corners `i`, `ρ` and `ρ + 1` of the fundamental domain
...
The file also collects the elementary corner values every consumer of the fundamental domain's
boundary needs: the real part of `ρ`, the pairwise distinctness of the three corners, and their
common unit modulus.
...
* `UpperHalfPlane.re_ρ`, `UpperHalfPlane.norm_eq_one_of_mem_corners` and the pairwise
...
/-- The corners `i` and `ρ` are distinct. -/
lemma I_ne_ρ : (I : ℍ) ≠ ρ :=
  ne_of_apply_ne UpperHalfPlane.re (by norm_num)

/-- The corners `i` and `ρ + 1` are distinct. -/
lemma I_ne_vadd_ρ : (I : ℍ) ≠ (1 : ℝ) +ᵥ ρ :=
  ne_of_apply_ne UpperHalfPlane.re (by norm_num)

/-- The corners `ρ` and `ρ + 1` are distinct. -/
lemma ρ_ne_vadd_ρ : (ρ : ℍ) ≠ (1 : ℝ) +ᵥ ρ :=
  ne_of_apply_ne UpperHalfPlane.re (by norm_num)

/-- The three corners of the fundamental domain all lie on the unit circle. -/
lemma norm_eq_one_of_mem_corners {z : ℂ}
