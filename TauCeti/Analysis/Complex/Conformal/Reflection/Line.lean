...
into itself by reflection in the line `p + a·ℝ`, then its pullback `φ⁻¹(Ω)` along `φ w = p + a·w` is
carried into itself by ordinary complex conjugation — which is what the half-plane reflection
theorem asks for. -/
...
/-- **Continuity transports to the chart.** Reading `f` in the source chart and its values in the
target chart turns continuity on the closed positive side of the line into continuity on the closed
upper half-plane. -/
...
/-- **Holomorphy transports to the chart**, the open-side companion of
...
/-- **The boundary condition transports to the real axis.** On the chart, the source line is the
real axis, so the hypothesis that `f` sends the line into the target line becomes the hypothesis
that the chart-read function is real on the reals. -/
