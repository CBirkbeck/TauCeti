...
The drift operator's versions of the two facts above, and the maximizer step they support. They
live here rather than with the drift maximum principle because both the `-Δ + c` and the
`-Δ - b·∇ + c` weak principles run that step, and neither file is upstream of the other. -/
...
/-- The operator `Δ + b·∇` is linear under addition of a constant multiple. This is the drift
version of `TauCeti.laplacian_add_const_smul_norm_sq`, for an arbitrary barrier. -/
...
/-- **The maximizer step shared by the lower-order weak maximum principles.** At a maximizer `z`
of `f + ε • w` over `K`, where `w` is a barrier that is strictly positive under `Δ + b·∇` at `z`,
the frontier bound already holds. On the frontier that is the hypothesis; in the interior,
`f z > m ≥ 0` makes `Δ f + ∇_b f` nonnegative there, so the perturbed sum is strictly positive and
`z` cannot be a local maximum.

The barrier is arbitrary: only its regularity and the sign of `Δ w + ∇_b w` at `z` are used, and
both only when `z` is interior, since the frontier branch is the hypothesis `hbdry` outright.
Taking `b = 0` gives the step used by the `-Δ + c` principle, and `c = 0` the one used by the
bare-Laplacian principle. -/
