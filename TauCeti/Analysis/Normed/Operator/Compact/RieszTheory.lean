...
/-- **One step of the chain of ranges.** `range (T ^ (n + 1))` is the image under `T` of
`range (T ^ n)`, so the chain `range T ⊇ range T² ⊇ ⋯` is generated by iterating `Submodule.map T`
from `range T ^ 0 = ⊤`. -/
private lemma range_pow_succ (T : X →L[𝕜] X) (n : ℕ) :
    LinearMap.range ((T ^ (n + 1) : X →L[𝕜] X) : X →ₗ[𝕜] X) =
      Submodule.map (T : X →ₗ[𝕜] X) (LinearMap.range ((T ^ n : X →L[𝕜] X) : X →ₗ[𝕜] X)) := by
  ext x
  simp only [LinearMap.mem_range, Submodule.mem_map, ContinuousLinearMap.coe_coe]
  constructor
  · rintro ⟨z, rfl⟩
    exact ⟨(T ^ n) z, ⟨z, rfl⟩, by rw [pow_succ']; rfl⟩
  · rintro ⟨w, ⟨z, rfl⟩, rfl⟩
    exact ⟨z, by rw [pow_succ']; rfl⟩

omit [CompleteSpace 𝕜] [IsRCLikeNormedField 𝕜] [CompleteSpace X] in
/-- **A chain generated by `Submodule.map T` is constant past its first repeat.** One equality
`V (p + 1) = V p` propagates forward, because each later term is the `T`-image of its predecessor
and equal inputs give equal images. This is what makes "the chain of ranges stabilises" usable:
the Riesz argument produces a single repeat, and everything after it follows. -/
private lemma eq_of_map_eq_of_succ_eq {T : X →ₗ[𝕜] X} {V : ℕ → Submodule 𝕜 X} {p : ℕ}
    (hmap : ∀ n, V (n + 1) = Submodule.map T (V n)) (hp : V (p + 1) = V p) (n : ℕ) :
    V (p + n) = V p := by
  induction n with
  | zero => simp
...
omit [CompleteSpace 𝕜] [IsRCLikeNormedField 𝕜] [CompleteSpace X] in
/-- **Once the chain of ranges repeats, the kernel surjects onto the cokernel.** Suppose
`range (T ^ (m + m)) = range (T ^ m)`. Given `x`, the repeat writes `T ^ m x` as `T ^ m (T ^ m z)`,
so `x - T ^ m z` lies in `ker (T ^ m)` while `T ^ m z` lies in `range T`; the two summands are
exactly a preimage of `x` in the quotient. Positivity of `m` is what puts `T ^ m z` in `range T`. -/
private lemma surjective_mkQ_comp_ker_subtype {T : X →L[𝕜] X} {n : ℕ}
    (hn : LinearMap.range ((T ^ (n + 1 + (n + 1)) : X →L[𝕜] X) : X →ₗ[𝕜] X) =
      LinearMap.range ((T ^ (n + 1) : X →L[𝕜] X) : X →ₗ[𝕜] X)) :
    Function.Surjective ((LinearMap.range (T : X →ₗ[𝕜] X)).mkQ.comp
      (LinearMap.ker ((T ^ (n + 1) : X →L[𝕜] X) : X →ₗ[𝕜] X)).subtype) := by
  intro ξ
  obtain ⟨x, rfl⟩ := Submodule.mkQ_surjective _ ξ
  have hx : (T ^ (n + 1)) x ∈
      LinearMap.range ((T ^ (n + 1 + (n + 1)) : X →L[𝕜] X) : X →ₗ[𝕜] X) := by
    rw [hn]; exact ⟨x, rfl⟩
  obtain ⟨z, hz⟩ := hx
  have hcomp : (T ^ (n + 1 + (n + 1))) z = (T ^ (n + 1)) ((T ^ (n + 1)) z) := by
    rw [pow_add]; rfl
  simp only [ContinuousLinearMap.coe_coe] at hz
  rw [hcomp] at hz
  refine ⟨⟨x - (T ^ (n + 1)) z, ?_⟩, ?_⟩
  · rw [LinearMap.mem_ker, map_sub, sub_eq_zero]
    simpa only [ContinuousLinearMap.coe_coe] using hz.symm
...
theorem finiteDimensional_quotient_range_one_sub (hK : IsCompactOperator K) :
  ...
  have hVmap : ∀ n, V (n + 1) = Submodule.map (A : X →ₗ[𝕜] X) (V n) := fun n => range_pow_succ A n
  ...
  have hconst : ∀ n, V (p + n) = V p := eq_of_map_eq_of_succ_eq hVmap hp
  ...
  have hkerfin : FiniteDimensional 𝕜
    ...
    rw [sub_sub_cancel, ← hA] at h
    exact h
  exact FiniteDimensional.of_surjective _ (surjective_mkQ_comp_ker_subtype hV2)
