...
`TauCeti.Analysis.Contour.Winding.Number.Circle`, which this module re-exports so existing imports
keep working.

What remains here are the two arc values the valence formula names.
