...
* `nsmulAddMonoidHom_range_prod` and `nsmulAddMonoidHom_ker_prod`: both `n • (A × B)` and
  `(A × B)[n]` are computed componentwise.
* `AddMonoidHom.range_nsmulAddMonoidHom_eq_span`: on a commutative ring, the range of
  multiplication by `n` is the ideal generated by `n`. Mathlib's `Int.range_nsmulAddMonoidHom` is
...
/-- On a commutative ring, the range of multiplication by `n` as an additive map is the ideal
generated by `n`.

Mathlib's `Int.range_nsmulAddMonoidHom` is the `R = ℤ` case, stated as
`AddSubgroup.zmultiples (n : ℤ)`. -/
-- Adapted from `AddMonoidHom.range_nsmulAddMonoidHom` in Michael Stoll's `EllipticCurves`
-- (`EllipticCurves/Mathlib/SelmerGroup.lean`, pin `66889eada51a`), renamed: the base name
-- `range_nsmulAddMonoidHom` is Mathlib's for the `ℤ`-only statement.
lemma range_nsmulAddMonoidHom_eq_span (R : Type*) [CommRing R] (n : ℕ) :
...
/-- `n • (A × B)` is computed componentwise. -/
-- Adapted from `nsmulAddMonoidHom_range_prod` in Michael Stoll's `EllipticCurves`
-- (`EllipticCurves/Mathlib/SelmerGroup.lean`, pin `66889eada51a`).
...
/-- `(A × B)[n]` is computed componentwise. -/
-- Adapted from `nsmulAddMonoidHom_ker_prod` in Michael Stoll's `EllipticCurves`
-- (`EllipticCurves/Mathlib/SelmerGroup.lean`, pin `66889eada51a`).
