...
theorem isPElementary_pElementaryOfSylow [Fact p.Prime] (hs : ¬ p ∣ orderOf s) :
  ...
  refine isPElementary_sup_of_commute inferInstance hCp hQ
    (disjoint_of_not_dvd_natCard_of_isPGroup hCp hQ) ?_
  · rintro c hc - ⟨y, -, rfl⟩
    obtain ⟨k, rfl⟩ := mem_zpowers_iff.1 hc
    have hcom : Commute ((y : G)) s := mem_centralizer_singleton_iff.1 (SetLike.coe_mem y)
    exact hcom.symm.zpow_left k
