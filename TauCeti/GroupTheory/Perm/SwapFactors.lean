...
`Equiv.Perm.SameCycle.sameCycle_or_of_swap_mul` and
`TauCeti.sameCycle_swap_mul_of_mem_periodicPts_of_not_sameCycle` prove between them for the single
element `Equiv.swap a b * σ` of that group. -/
...
theorem _root_.Equiv.Perm.SameCycle.sameCycle_or_of_swap_mul
    (h : (Equiv.swap a b * σ).SameCycle x y) :
    σ.SameCycle x y ∨
...
theorem _root_.Equiv.Perm.SameCycle.swap_mul_of_sameCycle
    (hab : (Equiv.swap a b * σ).SameCycle a b) (h : σ.SameCycle x y) :
    (Equiv.swap a b * σ).SameCycle x y := by
  have h' : (Equiv.swap a b * (Equiv.swap a b * σ)).SameCycle x y := by
    rwa [Equiv.swap_mul_self_mul]
  rcases h'.sameCycle_or_of_swap_mul with hxy | ⟨hx, hy⟩
  · exact hxy
...
to `a`. -/
theorem sameCycle_swap_mul_of_mem_periodicPts_of_not_sameCycle
...
private theorem mem_periodicPts_and_not_sameCycle_swap_mul_of_sameCycle
  ...
  obtain ⟨j, hjpos, hja, hjmin⟩ : ∃ j : ℕ, 0 < j ∧ (σ ^ j) a = b ∧
    ...
    have hex := h.exists_pos_pow_eq_of_mem_periodicPts ha
  ...
  have hwalk : ∀ i, 0 < i → i < j → (σ ^ i) a ≠ a ∧ (σ ^ i) a ≠ b := by
    ...
    have hgo : (σ ^ (j - i)) ((σ ^ i) a) = b := by
      ...
      exact hja
    rwa [hcon] at hgo
  have hfix : Function.IsPeriodicPt (Equiv.swap a b * σ : α → α) j a :=
    pow_swap_mul_apply_eq hjpos hwalk hja
  have hper : a ∈ Function.periodicPts (Equiv.swap a b * σ : α → α) :=
    Function.mem_periodicPts.mpr ⟨j, hjpos, hfix⟩
  refine ⟨hper, fun hsc => ?_⟩
  -- The orbit of `a` under the product is spanned by the powers below `j`, and none of them is `b`.
  obtain ⟨r, -, hr⟩ := hsc.exists_pos_pow_eq_of_mem_periodicPts hper
  have hlt : r % j < j := Nat.mod_lt _ hjpos
  have hmod : ((Equiv.swap a b * σ) ^ (r % j)) a = b := by
    rw [Equiv.Perm.coe_pow, hfix.iterate_mod_apply r, ← Equiv.Perm.coe_pow]
    exact hr
  rcases Nat.eq_zero_or_pos (r % j) with hr0 | hrpos
  · rw [hr0, pow_zero] at hmod
    exact hab (by simpa using hmod)
  · rw [pow_swap_mul_apply_of_lt hwalk (r % j) hlt] at hmod
    exact hjmin (r % j) hrpos hlt hmod
...
`b` is not on. -/
theorem not_sameCycle_swap_mul_of_mem_periodicPts_of_ne_of_sameCycle
    (ha : a ∈ Function.periodicPts (σ : α → α)) (hab : a ≠ b) (h : σ.SameCycle a b) :
...
theorem orbitCount_swap_mul_add_one_of_mem_periodicPts_of_not_sameCycle
    [Finite (Quotient (SameCycle.setoid σ))]
    (hb : b ∈ Function.periodicPts (σ : α → α)) (h : ¬ σ.SameCycle a b) :
    orbitCount (Equiv.swap a b * σ) + 1 = orbitCount σ :=
  have hab := sameCycle_swap_mul_of_mem_periodicPts_of_not_sameCycle hb h
  orbitCount_add_one_of_merge (fun huv => hab.swap_mul_of_sameCycle huv)
    (fun huv => huv.sameCycle_or_of_swap_mul) hab h
...
theorem orbitCount_swap_mul_add_one_of_not_sameCycle [Finite α] (h : ¬ σ.SameCycle a b) :
    orbitCount (Equiv.swap a b * σ) + 1 = orbitCount σ :=
  orbitCount_swap_mul_add_one_of_mem_periodicPts_of_not_sameCycle
...
theorem orbitCount_swap_mul_of_mem_periodicPts_of_ne_of_sameCycle
  ...
  have hmerge := orbitCount_swap_mul_add_one_of_mem_periodicPts_of_not_sameCycle
...
theorem orbitCount_swap_mul_of_sameCycle [Finite α] (hab : a ≠ b) (h : σ.SameCycle a b) :
  ...
  orbitCount_swap_mul_of_mem_periodicPts_of_ne_of_sameCycle
...
theorem orbitCount_mul_swap_add_one_of_mem_periodicPts_of_not_sameCycle
    [Finite (Quotient (SameCycle.setoid σ))]
    (hb : b ∈ Function.periodicPts (σ : α → α)) (h : ¬ σ.SameCycle a b) :
    orbitCount (σ * Equiv.swap a b) + 1 = orbitCount σ := by
  rw [Equiv.mul_swap_eq_swap_mul]
  refine orbitCount_swap_mul_add_one_of_mem_periodicPts_of_not_sameCycle
...
theorem orbitCount_mul_swap_add_one_of_not_sameCycle [Finite α] (h : ¬ σ.SameCycle a b) :
    orbitCount (σ * Equiv.swap a b) + 1 = orbitCount σ :=
  orbitCount_mul_swap_add_one_of_mem_periodicPts_of_not_sameCycle
    (σ.injective.mem_periodicPts b) h
...
theorem orbitCount_mul_swap_of_mem_periodicPts_of_ne_of_sameCycle
  ...
  let _ : Finite (Quotient
    ...
    infer_instance
  rw [Equiv.mul_swap_eq_swap_mul]
  refine orbitCount_swap_mul_of_mem_periodicPts_of_ne_of_sameCycle
...
theorem orbitCount_mul_swap_of_sameCycle [Finite α] (hab : a ≠ b) (h : σ.SameCycle a b) :
    orbitCount (σ * Equiv.swap a b) = orbitCount σ + 1 :=
  orbitCount_mul_swap_of_mem_periodicPts_of_ne_of_sameCycle
    (σ.injective.mem_periodicPts a) hab h
...
-- Source: the combinatorial route of Layer 0.6 of the Tau Ceti `BelyiMaps` roadmap, which
-- proves the connected Euler bound (step 3) from a transposition factorization; here that
-- argument is run one orbit of the generated group at a time, as needed for the general bound
-- `χ(t) ≤ 2 · (number of monodromy orbits)` (step 4).
...
theorem card_add_orbitCount_le_length_add_two_mul_card_orbits [Finite α]
    {L : List (Perm α)} (hL : ∀ g ∈ L, g.IsSwap) :
    Nat.card α + orbitCount L.prod ≤ L.length +
      2 * Nat.card (MulAction.orbitRel.Quotient (Subgroup.closure {g | g ∈ L}) α) := by
  cases nonempty_fintype α
  let H := Subgroup.closure {g : Perm α | g ∈ L}
  obtain ⟨π, hbound, hpres⟩ := exists_componentPerm L hL
  have hclosure : H ≤ cyclePreservingSubgroup π := by
    rw [Subgroup.closure_le]
    exact fun g hg => hpres g hg
  let f : MulAction.orbitRel.Quotient H α → Quotient (SameCycle.setoid π) :=
    fun q => Quotient.liftOn' q (fun x => Quotient.mk'' x) fun a b hab => by
      apply Quotient.sound
      rw [MulAction.orbitRel_apply, MulAction.mem_orbit_iff] at hab
      obtain ⟨g, rfl⟩ := hab
      exact (mem_cyclePreservingSubgroup.mp (hclosure g.2) b).symm
...
private theorem swapFactorsAux_orbitCount_add_length_eq_card [Fintype α]
  ...
  | nil =>
    ...
    subst h1
    rw [Equiv.Perm.swapFactorsAux.eq_1]
    simp
  | cons x l ih =>
    by_cases hx : x = σ x
    ...
    · have hsplit : orbitCount (Equiv.swap x (σ x) * σ) = orbitCount σ + 1 :=
      ...
      have hτ : ∀ {y : α}, (Equiv.swap x (σ x) * σ) y ≠ y → y ∈ l := fun {y} hy =>
      ...
      have hrec := ih (Equiv.swap x (σ x) * σ) hτ
      rw [Equiv.Perm.swapFactorsAux.eq_2]
      dsimp only
      rw [dite_eq_right hx, List.length_cons]
      -- The recursive call in `Equiv.Perm.swapFactorsAux` carries a membership proof built inside
      -- that definition; `hrec` uses `hτ` instead, and the two agree by proof irrelevance.
      omega
