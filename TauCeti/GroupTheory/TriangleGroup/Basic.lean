...
theorem closure_x_y : Subgroup.closure {x a b c, y a b c} = ⊤ := by
  ...
  · exact hx
  · exact hy
  · -- `PresentedGroup.of 2` is `z` by definition
    exact (z_eq a b c ▸ inv_mem (mul_mem hy hx) : z a b c ∈ Subgroup.closure {x a b c, y a b c})
...
theorem hom_ext {f g : TriangleGroup a b c →* G} (hx : f (x a b c) = g (x a b c))
  ...
  · exact hy
  · -- `PresentedGroup.of 2` is `z` by definition
    exact (by simp only [z_eq, map_inv, map_mul, hx, hy] : f (z a b c) = g (z a b c))
