...
integer-specific one.

## Main definitions

* `Matrix.SpecialLinearGroup.transvection`: the transvection `I + c·e_{ij}` as an element of
  `SL_n(R)`.
* `Matrix.SpecialLinearGroup.IsTransvection`: the predicate selecting these elements.

## Main results

* `Matrix.SpecialLinearGroup.exists_list_isTransvection_prod`: every `σ ∈ SL_n(ℤ)` is the
  product of a list of transvections.
* `Matrix.SpecialLinearGroup.closure_isTransvection_eq_top`: `SL_n(ℤ)` is generated by
  its transvections.
...
/-- An element of `SL_n(ℤ)` is an elementary transvection `I + c·e_{ij}`.  Private: the
public statements are phrased through `Matrix.TransvectionStruct`. -/
private def IsTransvection {m : ℕ} (E : SpecialLinearGroup (Fin m) ℤ) : Prop :=
  ∃ (i j : Fin m) (hij : i ≠ j) (c : ℤ), E = transvection hij c
...
private lemma transvection_mul_right_entry {m : ℕ} [NeZero m] (i j : Fin m) (hij : i ≠ j) (c : ℤ)
    (σ : SpecialLinearGroup (Fin m) ℤ) (a b : Fin m) :
...
private lemma isTransvection_append {m : ℕ} (L₁ L₂ : List (SpecialLinearGroup (Fin m) ℤ))
    (h₁ : ∀ E ∈ L₁, IsTransvection E) (h₂ : ∀ E ∈ L₂, IsTransvection E) :
    ∀ E ∈ L₁ ++ L₂, IsTransvection E :=
...
private lemma col0_euclidean_step {m : ℕ} (σ : SpecialLinearGroup (Fin (m + 1)) ℤ)
    ...
    ∃ (i j : Fin (m + 1)) (hij : i ≠ j) (c : ℤ),
      col0Sum (transvection hij c * σ) < col0Sum σ := by
...
private lemma col0_reduce {m : ℕ} (σ : SpecialLinearGroup (Fin (m + 1)) ℤ) :
    ∃ (L : List (SpecialLinearGroup (Fin (m + 1)) ℤ)),
      (∀ E ∈ L, IsTransvection E) ∧ nzCount (L.prod * σ) ≤ 1 := by
  suffices key : ∀ (k : ℕ) (τ : SpecialLinearGroup (Fin (m + 1)) ℤ),
      col0Sum τ ≤ k → ∃ (L : List (SpecialLinearGroup (Fin (m + 1)) ℤ)),
        (∀ E ∈ L, IsTransvection E) ∧ nzCount (L.prod * τ) ≤ 1 from
...
private lemma isTransvection_inv {m : ℕ} {E : SpecialLinearGroup (Fin m) ℤ}
    (hE : IsTransvection E) : IsTransvection E⁻¹ := by
  obtain ⟨i, j, hij, c, rfl⟩ := hE
  rw [transvection_inv]; exact ⟨i, j, hij, -c, rfl⟩

private lemma transvec_list_inv {m : ℕ} (L : List (SpecialLinearGroup (Fin m) ℤ))
    (hL : ∀ E ∈ L, IsTransvection E) :
    ∃ L' : List (SpecialLinearGroup (Fin m) ℤ),
      (∀ E ∈ L', IsTransvection E) ∧ L.prod⁻¹ = L'.prod := by
  refine ⟨(L.map (·⁻¹)).reverse, fun E hE ↦ ?_, List.prod_inv_reverse L⟩
  rw [List.mem_reverse, List.mem_map] at hE
  obtain ⟨F, hF, rfl⟩ := hE; exact isTransvection_inv (hL F hF)
...
private lemma blockLift_isTransvection {m : ℕ} (i j : Fin m) (hij : i ≠ j) (c : ℤ) :
    IsTransvection (blockLift i j hij c) :=
...
private lemma row0_clear_step {m : ℕ} (σ : SpecialLinearGroup (Fin (m + 1)) ℤ)
    ...
    ∃ E, IsTransvection E ∧ (σ * E).1 0 0 = 1 ∧ (∀ i, i ≠ 0 → (σ * E).1 i 0 = 0) ∧
  ...
  have hj₀_entry : σ.1 0 j₀ ≠ 0 := fun h ↦ by simp [h, hj₀_val] at hj₀_nz
  set E := transvection hj₀.symm (-σ.1 0 j₀)
  refine ⟨E, ⟨0, j₀, hj₀.symm, _, rfl⟩, ?_, ?_, ?_⟩
  · rw [transvection_mul_right_entry]; simp [hj₀.symm, hσ00]
  · intro i hi; rw [transvection_mul_right_entry]; simp [hj₀.symm, hσi0 i hi]
  · have hclear : (σ * E).1 0 j₀ = 0 := by
      rw [transvection_mul_right_entry]; simp [hσ00]
    have hother : ∀ k, k ≠ j₀ → (σ * E).1 0 k = σ.1 0 k := fun k hk ↦ by
      rw [transvection_mul_right_entry]; simp [hk]
    simp only [row0Sum]
...
private lemma row0_clear {m : ℕ} (τ : SpecialLinearGroup (Fin (m + 1)) ℤ)
  ...
  · exact ⟨[], fun _ h ↦ absurd h (by simp), by simp [hσ00],
  ...
  · obtain ⟨E, hE_tv, hσ'00, hσ'i0, hlt⟩ := row0_clear_step σ hσ00 hσi0 hzero
    ...
    · intro j hj; simpa only [List.prod_cons, ← mul_assoc] using hL'_0j j hj
    · intro i hi; simpa only [List.prod_cons, ← mul_assoc] using hL'_i0 i hi
...
private lemma block_form_transvec_lift {m : ℕ} (M : SpecialLinearGroup (Fin (m + 1)) ℤ)
    ...
    (Hi0 : ∀ i, i ≠ 0 → M.1 i 0 = 0)
    (L : List (SpecialLinearGroup (Fin m) ℤ)) (hL : ∀ E ∈ L, IsTransvection E)
    (hL_eq : extractBlock M H00 H0j Hi0 = L.prod) :
    ∃ (L' : List (SpecialLinearGroup (Fin (m + 1)) ℤ)),
      (∀ E ∈ L', IsTransvection E) ∧ M = L'.prod := by
  ...
  | cons E L' ihL' =>
    ...
    have hext' : extractBlock (blockLift i j hij (-c) * M) H00' H0j' Hi0' = L'.prod :=
      (extractBlock_blockLift i j hij (-c) M H00 H0j Hi0).trans
        (by rw [hL_eq, ← mul_assoc, ← transvection_add, neg_add_cancel,
    ...
    refine ⟨blockLift i j hij c :: L'',
      fun F hF ↦ (List.mem_cons.mp hF).elim
        (fun h ↦ h ▸ blockLift_isTransvection i j hij c) (hL''_tv F), ?_⟩
    simp only [List.prod_cons]; rw [← hL''_eq, ← mul_assoc]
    unfold blockLift; rw [← transvection_add, add_neg_cancel, transvection_coeff_zero, one_mul]
...
private lemma column_pivot_of_neg_one_at_zero {m' : ℕ}
    ...
    ∃ (L₁ : List (SpecialLinearGroup (Fin (m' + 2)) ℤ)),
      (∀ E ∈ L₁, IsTransvection E) ∧ (L₁.prod * τ).1 0 0 = 1 ∧
      (∀ i, i ≠ 0 → (L₁.prod * τ).1 i 0 = 0) := by
  have h10 : (1 : Fin (m' + 2)) ≠ 0 := by simp
  have h01 : (0 : Fin (m' + 2)) ≠ 1 := h10.symm
  set σ₁ := transvection h10 1 * τ
  have hσ₁_00 : σ₁.1 0 0 = -1 := by rw [transvection_mul_entry]; simp [h01, h_neg1]
  have hσ₁_10 : σ₁.1 1 0 = -1 := by
    rw [transvection_mul_entry]; simp [h_neg1, h_others 1 h10]
  have hσ₁_i0 : ∀ i, i ≠ 0 → i ≠ 1 → σ₁.1 i 0 = 0 := fun i hi0 hi1 ↦ by
    rw [transvection_mul_entry]; simp [hi1, h_others i hi0]
  set σ₂ := transvection h01 (-2) * σ₁
  have hσ₂_00 : σ₂.1 0 0 = 1 := by rw [transvection_mul_entry]; simp [hσ₁_00, hσ₁_10]
  have hσ₂_10 : σ₂.1 1 0 = -1 := by rw [transvection_mul_entry]; simp [h10, hσ₁_10]
  have hσ₂_i0 : ∀ i, i ≠ 0 → i ≠ 1 → σ₂.1 i 0 = 0 := fun i hi0 hi1 ↦ by
    rw [transvection_mul_entry]; simp [hi0, hσ₁_i0 i hi0 hi1]
  set σ₃ := transvection h10 1 * σ₂
  have hσ₃_00 : σ₃.1 0 0 = 1 := by rw [transvection_mul_entry]; simp [h01, hσ₂_00]
  have hσ₃_i0 : ∀ i, i ≠ 0 → σ₃.1 i 0 = 0 := fun i hi ↦ by
    rw [transvection_mul_entry]
    rcases eq_or_ne i 1 with rfl | hi1
    · simp [hσ₂_00, hσ₂_10]
    · simp [hi1, hσ₂_i0 i hi hi1]
  have hprod : [transvection h10 1, transvection h01 (-2),
      transvection h10 1].prod * τ = σ₃ := by
    simp only [List.prod_cons, List.prod_nil, mul_one, mul_assoc, σ₃, σ₂, σ₁]
  exact ⟨[transvection h10 1, transvection h01 (-2), transvection h10 1],
    fun E hE ↦ by
      simp only [List.mem_cons, List.mem_nil_iff, or_false] at hE
      rcases hE with rfl | rfl | rfl <;> exact ⟨_, _, _, _, rfl⟩,
    by rw [hprod]; exact hσ₃_00, fun i hi ↦ by rw [hprod]; exact hσ₃_i0 i hi⟩
...
private lemma column_pivot_of_unit_off_diagonal {m : ℕ} [NeZero (m + 1)]
  ...
  set σ₁ := transvection hi₀0 v * τ
  have hσ₁_00 : σ₁.1 0 0 = 1 := by
    rw [transvection_mul_entry]; simp [h_others 0 hi₀0, ← hv_def, hv2]
  have hσ₁_i₀0 : σ₁.1 i₀ 0 = v := by
    rw [transvection_mul_entry]; simp [hi₀_zero, ← hv_def]
  have hσ₁_other : ∀ k, k ≠ 0 → k ≠ i₀ → σ₁.1 k 0 = 0 := fun k hk0 hki₀ ↦ by
    rw [transvection_mul_entry]; simp [hk0, h_others k hki₀]
  set σ₂ := transvection hi₀_zero (-v) * σ₁
  have hσ₂_00 : σ₂.1 0 0 = 1 := by rw [transvection_mul_entry]; simp [hi₀0, hσ₁_00]
  have hσ₂_i0 : ∀ i, i ≠ 0 → σ₂.1 i 0 = 0 := fun i hi ↦ by
    rw [transvection_mul_entry]
    rcases eq_or_ne i i₀ with rfl | hne
    · simp [hσ₁_i₀0, hσ₁_00, add_neg_cancel]
    · simp [hne, hσ₁_other i hi hne]
  have hprod : [transvection hi₀_zero (-v),
      transvection hi₀0 v].prod * τ = σ₂ := by
    simp only [List.prod_cons, List.prod_nil, mul_one, mul_assoc, σ₂, σ₁]
  exact ⟨[transvection hi₀_zero (-v), transvection hi₀0 v],
    fun E hE ↦ by
      simp only [List.mem_cons, List.mem_nil_iff, or_false] at hE
...
private lemma to_block_form {m : ℕ} (τ : SpecialLinearGroup (Fin (m + 1)) ℤ)
    ...
    ∃ (L_left L_right : List (SpecialLinearGroup (Fin (m + 1)) ℤ)),
      (∀ E ∈ L_left, IsTransvection E) ∧ (∀ E ∈ L_right, IsTransvection E) ∧
  ...
  suffices h_col : ∃ (L₁ : List (SpecialLinearGroup (Fin (m + 1)) ℤ)),
      (∀ E ∈ L₁, IsTransvection E) ∧
      (L₁.prod * τ).1 0 0 = 1 ∧
      (∀ i, i ≠ 0 → (L₁.prod * τ).1 i 0 = 0) by
    ...
    exact ⟨L₁, L₂, hL₁, hL₂, h₂_00, h₂_0j, h₂_i0⟩
...
private lemma exists_list_isTransvection_prod (m : ℕ) (σ : SpecialLinearGroup (Fin m) ℤ) :
    ∃ (L : List (SpecialLinearGroup (Fin m) ℤ)),
      (∀ E ∈ L, IsTransvection E) ∧ σ = L.prod := by
  induction m with
  | zero =>
    refine ⟨[], fun _ h ↦ absurd h (by simp), ?_⟩
  ...
  | succ m ih =>
    ...
    set τ := L_col.prod * σ
    have hσ_eq : σ = L_col.prod⁻¹ * τ := by simp [τ]
    ...
    refine ⟨L_inv ++ L_left_inv ++ L_block ++ L_right_inv,
      isTransvection_append _ _ (isTransvection_append _ _
        (isTransvection_append _ _ hL_inv_tv hL_left_inv_tv) hL_block_tv) hL_right_inv_tv, ?_⟩
...
/-- Every element of `SL_n(ℤ)` is a product of elementary transvections. Integer analogue of
the division-ring generation theorem; the proof is the Euclidean algorithm on columns. -/
theorem exists_list_transvectionStruct_prod (m : ℕ) (σ : SpecialLinearGroup (Fin m) ℤ) :
    ∃ T : List (TransvectionStruct (Fin m) ℤ),
      σ = (T.map TransvectionStruct.toSpecialLinearGroup).prod := by
  obtain ⟨L, hL, rfl⟩ := exists_list_isTransvection_prod m σ
  suffices h : ∃ T : List (TransvectionStruct (Fin m) ℤ),
      T.map TransvectionStruct.toSpecialLinearGroup = L from
    h.imp fun T hT ↦ by rw [hT]
  induction L with
  | nil => exact ⟨[], rfl⟩
  | cons E L ih =>
    obtain ⟨T', hT'⟩ := ih fun F hF ↦ hL F (List.mem_cons_of_mem E hF)
    obtain ⟨i, j, hij, c, rfl⟩ := hL E List.mem_cons_self
    exact ⟨⟨i, j, hij, c⟩ :: T',
      by rw [List.map_cons, hT', TransvectionStruct.toSpecialLinearGroup_mk]⟩

/-- `SL_n(ℤ)` is generated by its elementary transvections. -/
theorem closure_range_toSpecialLinearGroup_eq_top (m : ℕ) :
  ...
  rw [eq_top_iff]
  intro σ _
  obtain ⟨T, rfl⟩ := exists_list_transvectionStruct_prod m σ
