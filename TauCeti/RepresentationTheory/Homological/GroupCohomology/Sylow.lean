...
1.31). -/
theorem natCard_nsmul_eq_zero (x : groupCohomology A (n + 1)) : Nat.card G • x = 0 := by
...
theorem eq_zero_of_map_sylow_eq_zero (p : ℕ) [Fact p.Prime] (P : Sylow p G)
  ...
  exact (nsmul_eq_zero_iff_of_coprime <| Nat.Coprime.pow_left j <|
    ...
    ⟨hx, TauCeti.groupCohomology.index_nsmul_eq_zero_of_map_eq_zero (P : Subgroup G) h⟩

/-- **The Sylow reduction of Tate's triviality criterion** (Milne II 3.10, last paragraph): if
`Hⁿ⁺¹(P, A) = 0` for a Sylow `p`-subgroup `P` of every prime `p`, then `Hⁿ⁺¹(G, A) = 0`. -/
theorem isZero_of_forall_sylow (h : ∀ (p : ℕ) [Fact p.Prime] (P : Sylow p G),
  ...
  suffices hsub : ∀ (m : ℕ) (x : groupCohomology A (n + 1)), 0 < m → m • x = 0 → x = 0 by
    have : Subsingleton (groupCohomology A (n + 1)) :=
      subsingleton_of_forall_eq 0 fun x ↦ hsub _ x Nat.card_pos (natCard_nsmul_eq_zero A n x)
  ...
  | prime_pow_mul a p m hp hpa hm ih =>
    ...
    exact ih x (Nat.pos_of_mul_pos_left hpos)
      (eq_zero_of_map_sylow_eq_zero A n p P (j := m) (by rw [← mul_smul, hx])
        ((ModuleCat.subsingleton_of_isZero (h p P)).allEq _ _))
