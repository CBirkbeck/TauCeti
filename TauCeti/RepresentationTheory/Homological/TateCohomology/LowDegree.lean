...
`H0π`, `HNegOneπ`, `groupHomology.H1π` and `groupHomology.cycles₁IsoOfIsTrivial` are maps out of
modules `ModuleCat.of R X`. Before `simp` looks a term up among its lemmas, it reduces the carrier
of `ModuleCat.of R X` to `X`, and `(Rep.trivial ℤ H ℤ).ρ` to `Representation.trivial ℤ H ℤ`, even
inside the implicit type arguments of a coercion, whereas a `simp` lemma is indexed by its
left-hand side as elaborated. The `simp` lemmas evaluating these maps therefore state their
left-hand sides through `dsimp% only`, and `range_norm_trivial_int` is stated over
`Representation.trivial ℤ H ℤ`, so that `simp` finds them. This follows #8315, which introduced
the convention for the `Formation` files.
