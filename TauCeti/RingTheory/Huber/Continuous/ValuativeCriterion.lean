...
/-- **The coarsening step.** A valuation of `R` that is `≤ 1` on `B`, `> 1` at `x` and strictly
below `1` on a spanning set of the ideal of definition, and which is *not* identically zero
there, yields a **continuous** valuation with the same two properties.

This is the whole of moves 2 and 3 of the module docstring. Stating it for an abstract `w₀`
rather than for the pullback of a valuation subring is what makes it a statement rather than a
cut: the convex subgroup, the coarsening and the extension are all built here and none of them
appears in the conclusion, because `Spv R` already hides the value group. The degenerate case —
every generator of value `0` — needs none of this and is handled by its caller. -/
private theorem exists_continuous_valuation_of_exists_ne_zero (P : PairOfDefinition R)
    {B : Subring R} (hA₀B : (P.ringOfDefinition : Set R) ⊆ B) {Γ₀ : Type*}
    [LinearOrderedCommGroupWithZero Γ₀] (w₀ : Valuation R Γ₀) (hBle : ∀ b ∈ B, w₀ b ≤ 1)
    {x : R} (hxgt : 1 < w₀ x) {S : Finset P.ringOfDefinition}
    (hS : Ideal.span (S : Set P.ringOfDefinition) = P.idealOfDefinition)
    (hgenlt : ∀ t ∈ S, w₀.comap P.ringOfDefinition.subtype t < 1)
    (hne : ∃ t ∈ S, w₀.comap P.ringOfDefinition.subtype t ≠ 0) :
    ∃ v : Spv R, v.IsContinuous ∧ (∀ b ∈ B, v.toValuativeRel.vle b 1) ∧
  ...
  obtain ⟨t₁, ht₁S, ht₁ne⟩ := hne
  have hSne : S.Nonempty := ⟨t₁, ht₁S⟩
  set g : Γ₀ := S.sup' hSne (fun t ↦ v₀ t) with hg
  ...
  set u : Γ₀ˣ := Units.mk0 g hgne with hu
  ...
  have hextB : ∀ b ∈ B, vext b ≤ 1 := by
    ...
    calc v₀ ⟨(t₀ : R) ^ k * b, hk⟩
        = w₀ ((t₀ : P.ringOfDefinition) : R) ^ k * w₀ b := by
  ...
  have hextx : 1 < vext x := by
    ...
    have hxgt' : (1 : Γ₀) < w₀ x := hxgt
    have hpos : (0 : Γ₀) < w₀ ((t₀ : P.ringOfDefinition) : R) ^ k :=
      pow_pos (zero_lt_iff.mpr ((hw₀app t₀).symm ▸ hv₀t₀ne)) k
    calc v₀ (t₀ ^ k) = w₀ ((t₀ : P.ringOfDefinition) : R) ^ k := hvals k
      _ < w₀ ((t₀ : P.ringOfDefinition) : R) ^ k * w₀ x :=
...
private theorem exists_continuous_valuation_of_not_isIntegral_of_isDomain [IsDomain R]
  ...
  · push Not at hzero
    exact exists_continuous_valuation_of_exists_ne_zero P hA₀B w₀ hBle hxgt hS hgenlt hzero
