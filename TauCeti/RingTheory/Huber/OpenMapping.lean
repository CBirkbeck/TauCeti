...
* `TauCeti.Huber.isOpenMap_mul_of_isOpen_span`: the scalar case — over a complete Tate ring,
  an open principal ideal makes multiplication by its generator an open map.
...
/-- **A continuous linear map with open range is open.** Corestricting to the range gives a
surjection onto a submodule that is complete, because an open subgroup of a complete group is
closed; `TauCeti.Huber.IsTateRing.isOpenMap` opens that corestriction, and the inclusion of an
open submodule is open.

This is the form the multiplication case below is an instance of: nothing in the argument uses
that the map is multiplication by a scalar. -/
theorem IsTateRing.isOpenMap_of_isOpen_range {A N : Type*} [CommRing A] [TopologicalSpace A]
    [IsTopologicalRing A] [IsTateRing A] [AddCommGroup N] [UniformSpace N] [IsUniformAddGroup N]
    [CompleteSpace N] [(𝓤 N).IsCountablyGenerated] [T0Space N] [NonarchimedeanAddGroup N]
    [Module A N] [ContinuousSMul A N] (f : N →ₗ[A] N) (hf : Continuous (f : N → N))
    (hr : IsOpen ((LinearMap.range f : Submodule A N) : Set N)) : IsOpenMap (f : N → N) := by
  have _ : CompleteSpace (LinearMap.range f) :=
    ((LinearMap.range f).toAddSubgroup.isClosed_of_isOpen hr).completeSpace_coe
  have hcont : Continuous (f.rangeRestrict : N → LinearMap.range f) :=
    continuous_induced_rng.mpr hf
  have hopen : IsOpenMap (f.rangeRestrict : N → LinearMap.range f) :=
    IsTateRing.isOpenMap f.rangeRestrict f.surjective_rangeRestrict hcont.continuousAt
  have hcomp : (f : N → N) = (Subtype.val : LinearMap.range f → N) ∘ f.rangeRestrict := by
    funext a
...
the ideal generated by `t` is open, then `x ↦ t * x` carries open sets to open sets. -/
theorem isOpenMap_mul_of_isOpen_span {t : A}
  ...
  have hrange : (LinearMap.range (LinearMap.lsmul A A t) : Submodule A A) = Ideal.span {t} := by
    ext x
    simp only [LinearMap.mem_range, LinearMap.lsmul_apply, smul_eq_mul,
      Ideal.mem_span_singleton']
    exact ⟨fun ⟨a, ha⟩ ↦ ⟨a, by rw [← ha]; ring⟩, fun ⟨a, ha⟩ ↦ ⟨a, by rw [← ha]; ring⟩⟩
  exact IsTateRing.isOpenMap_of_isOpen_range (LinearMap.lsmul A A t) (continuous_const_mul t)
    (by rw [hrange]; exact ht)
