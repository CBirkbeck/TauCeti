...
This is an instance of `TauCeti.AddSubgroup.closure_mul_closure`, after expressing `U` as the
additive closure of its carrier. -/
theorem weightMul_def (T : Fin k → Set A) (ν : Fin k →₀ ℕ) (U : AddSubgroup A) :
  ...
  calc
    weightMul T ν U =
        AddSubgroup.closure (weightPow T ν) * AddSubgroup.closure (U : Set A) := by
      rw [weightMul, AddSubgroup.closure_eq]
    _ = AddSubgroup.closure (weightPow T ν * (U : Set A)) :=
      AddSubgroup.closure_mul_closure _ _
