...
The definition and the basic calculus below are deliberately stated exactly as in
mathlib4#40013, so that the two can be identified once that pull request lands.
...
* `TauCeti.Huber.IsBounded.add`, `TauCeti.Huber.IsBounded.addSubgroupClosure`: over a
  nonarchimedean ring, sums and the generated additive subgroup of bounded sets stay bounded.
...
theorem IsBounded.union {S T : Set M} (hS : IsBounded S) (hT : IsBounded T) :
    IsBounded (S ∪ T) := by
  intro U hU
  obtain ⟨V₁, hV₁, hSV⟩ := hS U hU
  obtain ⟨V₂, hV₂, hTV⟩ := hT U hU
  refine ⟨V₁ ∩ V₂, inter_mem hV₁ hV₂, ?_⟩
  rw [Set.mul_union]
  exact Set.union_subset ((Set.mul_subset_mul_right Set.inter_subset_left).trans hSV)
    ((Set.mul_subset_mul_right Set.inter_subset_right).trans hTV)
...
theorem IsBounded.add {S T : Set A} (hS : IsBounded S) (hT : IsBounded T) : IsBounded (S + T) := by
  ...
  obtain ⟨V₁, hV₁, hSV⟩ := hS _ hG
  obtain ⟨V₂, hV₂, hTV⟩ := hT _ hG
  refine ⟨V₁ ∩ V₂, inter_mem hV₁ hV₂, ?_⟩
...
theorem isBounded_image_ringEquiv_iff (e : A ≃+* B) (he : Continuous e) (he' : Continuous e.symm)
  ...
  ⟨fun h ↦ by simpa [Set.image_image] using h.ringEquiv e.symm he' (by simpa using he),
    fun h ↦ h.ringEquiv e he he'⟩
