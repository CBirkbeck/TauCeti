...
* `locNhd_preimage_eq_locIdeal_pow`: the image and preimage along the subtype embedding are
...
private theorem coe_smul_locSubring (P : PairOfDefinition A) (T : Finset A) (s : A)
    ...
    ((r • d : locSubring P T s) : Localization.Away s) =
      (r : Localization.Away s) * (d : Localization.Away s) := by
  rw [smul_eq_mul]
...
theorem locNhd_leftMul [IsTopologicalRing A] (P : PairOfDefinition A) (T : Finset A) (s : A)
  ...
  | H p =>
    ...
    induction k with
    | zero =>
      -- `s ^ 0 = 1`, and `mk a 1` is the structure map, which is what `locNhd_algMap_step` is
      -- stated for; `Localization.mk_one_eq_algebraMap` is the step that identifies them.
      have hmk : Localization.mk a (⟨s ^ 0, ⟨0, rfl⟩⟩ : Submonoid.powers s) =
          algebraMap A (Localization.Away s) a := by
        rw [show (⟨s ^ 0, ⟨0, rfl⟩⟩ : Submonoid.powers s) = 1 from Subtype.ext (pow_zero s)]
        exact Localization.mk_one_eq_algebraMap a
      rw [hmk]
...
theorem continuous_of_continuous_algebraMap_of_isPowerBounded {B : Type*}
  ...
  · rintro _ ⟨b, hb, rfl⟩ r
    rw [Subring.coe_subtype, MulMemClass.coe_mul, coe_algebraMapD]
