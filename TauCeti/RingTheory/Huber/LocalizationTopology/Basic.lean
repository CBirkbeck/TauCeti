...
* `locBaseSubring B T s S` : the subring `B[t₁/s, …, tₙ/s]` generated by a base subring `B` and
  the fractions. Both subrings of `Aₛ` that matter are instances of it — `B = A₀` gives the
  candidate ring of definition below, and `B = A⁺` gives the generator of the plus part of the
  rational localisation (roadmap Layer 3.1). The construction and its interface are stated once,
  here.
* `locSubring P T s S` : the candidate ring of definition `D = A₀[t₁/s, …, tₙ/s]`, *defined* as
  `locBaseSubring` at the ring of definition. Its own interface lemmas are the general ones
  specialised, so the two cannot drift apart.
...
/-- **`B[t₁/s, …, tₙ/s]`, the subring of `S` generated by a base subring and the fractions.**
Two subrings of this shape matter: with `B = A₀` it is the candidate ring of definition
`PairOfDefinition.locSubring`, and with `B = A⁺` it is the generator of the plus part `A(T/s)⁺` of
the rational localisation (roadmap Layer 3.1). Only the base differs, so the construction and its
interface are stated once here. -/
noncomputable def locBaseSubring (B : Subring A) (T : Finset A)
    (s : A) (S : Type*) [CommRing S] [Algebra A S] [IsLocalization.Away s S] : Subring S :=
  Subring.closure
    ((algebraMap A S) '' (B : Set A) ∪ Set.range (fun t : T ↦ divBy (t : A) s))

omit [TopologicalSpace A] in
/-- The generators of `B[T/s]`. The body is not exported, so this is how a consumer reaches
them. -/
theorem locBaseSubring_def (B : Subring A) (T : Finset A) (s : A)
    (S : Type*) [CommRing S] [Algebra A S] [IsLocalization.Away s S] :
    locBaseSubring B T s S = Subring.closure
      ((algebraMap A S) '' (B : Set A) ∪ Set.range (fun t : T ↦ divBy (t : A) s)) := (rfl)

omit [TopologicalSpace A] in
/-- An element of the base maps into `B[T/s]`. -/
theorem algebraMap_mem_locBaseSubring (B : Subring A) (T : Finset A) (s : A)
    (S : Type*) [CommRing S] [Algebra A S] [IsLocalization.Away s S] {a : A} (ha : a ∈ B) :
    algebraMap A S a ∈ locBaseSubring B T s S :=
  Subring.subset_closure (Set.mem_union_left _ ⟨a, ha, rfl⟩)

omit [TopologicalSpace A] in
/-- Each fraction `t/s` (for `t ∈ T`) belongs to `B[T/s]`. -/
theorem divBy_mem_locBaseSubring (B : Subring A) (T : Finset A) (s : A)
    (S : Type*) [CommRing S] [Algebra A S] [IsLocalization.Away s S] {t : A} (ht : t ∈ T) :
    divBy t s ∈ locBaseSubring B T s S :=
  Subring.subset_closure (Set.mem_union_right _ ⟨⟨t, ht⟩, rfl⟩)

omit [TopologicalSpace A] in
/-- **The universal property of `B[T/s]`**: a subring contains it exactly when it contains the
image of `B` and every distinguished fraction. -/
theorem locBaseSubring_le_iff (B : Subring A) (T : Finset A) (s : A)
    (S : Type*) [CommRing S] [Algebra A S] [IsLocalization.Away s S] {C : Subring S} :
    locBaseSubring B T s S ≤ C ↔
      ((algebraMap A S) '' (B : Set A) ⊆ (C : Set S) ∧ ∀ t ∈ T, divBy t s ∈ C) := by
  rw [locBaseSubring_def, Subring.closure_le, Set.union_subset_iff]
  refine and_congr_right fun _ ↦ ⟨fun h t ht ↦ h ⟨⟨t, ht⟩, rfl⟩, ?_⟩
  rintro h _ ⟨⟨t, ht⟩, rfl⟩
  exact h t ht

omit [TopologicalSpace A] in
/-- It is monotone in the base subring. -/
theorem locBaseSubring_mono_base {B C : Subring A} (h : B ≤ C) (T : Finset A) (s : A)
    (S : Type*) [CommRing S] [Algebra A S] [IsLocalization.Away s S] :
    locBaseSubring B T s S ≤ locBaseSubring C T s S :=
  (locBaseSubring_le_iff B T s S).mpr
    ⟨fun _ hx ↦ by
        obtain ⟨a, ha, rfl⟩ := hx
        exact algebraMap_mem_locBaseSubring C T s S (h ha),
      fun _ ht ↦ divBy_mem_locBaseSubring C T s S ht⟩
...
noncomputable def locSubring (P : PairOfDefinition A) (T : Finset A)
    (s : A) (S : Type*) [CommRing S] [Algebra A S] [IsLocalization.Away s S] : Subring S :=
  locBaseSubring P.ringOfDefinition T s S

/-- `D` is `locBaseSubring` at the ring of definition — by definition, not by coincidence: this
is how `locSubring` is defined, so the two cannot drift apart. -/
theorem locSubring_eq_locBaseSubring (P : PairOfDefinition A) (T : Finset A) (s : A)
    (S : Type*) [CommRing S] [Algebra A S] [IsLocalization.Away s S] :
    locSubring P T s S = locBaseSubring P.ringOfDefinition T s S := (rfl)
...
theorem locSubring_def (P : PairOfDefinition A) (T : Finset A) (s : A)
  ...
  locBaseSubring_def P.ringOfDefinition T s S
...
theorem divBy_mem_locSubring (P : PairOfDefinition A)
  ...
  divBy_mem_locBaseSubring P.ringOfDefinition T s S ht
...
theorem algebraMap_mem_locSubring (P : PairOfDefinition A)
  ...
  algebraMap_mem_locBaseSubring P.ringOfDefinition T s S ha
