...
* `locBaseSubring B T s S` : the subring `B[t₁/s, …, tₙ/s]` generated by a base subring `B` and
  ...
  rational localisation (roadmap Layer 3.1).
* `locSubring P T s S` : the candidate ring of definition `D = A₀[t₁/s, …, tₙ/s]`, which is
  `locBaseSubring` at the ring of definition (`locSubring_eq_locBaseSubring`, by `rfl`).
...
/-- `D` is `locBaseSubring` at the ring of definition: the two constructions differ only in their
base subring. -/
