...
* `locIdealImage_le_of_image_subset` and `isOpen_map_algebraMap_locTopology`: an ideal of `Aₛ`
  containing the image of `Iⁿ` contains the whole `n`-th neighbourhood, so the ideal generated by
  the image of an open ideal of `A` is open.
...
theorem locIdealImage_le_of_image_subset (P : PairOfDefinition A) (T : Finset A) (s : A)
    ...
    locIdealImage P T s S n ≤ K.toAddSubgroup := by
  have hspan : (locIdeal P T s S ^ n : Ideal (locSubring P T s S)) ≤
      K.comap (locSubring P T s S).subtype := by
    rw [locIdeal_pow_eq_span, Ideal.span_le]
    rintro _ ⟨b, hb, rfl⟩
    exact hK ⟨(b : A), (P.mem_idealImage n).mpr ⟨b, hb, rfl⟩, rfl⟩
  intro x hx
