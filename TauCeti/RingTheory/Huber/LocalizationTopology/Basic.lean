...
Stated as an equality rather than the containment its consumers happen to need, since both
directions are immediate from `locSubring_le_iff` and `locSubring_mono`. -/
