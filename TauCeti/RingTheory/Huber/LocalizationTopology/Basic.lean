...
* `exists_isTopologicallyNilpotent_rescaled`: over a Tate ring the two steps above combine — every
  presentation has a rescaled form with topologically nilpotent denominator, the same `D`, and the
  same standing hypothesis. It supplies the `IsLocalization.Away` instance the two steps assume.
...
theorem exists_isTopologicallyNilpotent_rescaled [IsTopologicalRing A] [IsTateRing A]
  ...
  have hassoc : Associated s (q ^ i * s) := ⟨hu.unit, by simp [mul_comm]⟩
