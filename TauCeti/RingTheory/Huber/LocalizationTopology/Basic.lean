...
the previous `D` together with that fraction.

This is the recursive step for `D` on a `Finset`, with `locSubring_empty` as its base: together
they reduce `locSubring` on any explicit finset to iterated `Subring.closure`. Stated as an
equality rather than the containment a consumer might need, so that it can rewrite in either
direction. -/
@[simp]
