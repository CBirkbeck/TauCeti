...
/-- Adjoining one more fraction lands inside the subring generated by the previous `D` together
with that fraction. This is the containment the induction in
`exists_pow_mul_locSubring_mem` steps along. -/
theorem locSubring_insert_le [DecidableEq A] (P : PairOfDefinition A) (t : A)
    (U : Finset A) (s : A)
    (S : Type*) [CommRing S] [Algebra A S] [IsLocalization.Away s S] :
    locSubring P (insert t U) s S ≤
      Subring.closure ((locSubring P U s S : Set S) ∪ {divBy t s}) := by
  refine (locSubring_le_iff P _ s S).mpr ⟨fun _ ha ↦ ?_, fun t' ht' ↦ ?_⟩
