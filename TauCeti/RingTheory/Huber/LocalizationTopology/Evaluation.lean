...
/-- **The subring generated by the image of `A` and the fractions is hit by the evaluation
map.** Both families of generators are hit — the constants by
`TauCeti.Huber.PairOfDefinition.rationalEvalHom_coe_weightedC` and the fractions by
`..._coe_weightedX` — and the range is a subring, so `Subring.closure_le` finishes. -/
private theorem adjoin_toSubring_le_comap_range {k : ℕ} (t : Fin k → A) (ht : ∀ i, t i ∈ T) :
    letI := locUniformSpace P T s S hden
    letI := isUniformAddGroup_locUniformSpace P T s S hden
    letI := isTopologicalRing_locUniformSpace P T s S hden
    (Algebra.adjoin A (Set.range fun i ↦ (divBy (t i) s : S))).toSubring
...
The range is a subring; it contains the image of `A` and every fraction `tᵢ/s`, so it contains the
subring those generate, which is the subalgebra `TauCeti.Localization.adjoin_divBy_eq_top` shows to
be all of `Aₛ`. -/
theorem coe_mem_range_rationalEvalHom {k : ℕ} (t : Fin k → A) (ht : ∀ i, t i ∈ T)
  ...
  have _ := isTopologicalRing_locUniformSpace P T s S hden
  have _ := isHuberRing_locUniformSpace P T s S hden
  have htop : Algebra.adjoin A (Set.range fun i ↦ (divBy (t i) s : S)) = ⊤ := by
    have h := adjoin_divBy_eq_top (S := S) s hspan
    rwa [show (Set.range fun x : (Set.range t) ↦ (divBy (x : A) s : S))
        = Set.range fun i ↦ (divBy (t i) s : S) by
      rw [show (fun x : (Set.range t) ↦ (divBy (x : A) s : S))
            = (fun a ↦ (divBy a s : S)) ∘ ((↑) : (Set.range t) → A) from rfl,
        show (fun i ↦ (divBy (t i) s : S)) = (fun a ↦ (divBy a s : S)) ∘ t from rfl,
        Set.range_comp, Set.range_comp, Subtype.range_coe]] at h
  exact adjoin_toSubring_le_comap_range P T s S hden t ht (by rw [htop]; trivial)
