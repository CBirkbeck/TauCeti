...
* `TauCeti.Huber.PairOfDefinition.mem_range_rationalEvalHom_of_mem_adjoin`: the range of the
...
* `TauCeti.Huber.PairOfDefinition.coe_mem_range_rationalEvalHom` and
  ...
  unit ideal that subalgebra is everything, so the range already contains `Aₛ` and is therefore
  dense in `A⟨T/s⟩`.
...
subalgebra is everything, which is what generating the unit ideal buys. -/
theorem mem_range_rationalEvalHom_of_mem_adjoin {k : ℕ} (t : Fin k → A) (ht : ∀ i, t i ∈ T)
...
theorem coe_mem_range_rationalEvalHom {k : ℕ} (t : Fin k → A) (ht : ∀ i, t i ∈ T)
  ...
  have hrange : (Set.range fun y : (Set.range t) ↦ (divBy (y : A) s : S))
    ...
    · rintro ⟨i, rfl⟩
      exact ⟨⟨t i, i, rfl⟩, rfl⟩
  refine mem_range_rationalEvalHom_of_mem_adjoin P T s S hden t ht ?_
