...
/-- **The `A`-subalgebra generated by the fractions lands in the range of the evaluation map**,
after the completion map. Step towards
`TauCeti.Huber.PairOfDefinition.coe_mem_range_rationalEvalHom`, which is the case where that
subalgebra is everything. -/
...
/-- **The evaluation map has dense range when the numerators generate the unit ideal.**
