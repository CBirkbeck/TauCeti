...
of them — so on its own it exhibits `A⟨T/s⟩` as generated by nothing, and **nothing here says
that map is surjective**. Surjectivity for a suitably exhaustive family is what Wedhorn's Examples
6.38 and 6.39 assert, and it is not proved here.
