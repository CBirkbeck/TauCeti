...
first because `A⁺ ⊆ A°` transfers along `A → Aₛ → A⟨T/s⟩`, the second because every element of `D`
is power-bounded in `Aₛ`. `(A_U)°` is a subring, so the generated subring stays inside it. -/
theorem completedPlusSubring_le_powerBoundedSubring [IsHuberRing A] (P : PairOfDefinition A)
