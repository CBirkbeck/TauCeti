...
which is the same factor-of-an-inverted-element argument as `isUnit_algebraMap_of_dvd` here. Its
...
theorem isUnit_toCompletionLoc_of_dvd {a : A} (h : a ∣ s) :
  ...
  exact (isUnit_algebraMap_of_dvd s h).map UniformSpace.Completion.coeRingHom
