...
* `TauCeti.Huber.IsTateRing.exists_presentation_isStrictMap_isOpenMap`: over a complete
  noetherian Tate ring, a finite complete metrisable module has a presentation
...
The presentation is Mathlib's `Module.FinitePresentation.exists_fin'`, available because a finite
module over a noetherian ring is finitely presented. Both maps are continuous by
`IsModuleTopology.continuous_of_linearMap`, since `Aⁿ` and `Aᵐ` carry the module topology. `p` is
open by the open mapping theorem `TauCeti.Huber.IsTateRing.isOpenMap`, and `u` is strict by
`TauCeti.Huber.IsTateRing.isStrictMap_of_isClosed_range`, its range being closed in `Aᵐ` by
`TauCeti.Huber.isClosed_of_isNoetherian` — the one place, beyond the existence of the
presentation, where `A` being noetherian enters.

The hypotheses on `M` are those of `TauCeti.Huber.IsTateRing.isModuleTopology`: a finite complete
metrisable topological `A`-module, its topology *given* rather than constructed. Wedhorn's
canonical topology of Proposition 6.18(1) is such a topology, but it is not built here, exactly as
for `TauCeti.Huber.isClosed_of_isNoetherian`. -/
theorem IsTateRing.exists_presentation_isStrictMap_isOpenMap :
  ...
  have hp : Continuous p := IsModuleTopology.continuous_of_linearMap p
  exact ⟨n, m, u, p, hu, hp, hsurj, hexact,
    IsTateRing.isStrictMap_of_isClosed_range u hu.continuousAt (isClosed_of_isNoetherian _),
    IsTateRing.isOpenMap p hsurj hp.continuousAt⟩
