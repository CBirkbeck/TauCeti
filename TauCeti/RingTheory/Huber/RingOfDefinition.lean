...
elements for `A₀[T]`, and directly from `IsBounded.mul` for `A₀ ⊔ A₁`, whose elements are already
`ℤ`-combinations of products.
...
* `TauCeti.Huber.isBounded_subringClosure_union`: the subring generated by two bounded subrings
  is bounded.
...
omit [TopologicalSpace A] in
/-- The additive subgroup generated by `B₀ · B₁` is closed under multiplication, because the ring
is commutative and both factors are subrings: `(b₀b₁)(b₀'b₁') = (b₀b₀')(b₁b₁')`. -/
private theorem mul_mem_addSubgroupClosure_mul (B₀ B₁ : Subring A) {x y : A}
    (hx : x ∈ AddSubgroup.closure ((B₀ : Set A) * (B₁ : Set A)))
    (hy : y ∈ AddSubgroup.closure ((B₀ : Set A) * (B₁ : Set A))) :
    x * y ∈ AddSubgroup.closure ((B₀ : Set A) * (B₁ : Set A)) := by
  induction hx, hy using AddSubgroup.closure_induction₂ with
  | mem u v hu hv =>
    obtain ⟨b₀, hb₀, b₁, hb₁, rfl⟩ := Set.mem_mul.mp hu
    obtain ⟨c₀, hc₀, c₁, hc₁, rfl⟩ := Set.mem_mul.mp hv
    apply AddSubgroup.subset_closure
    convert Set.mul_mem_mul (s := (B₀ : Set A)) (t := (B₁ : Set A))
      (B₀.mul_mem hb₀ hc₀) (B₁.mul_mem hb₁ hc₁) using 1
    ring
  | zero_left => simp
  | zero_right => simp
  | add_left _ _ _ _ _ _ hx hy => simpa [add_mul] using AddSubgroup.add_mem _ hx hy
...
/-- The subring generated by two bounded subrings is bounded. This is the boundedness input for
the other half of Wedhorn's Corollary 6.4, the one about products of rings of definition.

Unlike `isBounded_subringClosure_union_finset` no induction is needed: every element of the
generated subring is already a `ℤ`-combination of products `b₀ * b₁`, so `IsBounded.mul` and
`IsBounded.addSubgroupClosure` suffice. -/
theorem isBounded_subringClosure_union [NonarchimedeanAddGroup A]
    (B₀ B₁ : Subring A) (hB₀ : IsBounded (B₀ : Set A)) (hB₁ : IsBounded (B₁ : Set A)) :
    IsBounded (Subring.closure ((B₀ : Set A) ∪ (B₁ : Set A)) : Set A) := by
  let C : Subring A :=
    { carrier := AddSubgroup.closure ((B₀ : Set A) * (B₁ : Set A))
      mul_mem' := fun hx hy ↦ mul_mem_addSubgroupClosure_mul B₀ B₁ hx hy
      one_mem' := by
        simpa using AddSubgroup.subset_closure
          (Set.mul_mem_mul (s := (B₀ : Set A)) (t := (B₁ : Set A)) B₀.one_mem B₁.one_mem)
      zero_mem' := (AddSubgroup.closure _).zero_mem
      add_mem' := fun hx hy ↦ (AddSubgroup.closure _).add_mem hx hy
      neg_mem' := fun hx ↦ (AddSubgroup.closure _).neg_mem hx }
  have hC_carrier : (C : Set A) = AddSubgroup.closure ((B₀ : Set A) * (B₁ : Set A)) := rfl
  have hgen : (B₀ : Set A) ∪ (B₁ : Set A) ⊆ (C : Set A) := by
    rintro x (hx | hx)
    · rw [hC_carrier]
      simpa using AddSubgroup.subset_closure
        (Set.mul_mem_mul (s := (B₀ : Set A)) (t := (B₁ : Set A)) hx B₁.one_mem)
    · rw [hC_carrier]
      simpa using AddSubgroup.subset_closure
        (Set.mul_mem_mul (s := (B₀ : Set A)) (t := (B₁ : Set A)) B₀.one_mem hx)
  exact (hB₀.mul hB₁).addSubgroupClosure.subset (Subring.closure_le.mpr hgen)
...
Openness is inherited from `A₀`, and finite generation survives because the new ideal is a
`Ideal.map` — an image, not a preimage — of the old one. Boundedness of `B` is the only genuinely
new input, and it is exactly what the two halves of Wedhorn's Corollary 6.4 supply in their two
different ways: `isBounded_subringClosure_union_finset` for `A₀[T]` and
`isBounded_subringClosure_union` for `A₀ · A₁`.

The containment `A₀ ≤ B` is essential and not merely convenient. For a *smaller* subring the
ideal would arrive by `Ideal.comap`, which does not preserve finite generation — which is why the
intersection half of Corollary 6.4 is not an instance of this. -/
...
/-- **Wedhorn Corollary 6.4, the product half.** The subring generated by two rings of definition
is again a ring of definition; its ideal of definition is the image of the first one's.

The construction is `enlarge` applied to `A₀ ⊔ A₁`, which contains `A₀`, with the boundedness
supplied by `isBounded_subringClosure_union`. The choice of `P` over `Q` is immaterial to the
underlying subring — only to which ideal is carried along — since `⊔` is symmetric. -/
def sup (P Q : PairOfDefinition A) : PairOfDefinition A :=
  letI := P.toNonarchimedeanRing
  P.enlarge (P.ringOfDefinition ⊔ Q.ringOfDefinition) le_sup_left <| by
    rw [← Subring.closure_eq P.ringOfDefinition, ← Subring.closure_eq Q.ringOfDefinition,
      ← Subring.closure_union]
    exact isBounded_subringClosure_union P.ringOfDefinition Q.ringOfDefinition
      P.isBounded_ringOfDefinition Q.isBounded_ringOfDefinition
...
/-- Both rings of definition sit inside their join. -/
theorem le_sup_ringOfDefinition (P Q : PairOfDefinition A) :
