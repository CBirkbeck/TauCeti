...
/-- The subring generated by the products `b * a ^ n` with `b` in a subring `B` is, as an additive
subgroup, exactly their additive span: `1` is such a product, and any two of them multiply to a
third. -/
private theorem toAddSubgroup_subringClosure_mul_range_pow (B : Subring A) (a : A) :
    (Subring.closure ((B : Set A) * Set.range (a ^ · : ℕ → A))).toAddSubgroup =
      AddSubgroup.closure ((B : Set A) * Set.range (a ^ · : ℕ → A)) := by
  have hmem : ∀ {b : A}, b ∈ B → ∀ n : ℕ,
      b * a ^ n ∈ AddSubgroup.closure ((B : Set A) * Set.range (a ^ · : ℕ → A)) :=
    fun hb n ↦ AddSubgroup.subset_closure (Set.mul_mem_mul hb ⟨n, rfl⟩)
  refine Subring.toAddSubgroup_closure_of_one_mem_of_mul_mem (by simpa using hmem B.one_mem 0) ?_
  -- two generators multiply to a third: `(b₁ * a ^ k₁) * (b₂ * a ^ k₂) = b₁ * b₂ * a ^ (k₁ + k₂)`
  rintro _ ⟨b₁, hb₁, _, ⟨k₁, rfl⟩, rfl⟩ _ ⟨b₂, hb₂, _, ⟨k₂, rfl⟩, rfl⟩
  convert hmem (B.mul_mem hb₁ hb₂) (k₁ + k₂) using 1
  ring
...
theorem isBounded_subringClosure_union_finset [NonarchimedeanAddGroup A] (B₀ : Subring A)
    (hB₀ : IsBounded (B₀ : Set A)) (T : Finset A) (hT : ∀ t ∈ T, IsPowerBounded t) :
    IsBounded (Subring.closure ((B₀ : Set A) ∪ (T : Set A)) : Set A) := by
  classical
  induction T using Finset.induction with
  | empty => simpa [Subring.closure_eq] using hB₀
  | @insert a S ha ih =>
    set B := Subring.closure ((B₀ : Set A) ∪ (S : Set A))
    have hM : IsBounded ((B : Set A) * Set.range (a ^ · : ℕ → A)) :=
      (ih fun t ht ↦ hT t (Finset.mem_insert_of_mem ht)).mul
        (isPowerBounded_iff.mp (hT a (Finset.mem_insert_self a S)))
    -- each generator is `b * a ^ n` for a suitable `b ∈ B`: `n = 1` for `a`, `n = 0` for `B`
    have hmem : ∀ {b : A}, b ∈ B → ∀ n : ℕ, b * a ^ n ∈ (B : Set A) * Set.range (a ^ · : ℕ → A) :=
      fun hb n ↦ Set.mul_mem_mul hb ⟨n, rfl⟩
    have hgen : (B₀ : Set A) ∪ (insert a S : Finset A) ⊆
        (B : Set A) * Set.range (a ^ · : ℕ → A) := by
      rw [Finset.coe_insert, Set.union_insert]
      rintro x (rfl | hx)
      · simpa using hmem B.one_mem 1
      · simpa using hmem (Subring.subset_closure hx) 0
    refine hM.addSubgroupClosure.subset fun x hx ↦ ?_
    rw [← toAddSubgroup_subringClosure_mul_range_pow B a]
    exact Subring.closure_mono hgen hx
