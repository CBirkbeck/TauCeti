...
topology of any topology Mathlib puts on `MvPowerSeries`; it asks only that `B` be nonarchimedean,
a basis by open *subgroups*. The weaker hypothesis on the target is the one that matters here: a
