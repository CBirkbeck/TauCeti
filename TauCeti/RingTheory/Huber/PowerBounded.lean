...
* `TauCeti.Huber.isBounded_submonoidClosure` and `TauCeti.Huber.isBounded_subringClosure`
  ...
  power-bounded elements is bounded. Neither needs multiplication to be continuous;
  `isBounded_singleton_one` supplies the empty case, where `isBounded_singleton` would not.
* `TauCeti.Huber.isBounded_subringClosure_union`: the same with a bounded subring adjoined,
  which is the boundedness input in Wedhorn's Corollary 6.4.
...
* [Wedhorn, *Adic Spaces*][wedhorn_adic], Definition 5.25, Definition 5.27, Remark 5.28,
  Proposition 5.30 and Corollary 6.4.
...
`B₀[C]` is the join `B₀ ⊔ Subring.closure C`, bounded on the left by hypothesis and on the right
by `isBounded_subringClosure`. This is the boundedness input in Wedhorn's Corollary 6.4. -/
theorem isBounded_subringClosure_union (B₀ : Subring A) (hB₀ : IsBounded (B₀ : Set A))
    {C : Set A} (hC : C.Finite) (h : ∀ c ∈ C, IsPowerBounded c) :
    IsBounded (Subring.closure ((B₀ : Set A) ∪ C) : Set A) := by
  rw [Subring.closure_union, Subring.closure_eq]
  exact isBounded_sup B₀ _ hB₀ (isBounded_subringClosure hC h)
