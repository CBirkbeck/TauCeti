...
* `TauCeti.Huber.isBounded_subringClosure_union_finset`: the same with a bounded subring adjoined,
...
`B₀[T]` is the join `B₀ ⊔ Subring.closure T`, bounded on the left by hypothesis and on the right
by `isBounded_subringClosure`. This is the boundedness input in Wedhorn's Corollary 6.4. -/
theorem isBounded_subringClosure_union_finset (B₀ : Subring A) (hB₀ : IsBounded (B₀ : Set A))
    (T : Finset A) (hT : ∀ t ∈ T, IsPowerBounded t) :
