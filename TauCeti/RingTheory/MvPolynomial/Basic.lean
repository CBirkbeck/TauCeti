...
-- `MvPolynomial.algebraMvPolynomial` is deliberately not a global instance: it would produce a
-- diamond for `Algebra (MvPolynomial σ R) (MvPolynomial σ (MvPolynomial σ S))`. Mathlib installs
-- it locally wherever the `MvPolynomial σ R`-algebra structure on `MvPolynomial σ S` is wanted,
-- and the pushout and scalar-tower instances used below are stated under it.
...
theorem MvPolynomial.finite_map {σ R S : Type*} [CommRing R] [CommRing S] {f : R →+* S}
  ...
  -- `MvPolynomial σ S` is the base change of `S` along `R → MvPolynomial σ R`; stating this as a
  -- `have` fixes the `MvPolynomial σ R`-algebra structure to be `algebraMvPolynomial`, which is
  -- the one Mathlib's pushout and scalar-tower instances are stated for
