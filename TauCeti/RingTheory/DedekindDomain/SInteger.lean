...
`{x : K | ∀ v ∉ S, v x ≤ 1}` as `Set.integer S K`, but proves nothing about its ring structure.
...
The key step is `map_comap_eq`: every ideal of the `S`-integers is generated by its contraction
to `R`, proved through the denominator ideal `denomIdeal K x = (R : x)` of an element of `K`.
