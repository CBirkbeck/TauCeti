...
`Exists.choose`. -/
lemma integer_extendedHom_mk0_comap (I : (Ideal (S.integer K))⁰) :
    ClassGroup.extendedHom R (S.integer K)
        (ClassGroup.mk0 ⟨(I : Ideal (S.integer K)).comap (algebraMap R (S.integer K)),
...
lemma integer_extendedHom_surjective :
  ...
  exact ⟨_, integer_extendedHom_mk0_comap K S I⟩
...
lemma integer_count_coeIdeal_map {v : HeightOneSpectrum R} (hv : v ∉ S) {J : Ideal R} :
  ...
  -- the two multiplicities dominate the same `k`, and `le_count_iff_le_pow` turns each side into
...
class of the contraction of `I` to `R`.

The `mk0`-level companion of `integerClassGroupEquiv_extendedHom`, and the one that fires in
practice: Mathlib tags `ClassGroup.extendedHom_mk` `@[simp]`, so on a goal whose argument is an
`extendedHom` applied to a representative, `simp` rewrites the inner `extendedHom` first and
`integerClassGroupEquiv_extendedHom` never matches. -/
@[simp] lemma integerClassGroupEquiv_mk0 (I : (Ideal (S.integer K))⁰) :
    integerClassGroupEquiv K S (ClassGroup.mk0 I) =
      QuotientGroup.mk (ClassGroup.mk0
        ⟨(I : Ideal (S.integer K)).comap (algebraMap R (S.integer K)),
          mem_nonZeroDivisors_iff_ne_zero.mpr
            (integer_comap_ne_bot K S (mem_nonZeroDivisors_iff_ne_zero.mp I.2))⟩) := by
  rw [← integer_extendedHom_mk0_comap K S I, integerClassGroupEquiv_extendedHom]
