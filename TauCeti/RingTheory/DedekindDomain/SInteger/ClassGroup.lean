...
`integerPrimeUnder`. -/
noncomputable abbrev integerContractedIdeal (I : (Ideal (S.integer K))⁰) : (Ideal R)⁰ :=
  ⟨(I : Ideal (S.integer K)).comap (algebraMap R (S.integer K)),
    mem_nonZeroDivisors_iff_ne_zero.mpr
      (integer_comap_ne_bot K S (mem_nonZeroDivisors_iff_ne_zero.mp I.2))⟩

@[simp] lemma coe_integerContractedIdeal (I : (Ideal (S.integer K))⁰) :
    (integerContractedIdeal K S I : Ideal R) =
      (I : Ideal (S.integer K)).comap (algebraMap R (S.integer K)) := rfl
...
lemma integer_extendedHom_mk0_comap (I : (Ideal (S.integer K))⁰) :
    ClassGroup.extendedHom R (S.integer K) (ClassGroup.mk0 (integerContractedIdeal K S I)) =
      ClassGroup.mk0 I := by
...
@[simp] lemma integerClassGroupEquiv_mk0 (I : (Ideal (S.integer K))⁰) :
    integerClassGroupEquiv K S (ClassGroup.mk0 I) =
      QuotientGroup.mk (ClassGroup.mk0 (integerContractedIdeal K S I)) := by
