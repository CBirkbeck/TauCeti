...
The factorization `I = ∏_v v ^ count v I` at unit level is
`FractionalIdeal.finprod_unitOfPrime_zpow_count` in
`TauCeti.RingTheory.DedekindDomain.Factorization`, where it sits beside the rest of the `count`
API; it lifts Mathlib's `finprod_heightOneSpectrum_factorization'` from fractional ideals to units
