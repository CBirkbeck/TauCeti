...
* `AdjoinRoot.mk_modByMonic`, `AdjoinRoot.exists_degree_lt_mk_eq`: for a monic relator, reducing
  a representative modulo it does not change its class, so every class is represented by a
...
`AdjoinRoot.mk_modByMonic` and `AdjoinRoot.exists_degree_lt_mk_eq` are adapted, with the same
statements and proofs, from Michael Stoll's `EllipticCurves` project
(`github.com/MichaelStollBayreuth/EllipticCurves`, Apache-2.0, pinned by
`TauCetiRoadmap/EllipticCurves/README.md` at `66889eada51a`), `EllipticCurves/Mathlib/Basic.lean`.
They are harvested here rather than in the file that consumes them because nothing about elliptic
curves enters either statement.
-/
...
/-- `AdjoinRoot.mk` is unchanged by reducing the representative modulo a monic relator. -/
@[simp]
lemma mk_modByMonic {g : R[X]} (hg : g.Monic) (q : R[X]) : mk g (q %ₘ g) = mk g q := by
