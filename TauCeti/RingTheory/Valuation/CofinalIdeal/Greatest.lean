...
* `TauCeti.Valuation.idealCofinalFor_of_span` and
  `TauCeti.Valuation.le_closure_singleton_of_idealCofinalFor` : the membership and maximality
  halves of **Lemma 7.2**.
...
/-! ### The membership and maximality halves of Wedhorn Lemma 7.2 -/
...
exists, and it meets `v(I)`. Lemma 7.2's minimality clause is not formalised; see the module
docstring for why.
...
Definition 7.3 invokes it. Some value of `I` lies in `cΓ_v(I)`: the construction picks the
subgroup generated by an attained value, and a power of that value is itself attained on `I`.

Nonvanishing is required, and not merely for convenience: if `v` vanishes identically on `I`
then `valueSet v I` is empty and nothing meets it. -/
