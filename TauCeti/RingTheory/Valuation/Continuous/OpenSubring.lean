...
If `B` is an open subring of `A` and *every* sublevel set `{b : B | v b < γ}` is open in `B`, then
`v` is continuous on `A`. This is the topological step towards Wedhorn's Lemma 7.44(2),
`Cont(A) = g⁻¹(Cont(B))` — **but it is not that lemma**, and the gap is recorded below rather than
papered over.
...
The trace `{b : B | v b < γ}` is in general far smaller than `{a : A | v a < γ}`, so openness does
...
of `0` is open. Openness of `B` is exactly what promotes the trace, open in the subspace topology,
...
* `Valuation.isOpen_lt_of_isOpen_subring_lt` : a sublevel set of `v` is open as soon as its trace on
  an open subring is.
* `Valuation.isContinuous_of_isOpen_subring_lt` : hence `v` is continuous as soon as every sublevel
  set has open trace.
...
arbitrary, the codomain is a monoid, and the ambient hypothesis is `SeparatelyContinuousAdd` rather
than a topological ring.
...
neighbourhood of `0` is open; openness of `B` is what makes the trace such a neighbourhood. -/
theorem isOpen_lt_of_isOpen_subring_lt {B : Subring A} (hB : IsOpen (B : Set A))
    {v : Valuation A Γ₀} {γ : Γ₀} (h : IsOpen {b : B | v (b : A) < γ}) :
    IsOpen {a : A | v a < γ} := by
  rcases eq_or_ne γ 0 with rfl | hγ
  · simp
  have himage : IsOpen ((↑) '' {b : B | v (b : A) < γ} : Set A) := hB.isOpenMap_subtype_val _ h
  have hzero : (0 : A) ∈ ((↑) '' {b : B | v (b : A) < γ} : Set A) :=
    ⟨⟨0, B.zero_mem⟩, by simpa using zero_lt_iff.mpr hγ, rfl⟩
...
/-- **A sufficient criterion for continuity, tested on an open subring.** If every sublevel set of
`v` has open trace on the open subring `B`, then `v` is continuous.

The hypothesis quantifies `γ` over the whole codomain, which is strictly stronger than continuity of
the restriction `v.comap B.subtype`: the latter is the attained-value predicate and supplies
...
theorem isContinuous_of_isOpen_subring_lt {B : Subring A} (hB : IsOpen (B : Set A))
    {v : Valuation A Γ₀} (h : ∀ γ : Γ₀, IsOpen {b : B | v (b : A) < γ}) : v.IsContinuous :=
