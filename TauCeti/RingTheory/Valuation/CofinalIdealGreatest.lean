...
inversion in `cofinalValueFor_closure_singleton_of_le`. Cofinality then propagates from `T` to
...
theorem isGreatestIdealCofinal_closure_singleton_of_span {v : Valuation A Γ₀}
    ...
    (hdom : ∀ t ∈ T, ∃ h0 : (MonoidWithZeroHom.ofClass v) t ≠ 0,
      valueGroup.mk (.ofClass v) 1 t (by simp) h0 ≤ h)
  ...
  · -- membership: generators → J (ideal) → I (radical)
    ...
    obtain ⟨h0, hle⟩ := hdom t ht
    exact cofinalValueFor_closure_singleton_of_le h0 hle hlt
