...
* The action of `(Perm (Fin 3))ᵐᵒᵖ` on `TauCeti.PermutationTriple.IsoClass n`: the six operations
  form a right action of the symmetric group on isomorphism classes of triples, generated by
  `swap01` and `swap1Inf` subject to the Coxeter relations.
...
/-- Reordering the branch points composes contravariantly: applying `ρ'` and then `ρ` is
reordering along `ρ' * ρ`. Once the `MulAction` instance below is in place, this is Mathlib's
`op_smul_op_smul`. -/
private theorem IsoClass.op_smul_op_smul (ρ ρ' : Perm (Fin 3)) (c : IsoClass n) :
    MulOpposite.op ρ • MulOpposite.op ρ' • c = MulOpposite.op (ρ' * ρ) • c := by
...
instance : MulAction (Perm (Fin 3))ᵐᵒᵖ (IsoClass n) where
  one_smul c := by
    obtain ⟨t, rfl⟩ := IsoClass.mk_surjective c
    exact IsoClass.op_smul_mk t 1
  mul_smul ρ ρ' c := by exact (IsoClass.op_smul_op_smul ρ.unop ρ'.unop c).symm
